...
Het gaat om tellingen in kerngebied Zavelberg (Leefdaal) en bijhorend controlegebied (Kortenberg).
...
#> ward ok
<!--
Geen van beide opties gebruikt. Nieuwe file geschreven "zandleemstreek_perimeters.gpkg" met enkel de zandleemstreek multipolygoon. Optie aan functie `path_to_perimeters()` toegevoegd zodat deze gemakkelijk wordt ingelezen. Als default leest de functie nog altijd "piloot_perimeters.gpkg" in zodat de originele targets pipeline nog werkt.
-->
...
#> ward ok
<!--
Shapefile 2021 omgezet naar .parquet. Simplificatie functie `path_to_lbg()`: geef jaar als argument en krijg bestand van dat jaar. 2022 nog niet beschikbaar.
-->
...
exclusie_landgebruik_osm() functie extra argument landuse en leisure toevoegen met als default:
...
Zodat we dit ook andere landgebruiken kunnen uitsluiten indien nodig
-->

#> ward
<!--
Functie aangepast. Indien je geen leisure wilt uitsluiten moet je NULL ingeven bij die variabele! Anders ga je automatisch de defaults uitsluiten. 
Mogelijkheden landuse in Zandleem:
forest, farmland, residential, grass, commercial, meadow, railway, farmyard
-> residential, commercial
Mogelijkheden leisure in Zandleem:
pitch, swimming_pool
-> niet relevant
Mogelijkheden man_made in Zandleem:
water_tower
-> niet relevant (super klein)

Landgebruiken extra uit te sluiten:
-Buffer van 50m rond snelwegen, treinsporen ... gebruiken (exclusie_buffer_osm() functie)
- zie ontoegankelijk issues_pilootmeetnet.Rmd
  -  serres: landuse = greenhouse_horticulture (niet aanwezig in zandleemstreek)
  -  autostrade 50 m buffer: highway = motorway -> ten noorden van Lovenjoel, geen effectieve overlap
  -  spoorwegen 50 m buffer: landuse = railway -> ten zuiden van kortenberg, bijna geen effectieve overlap
  -  aeroway 50 m buffer: -> ten noorden van kortenberg, geen effectieve overlap
  -  man_made = bridge (niet aanwezig in zandleemstreek)
- privégronden/paden?: https://wiki.openstreetmap.org/wiki/Map_features#Restrictions
    Misschien mogelijk om via trage wegen kaart de onverharde wegen van OSM eruit te halen die niet op de trage wegen kaart staan (risico dat we onterecht paadjes eruit halen?)
- wat met overlap andere cirkels?
-->
...
#> ward
<!--
Functie aangepast. Indien je geen water wilt includen moet je NULL ingeven bij die variabele! Anders ga je automatisch de defaults gebruiken. Functie nog niet klaar.

Afspraken Johannes:
-Sommige paadjes in OSM zijn privé, niet openbaar toegankelijk
-holle wegen: wel behouden, hopelijk lost visibiliteitsberekening dit op (eventueel gecombineerd met zenith-hoek berekening)
-grachten, beken niet te gebruiken
-->
...
#> ward
<!--
Problemen zichtbaarheid issues_pilootmeetnet.Rmd:
-zichtbaarheid > 50% van de cirkel, maar met laagstamboomgaarden is geen rekening gehouden (beschouwd als zichtbaar)
- HGL valt buiten de doelpop; enkel OL en HOL
...
list(
  tar_target(name = exclusie_osm_landgebruiken,
             command = exclusie_landgebruik_osm(
               ...
               osmdata = osm_belgium,
               landuse = c("residential", "commercial"),
               leisure = NULL,
               buffer_poly = 50, 
...
#> ward
<!--
staan al ingesteld als default, dus we kunnen deze gewoon aanpassen. Voor allocatie_leemstreek moeten we 0 invullen zodat de code blijft werken. Deze fractie wordt namelijk aan de Oostelijke leemstreek toegekend.  
mutate(allocatie_gebied = ifelse(Naam == "Oostelijke leemstreek",
                                     allocatie_leemstreek,
                                     1 - allocatie_leemstreek))
De naam "Oostelijke leemstreek" zal niet aanwezig zijn, dus als we 0 invullen, zal 100% aan de Zandleemstreek worden toegekend.
-->
