...
Enkel data met broedcode > 0 zijn weerhouden. De relatieve frequenties zullen dus niet exact hetzelfde zijn als in **2.2**.
...
Enkel data met broedcode > 0 zijn weerhouden. De relatieve frequenties zullen dus niet exact hetzelfde zijn als in **4.2**.
...
__Hier is ook geen rekening gehouden met detectiekansen.__
...
mean_maxabu_doelvogels_strat <- mas_vogels %>% 
  ...
  summarise(across(.cols = where(~ is.integer(.x) && min(.x) == 0),
                   .fns = ~max(.x)),
            .groups = "drop") %>%
  pivot_longer(cols = where(~ is.integer(.x) && min(.x) == 0),
               names_to = "naam",
               values_to = "max_aantal") %>%
  group_by(jaar, stratum, regio, naam) %>%
  summarise(n = n(),
            Hmisc::mean_cl_boot(max_aantal),
            .groups = "drop") %>%
