...
eerder als oefening/voorbereiding op meer complexe analyses verderop... Kunnen we de resultaten van **brms** repliceren met deze package?
</span>
...
The Poisson model is the simplest model with best fit. We have underdispersion:

```{r}
plot(dispersion_check(inlapois1))
```

A little more obserd than expected zeroes:
...
newdata <- cbind(newdata,
  inlapois1_pred2$summary.fitted.values[(nrow(gele_kwik_2022) + 1):nrow(data_pred),])
...
p_pois1_test <- plot(pred, plot = FALSE)[[1]] + 
                scale_y_continuous(sec.axis = sec_axis( ~. / cirkelopp * 1e6, 
                  name = ""), limits = c(0, 0.4)) +
                labs(y = "Aantal broedparen Gele kwikstaart per plot", x = "", 
                     title = "brms package") +
...
grid.arrange(p_pois1_test, test, ncol = 2)
```
...
# Detectiekans verschilt per regio en soortbeschermingszone
...
# Densiteit verschilt per regio en soortbeschermingszone
...
# Detectiekans en densiteit verschillen per regio en soortbeschermingszone
...
Het half-normal model waarbij zowel detectiekans als densiteit verschilt per regio, soortbeschermingszone en openheid landschap scoort het best.
...
```{r}
# Detectiekans verschilt per regio en soortbeschermingszone
# Densiteit verschilt per regio, soortbeschermingszone en openheid landschap
...
# Detectiekans verschilt per regio, soortbeschermingszone en openheid landschap
# Densiteit verschilt per regio en soortbeschermingszone
...
# Detectiekans verschilt per soortbeschermingszone
# Densiteit verschilt per regio, soortbeschermingszone en openheid landschap
...
# Detectiekans verschilt per regio en soortbeschermingszone
# Densiteit verschilt per soortbeschermingszone en openheid landschap
...
Het half-normal model waarbij detectiekans verschilt per soortbeschermingszone en densiteit verschilt per regio, soortbeschermingszone en openheid landschap scoort het best (eenvoudigst model met verschil in AIC < 2)
...
Detectiekans verschilt per soortbeschermingszone, maar we testen ook nog enkele combinaties voor densiteit.
...
# Detectiekans verschilt per soortbeschermingszone
# Densiteit verschilt per soortbeschermingszone
...
Het model waarbij detectiekans verschilt per soortbeschermingszone en densiteit verschilt per openheid landschap scoort het best.
...
# Detectiekans verschilt per soortbeschermingszone in interactie met 
# openheid landschap
# Densiteit verschilt per soortbeschermingszone in interactie met 
# openheid landschap
...
# Detectiekans verschilt per soortbeschermingszone in interactie met regio
...
# Detectiekans
# Binnen soortbeschermingszone
backTransform(linearComb(um_model_hn21["det"], c(1, 0)))

# Buiten soortbeschermingszone
backTransform(linearComb(um_model_hn21["det"], c(1, 1)))

# Predicties
new_df_dens <- data.frame(openheid = rep(c("OL", "HOL"), by = 2), 
                          regio = rep(c("De Moeren", "Oostelijke leemstreek"), 
                                      each = 2))
...
We kunnen visualiseren hoe de detectiefunctie verschilt binnen en buiten de soortbeschermingszones:
...
<span style="color: red;">
Hoe kunnen we hiermee rekening houden? Interactie werkt niet.
</span>
...
```{r}
test <- gdistsamp(~1,~1,~1, data = umf2, keyfun = "halfnorm", 
                  output = "density", unitsOut = "kmsq", mixture = "P")
test2 <- gdistsamp(~1,~1,~1, data = umf2, keyfun = "halfnorm", 
                   output = "density", unitsOut = "kmsq", mixture = "NB")
test3 <- gdistsamp(~openheid,~1,~sbp, data = umf2, keyfun = "halfnorm", 
                   output = "density", unitsOut = "kmsq", mixture = "P")
test4 <- gdistsamp(~openheid,~1,~sbp, data = umf2, keyfun = "halfnorm", 
                   output = "density", unitsOut = "kmsq", mixture = "NB")
```
...
```{r}
fm_list <- fitList(test, test2, test3, test4)
modSel(fm_list)
```
...
# Densiteit
# Half-open landschap
backTransform(linearComb(test4["lambda"], c(1, 0)))

# Open landschap
backTransform(linearComb(test4["lambda"], c(1, 1)))

# Detectiekans
# Binnen soortbeschermingszone
backTransform(linearComb(test4["det"], c(1, 0)))

# Buiten soortbeschermingszone
backTransform(linearComb(test4["det"], c(1, 1)))

# Predicties
Elambda2 <- predict(test4, type = "lambda", newdata = new_df_dens,
    appendData = TRUE)
Esigma2 <- predict(test4, type = "det", newdata = new_df_detect,
    appendData = TRUE)
```
...
brmpois_openheid1 <- brm(bf(aantal ~ regio + periode_in_jaar + openheid + 
                        ...
                        file_refit = "on_change")
summary(brmpois_openheid1)
plot(brmpois_openheid1)
```
...
```{r}
pp_check(brmpois_openheid1, type = "bars_grouped", ndraws = 100, group = "regio",
         facet_args = list(ncol = 1, scales = "free_y"))
...
<span style="color: red;">
als we voor een complex model kiezen (combi GLMM en distance sampling), is het misschien een goed idee om het met STAN te doen, je hebt een beter idee wat er gebeurt/wat je doet
</span>
...
<span style="color: red;">
met **inlabru** kunnen we rekening houden met spatiale autocorrlatie via een mesh
</span>
...
plot_dist_terr <- dst %>% as_tibble() %>% 
  ...
  ggplot() +
    ...
    geom_vline(xintercept = fun.mode(dst[dst != 0]), colour = "darkred", 
               size = 1.2) +
