...
\textcolour{red}{eerder als oefening/voorbereiding op meer complexe analyses verderop... Kunnen we de resultaten van *brms* repliceren met deze package?}
...
\textcolour{red}{Hoe kunnen we hiermee rekening houden? Interactie werkt niet.}
...
\textcolour{red}{als we voor een complex model kiezen (combi GLMM en distance sampling), is het misschien een goed idee om het met STAN te doen, je hebt een beter idee wat er gebeurt/wat je doet}
...
\textcolour{red}{met **inlabru** kunnen we rekening houden met spatiale autocorrlatie via een mesh}
...
Met de grotere fusieafstand van 600m hadden we `r nrow(result[[1]]$cluster)` clusters (= broedparen), nu hebben we met fusieafstand `r fusie_dist` m `r nrow(result2[[1]]$cluster)` clusters.
