...
```{r}
# Samenvattende tabel broedparen per stratum
df_dist_terr %>% group_by(regio, sbp, openheid, Area) %>%
  st_drop_geometry() %>%
...
df_dist_terr %>%
  ...
  mutate(Legende = paste0(openheid, "\n", sbp, " plan")) %>%
  ggplot() +
    geom_histogram(aes(x = distance, fill = Legende), bins = 30) +
    facet_wrap(~regio) +
