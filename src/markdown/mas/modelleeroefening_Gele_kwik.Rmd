...
renv::restore()
library(fdrtool) # to sample from halfnormal function
library(mapview)
library(INLA)
library(inlatools)
library(Distance)
...
In de GLMM's houden we echter geen rekening met een afnemende detectiekans naarmate de dieren zich verder bevinden. In \@ref(GLMM) hadden we de data geaggregeerd per telcirkel per telperiode, in deze sectie werken we met de directe observaties: waarnemingen van $n$ aantal broedparen Gele kwikstaart op een afstand van $x$ meter van het telpunt per telperiode in 2022.
...
<span style="color: red;">
als we voor een complex model kiezen (combi GLMM en distance sampling), is het misschien een goed idee om het met STAN te gebruiken, je hebt een beter idee wat er gebeurt/wat je doet  
  
je kan van brms naar STAN gaan (make stan...)
</span>
...
In de output van **territoria** krijgen we de centroide van de clusters (gemiddelde coordinaten van waarnemingen per cluster). Om niet te veel te moeten joinen berekenen we deze coordinaten opnieuw met de **sf** package. We maken een kolom `Sample.Label` met de het id van de plot, `size` is het aantal broedparen per cluster (overal 1), `regio`, `sbp` en `openheid` zijn covariaten, `Effort` is het aantal keer dat een plot is bezocht (overal 4). `Region.Label` is een label voor elke regio (zelfde als `regio`-kolom).
...
$p$ is dus de oppervlakte onder de detectiecurve. De volgende figuur toont een fictief voorbeeld van een histogram met afstanden van 0 tot 300 meter ($w = 300$). Het is duidelijk dat de er op grotere afstand minder waarnemingen zijn. Moest dit niet het geval zijn zouden de afstanden uniform verdeeld zijn als de blauwe box. Alles zou gezien zijn en $p = 1$. Dit is hier dus niet het geval. De rode curve is een half-normale detectiecurve gefit op de data. De oppervlakte onder de curve is de gemiddelde detectiekans (de geschatte fractie dieren die gezien zijn). De oppervlakte boven de curve de geschatte fractie dieren die gemist zijn. Als we de gemiddelde detectiekans weten, kunnen we dus eenvoudig berekenen hoeveel dieren we verwachten als we uitgaan van een uniforme verdeling van dieren over het telpunt. We controleren er dus voor dat het moeilijker is om dieren te detecteren op grote afstand terwijl we er van uitgaan dat er even veel zittenop grote als op kleine afstand.

```{r}
set.seed(20221121)
tibble(distances = rhalfnorm(500, theta = sqrt(pi/2) / 100)) %>%
  filter(distances <= 300) %>%
  ggplot(aes(x = distances)) +
    geom_rect(aes(xmin = 0, xmax = 300, ymin = 0, 
                  ymax = dhalfnorm(0, theta = sqrt(pi/2) / 100)), 
                  colour = "blue", size = 1, fill = "aliceblue", alpha = 0.05) +
    geom_histogram(aes(y = ..density..), breaks = seq(0, 300, by = 25), 
                   fill = "white", colour = "black") +
    stat_function(data = tibble(distances = 
                                rhalfnorm(500, theta = sqrt(pi/2) / 100)),
                  aes(x =  distances), fun = dhalfnorm, n = 1000, 
                  args = list(theta = sqrt(pi/2) / 100), colour = "red", 
                  size = 1) +
    geom_rect(aes(xmin = 0, xmax = 300, ymin = 0, ymax = 0.007978846), 
...
De gemiddelde detectiekans verschilt tussen hazard-ratio en half-normal functies. We bekijken model fit van modellen 6 (half-normal) 18 (hazard-rate) m.b.v. QQ-plots en probability density function plots. Voor punttransectstudies geven probability density function plots een beter idee van modelfit dan de detectiefunctieplots. Dit komt omdat bij het plotten van de detectiefunctie voor punttransectgegevens, de histogram opnieuw moet worden geschaald om rekening te houden met de geometrie van de punt. Punten op de plot geven de kans op detectie voor elke waarneming aan.
...
We hebben de studieregio in 2 strata opgesplits: De Moeren en de Oostelijke leemstreek. Beschouw $A(s)$ de totale oppervlakte van stratum $s$ tot waar we de abundantieschatting willen extrapoleren en $a(s)$ het gesampled gebied.

$$
a(s) = \pi w^2 T
$$
Met $T$ het aantal plots ($n_p$) vermenigvuldigd met het aantal bezoeken (effort). In ons geval hebben we dus $a(s) = \pi 300^2 n_p 4$. Dan is de geschatte abundantie in het gesampled gebied voor elk stratum $s$:

$$
\hat{N}_C(s) = \sum_{i=1}^n \frac{c_i(s)}{\hat{p}(\mathbf{z_i})}
$$
Met $c_i(s)$ de clustergrootte in stratum $s$ voor waarneming $i$. In ons geval 1 als de waarneming in stratum $i$ voorkomt en 0 indien niet. De geschatte abundantie in het gesampled gebied kan dan worden opgeschaald naar de totale oppervlakte van het stratum via:

$$
\hat{N}(s) = \frac{A(s)}{a(s)}\hat{N}_C(s)
$$

De geschatte densiteit is dan $\hat{N}(s) / A(s)$. De abundanties kunnen ook opgeteld worden voor een totale schatting en een totale densiteit door de totale schatting te delen door de totale oppervlakte (som van oppervlaktes per stratum). Voor berekening van de confidence limits zie `?dht`.  

Wie zien dat deze estimates ook licht verschillen tussen de modellen. Het geschatte aantal broedparen en de standard errors van het Hazard-rate model zijn telkens lager dan deze van het half-normal model: 
...
rbind(df_results_hr5, df_results_hn5) %>%
  select(model, regio, variable, Estimate, se, lcl, ucl) %>%
  arrange(model, regio, variable) %>%
  kable(digits = 3)
```
