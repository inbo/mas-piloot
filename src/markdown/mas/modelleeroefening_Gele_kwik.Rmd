...
Kunnen we deze plots repliceren in `ggplot()`? Herschaling onduidelijk. Combinatie van volgende links geeft een oplossing:  
https://groups.google.com/g/distance-sampling/c/Xf9KnmPdXuo/m/7PhjBe3YAAAJ  
https://rdrr.io/cran/mrds/src/R/plot.ds.R

We need some functions from the **mrds** package:
...
keyfct.hz <- function(distance, key.scale, key_shape_hr) {
  return(1 - exp(-(distance/key.scale)^(-key_shape_hr)))
  }
...
We calculate detection probability values:
...
# Gemiddelde
mat4 <- matrix(1 / 2, nrow = 100, ncol = 3)
mat4[, 1] <- 1
key.scale4 <- scalevalue(hr_model$ddf$ds$aux$ddfobj$scale$parameters, mat4)

# Gewogen gemiddelde
mat5 <- matrix(c(1, 308 / 387, 119 / 387), nrow = 100, ncol = 3, byrow = TRUE)
key.scale5 <- scalevalue(hr_model$ddf$ds$aux$ddfobj$scale$parameters, mat5)
...
y.val4 <- keyfct.hz(distances, key.scale4, key_shape_hr)
y.val5 <- keyfct.hz(distances, key.scale5, key_shape_hr)
...
df_hazard <- rbind(data.frame(dist = distances, y_val = y.val0, 
               ...
               data.frame(dist = distances, y_val = y.val4, 
                   Legende = "ongewogen gemiddelde"),
               data.frame(dist = distances, y_val = y.val5, 
                   Legende = "gewogen gemiddelde"))
...
mat4 <- matrix(1 / 2, nrow = 100, ncol = 3)
mat4[, 1] <- 1
key.scale4 <- scalevalue(hn_model$ddf$ds$aux$ddfobj$scale$parameters, mat4)

# Gewogen gemiddelde
mat5 <- matrix(c(1, 308 / 387, 119 / 387), nrow = 100, ncol = 3, byrow = TRUE)
key.scale5 <- scalevalue(hn_model$ddf$ds$aux$ddfobj$scale$parameters, mat5)
...
y.val4 <- keyfct.hn(distances, key.scale4)
y.val5 <- keyfct.hn(distances, key.scale5)
...
ggplot(hist_df_hr) + 
  ...
  geom_line(data = df_hazard %>% filter(!grepl("gemiddelde", df$Legende)), 
            aes(x = dist, y = y_val, colour = Legende), size = 1) +
...
ggplot(hist_df_hn) + 
  geom_bar(aes(x = mids, y = counts), stat = "identity",
           width = trunc / n_breaks, fill = "white", color = "black") + 
  geom_line(data = df_halfnorm %>% filter(!grepl("gemiddelde", df$Legende)), 
            aes(x = dist, y = y_val, colour = Legende), size = 1) +
