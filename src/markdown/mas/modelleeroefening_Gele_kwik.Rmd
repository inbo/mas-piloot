...
Een eerste belangrijke stap in data preparatie is het herberekenen van de 
...
# Bereken centroide per cluster 
dist_df_centroid <- df_territoria_lang %>%
  ...
  ungroup() %>%
  arrange(cluster) %>%
  select(plotnaam, aantal, regio, sbp, openheid, Region.Label, cluster) %>%
  distinct() %>%
  rename(geometry = geom)

# Bereken afstanden van centroide tot telpunt
dist_df <- inner_join(dist_df_centroid %>% as.data.frame(), 
  ...
  # Verander namen van kolommen
  rename("size" = aantal) %>%
  distinct()
```


!! probleem overlappende telcirkels!! controleer dit... effect op distance sampling?? sampled oppervlak kleiner maaktuit voor densities
...
mapview(design_langterm %>% left_join(telpunten_sf, by = "plotnaam") %>% st_as_sf() %>% st_buffer(300) %>% mutate(jaar = as.factor(jaar)), 
        zcol = "jaar", alpha.region = 0.1) +
mapview(df_territoria_lang %>%
          filter(cluster %in% clusters_dubble) %>%
          st_as_sf(sf_column_name = "geom"), zcol = "cluster")

df_territoria_lang %>%
          filter(cluster %in% clusters_dubble) %>%
          st_as_sf(sf_column_name = "geom") %>% View
```
