...
gele_kwik_presences <- mas_clean %>%
  ...
  inner_join(design_langterm, by = c("plotnaam", "jaar")) # Moeren en Leemstreek
...
In totaal hadden we `r nrow(result[[1]]$observations)` observaties (waarneming van een bepaald individu in een plot in een bepaalde telperiode in een bepaald jaar). Nu hebben we `r nrow(result[[1]]$cluster)` clusters (= broedparen).
