...
Samenvattende tabel aantal broedparen per stratum:
...
bugfix: waarom zijn originele distances niet gelijk aan herberekende?
kijk naar clusters die bestaan uit 1 individu 
...
original_dist <- sp::merge(gele_kwik_2022_territoria, df_territoria2, 
  ...
  mutate(n = n()) %>%
  filter(n == 1) %>%
  select(oid, plotnaam, cluster, distance2plot) %>%
...
new_dist <- df_dist_terr %>%
  ...
  select(Sample.Label, cluster, distance) %>%
...
all.equal(full_df$plotnaam, full_df$Sample.Label)
all.equal(full_df$cluster.x, full_df$cluster.y)
...
full_df %>%
  ggplot(aes(x = origi_distance, y = new_distance)) +
    geom_abline(intercept = 0, slope = 1, colour = "red") + 
    geom_point() +
    geom_point(data = intersect, aes(x = origi_distance, y = new_distance), colour = "blue")
```


```{r}
gele_kwik_2022_territoria %>% 
  filter(oid == "30185764") %>% 
  select(plotnaam, distance2plot)
...
ggplot() +
  geom_sf(data = original_dist %>% 
            filter(plotnaam == plot),
          colour = "red", size = 5) +
  geom_sf(data = original_dist %>% 
            filter(plotnaam == plot) %>% 
            st_buffer(original_dist$origi_distance),
          colour = "red", fill = alpha("white", 0)) +
  geom_sf_text(data = original_dist %>% 
            filter(plotnaam == plot),
            label = "original distance", colour = "red", 
            nudge_x = c(0, 0),
            nudge_y = c(50, 50)) +
  
  geom_sf(data = new_dist %>% 
            filter(Sample.Label == plot),
          colour = "blue", size = 3) +
  geom_sf(data = new_dist %>% 
            filter(Sample.Label == plot) %>% 
            st_buffer(new_dist$new_distance),
          colour = "blue", fill = alpha("white", 0)) +
  geom_sf_text(data = new_dist %>% 
            filter(Sample.Label == plot),
