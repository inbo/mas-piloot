...
Deze strategie werkt voor honkvaste soorten! Bovendien is de keuze van de fusieafstand belangrijk. Deze kan gebaseerd worden op basis van de data en/of literatuur. De volgende figuur geeft de afstanden weer tussen verschillende koppels binnen dezelfde telling (dus zelfde plot in zelfde telperiode). Dit kan echter een onderschatting geven aangezien onze tellingen zich tot cirkels met straal 300 m beperken. We weten niet of individuen op afstand > 600 m dezelfde zijn. We zouden de range kunnen vergroten door er van uit te gaan dat een waarnemer op dezelfde dag niet hetzelfde individu 2x heeft gezien. We verwachten dat dit echter in dit geval geen grote verschillen zal meebrengen.
...
gele_kwik_2022 %>%
  ...
  pivot_longer(cols = c("tot_gemiddelde_plots", "tot_max_plots"), names_to = "categorie", 
               values_to = "Estimate") %>%
  ggplot(aes(x = regio, y = Estimate)) + 
    geom_bar(aes(fill = regio), stat = "identity") + 
    scale_y_continuous(sec.axis = sec_axis( ~. / cirkelopp * 1e6, 
