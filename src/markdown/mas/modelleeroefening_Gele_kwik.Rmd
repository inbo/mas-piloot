...
plot(ds_scen1_hr5, pdf = FALSE, showpoints = FALSE, 
...
par(mfrow = c(1, 1))
plot(ds_scen1_hr5, pdf = TRUE, showpoints = FALSE,
...
plot(ds_scen1_hn5, pdf = FALSE, showpoints = FALSE, 
     main = "P(detectie) ~ regio + sbp")
...
par(mfrow = c(1, 1))
plot(ds_scen1_hn5, pdf = TRUE, showpoints = FALSE,
...
# Using a hazard-rate model with two (dichotomous, 1/0) factor variables that 
# only affect the scale (not the shape) of the detection function:

# Shape parameter
key.shape <- scalevalue(model$ddf$ds$aux$ddfobj$shape$parameters, 
                         matrix(1, nrow = 100, 1))
...
mat0[, 2:3] <- 0
key.scale0 <- scalevalue(model$ddf$ds$aux$ddfobj$scale$parameters, mat0)

# Factor = 1: De Moeren - buiten
mat1 <- matrix(1, nrow = 100, ncol = 3)
mat1[, 2] <- 0
key.scale1 <- scalevalue(model$ddf$ds$aux$ddfobj$scale$parameters, mat1)

# Factor = 2: Oostelijke leemstreek - binnen
mat2 <- matrix(1, nrow = 100, ncol = 3)
mat2[, 3] <- 0
key.scale2 <- scalevalue(model$ddf$ds$aux$ddfobj$scale$parameters, mat2)

# Factor = 3: Oostelijke leemstreek - buiten
mat3 <- matrix(1, nrow = 100, ncol = 3)
key.scale3 <- scalevalue(model$ddf$ds$aux$ddfobj$scale$parameters, mat3)

# Gemiddelde
mat4 <- matrix(1 / 2, nrow = 100, ncol = 3)
mat4[, 1] <- 1
key.scale4 <- scalevalue(model$ddf$ds$aux$ddfobj$scale$parameters, mat4)

# Gewogen gemiddelde
mat5 <- matrix(c(1, 308 / 387, 119 / 387), nrow = 100, ncol = 3, byrow = TRUE)
key.scale5 <- scalevalue(model$ddf$ds$aux$ddfobj$scale$parameters, mat5)

# Calculate detection probability values
y.val0 <- keyfct.hz(distances, key.scale0, key.shape)
y.val1 <- keyfct.hz(distances, key.scale1, key.shape)
y.val2 <- keyfct.hz(distances, key.scale2, key.shape)
y.val3 <- keyfct.hz(distances, key.scale3, key.shape)
y.val4 <- keyfct.hz(distances, key.scale4, key.shape)
y.val5 <- keyfct.hz(distances, key.scale5, key.shape)

df <- rbind(data.frame(dist = distances, y.val = y.val0, 
                       Legende = "De Moeren - binnen"),
            data.frame(dist = distances, y.val = y.val1, 
                       Legende = "De Moeren - buiten"),
            data.frame(dist = distances, y.val = y.val2, 
                       Legende = "Oostelijke leemstreek - binnen"),
            data.frame(dist = distances, y.val = y.val3, 
                       Legende = "Oostelijke leemstreek - buiten"),
            data.frame(dist = distances, y.val = y.val4, 
                       Legende = "ongewogen gemiddelde"),
            data.frame(dist = distances, y.val = y.val5, 
...
selected <- rep(TRUE, nrow(model$ddf$ds$aux$ddfobj$xmat))
...
h1 <- hist(dist.data[dist.data$distance <= 300,]$distance, breaks = breaks)
...
ggplot() + 
  geom_rect(aes(xmin = x[1:length(x) - 1], xmax = x[2:length(x)], 
                ymin = rep(0,length(x) - 1), ymax = h1$counts), 
            fill = "white", color = "black") + 
  geom_line(data = df %>% filter(!grepl("gemiddelde", df$Legende)), 
            aes(x = dist, y = y.val, colour = Legende), size = 1) +
  xlab("Afstand (m)") +
  ylab("Detectiekans") +
...
test <- dist.data %>%
  filter(distance <= 300, 
         ...
         sbp == "buiten")
h1 <- hist(test$distance, breaks = seq(0, 300, 300 / 15))
...
ggplot() + 
  geom_rect(aes(xmin = x[1:length(x) - 1], xmax = x[2:length(x)], 
                ymin = rep(0,length(x) - 1), ymax = h1$counts), 
            fill = "white", color = "black") + 
  geom_line(data = data.frame(x = distances, y = y.val3), 
            aes(x = x, y = y), , colour = "salmon") + # Factor level 3
  labs(title = "Oostelijke leemstreek - buiten", x = "Afstand (m)", 
       y = "Detectiekans")
```
