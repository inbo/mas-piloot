...
In een tweede stap maken we de dataset klaar om de detectiecurve te fitten. We hebben een kolom `object` nodig: volgnummer voor elke waarneming; een kolom `distance`: afstand tot waarnemer voor elke waarneming; kolommen met covariaten: `sbp`, `regio` en `openheid`. 

```{r}
detection_df <- dist_df %>%
  st_drop_geometry() %>%
  rename(object = id) %>%
  select(object, distance, regio, sbp, openheid)
```
