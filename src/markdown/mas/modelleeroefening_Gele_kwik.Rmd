...
stan_pois <- stan(path_stan_pois, data = stan_data, iter = niter, 
                  warmup = burnin, hains = nchains, cores = nparallel, 
                  seed = 1235)
...
We fitten een distance sampling model met gecategoriseerde afstandsdata in drie categoriën: 0-100, 100-200, 200-300. We gaan uit van een half-normale detectiefunctie. We berekenen voor elke afstandscategorie $k = 1, ..., n_{cat}$ de waarschijnlijkheid dat een dier aanwezig is en is gedetecteerd:

$$
p_{k} = \int_{w_k}^{w_{k+1}}\frac{2r}{w^2} \exp{\left( -\frac{r^2}{2\sigma^2} \right)}\,dr
$$

$$
p_{k} = \log{(2)} + 2\log{(\sigma)} - 2\log{(w)} + \log{( \exp{(-\frac{r_k^2}{2\exp{(2\log{(\sigma)})}})} - \exp{(-\frac{r_{k+1}^2}{2\exp{(2\log{(\sigma)})}})} )}
$$

Zodat 

$$
\log{(p_{k})} = \log{(2)} + 2\log{(\sigma)} - 2\log{(w)} + \log{( \exp{(-\frac{r_k^2}{2\exp{(2\log{(\sigma)})}})} - \exp{(-\frac{r_{k+1}^2}{2\exp{(2\log{(\sigma)})}})} )}
$$

Met $w$ de maximale afstand. $r_k$ de $k$-de cut-off afstand.
...
Dit model voorspelt gemiddeld gezien in totaal `r sum(summary(stan_dist, pars = c("n"))$summary[, "mean"])` individuen in de totale regio en een gemiddelde detectiekans van `r exp(sum(summary(stan_dist, pars = c("log_p"))$summary[, "mean"]))`. Wanneer we dit vergelijken met het **Distance** model waarbij we dezelfde breakpoints gebruikten, ook een half-normal functie gebruikten en zonder covariaten, dan zien we gelijkaardige estimates:
