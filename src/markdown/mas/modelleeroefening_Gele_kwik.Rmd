...
plot(dens_jaren_hr19, pdf = TRUE, showpoints = FALSE)
plot(dens_jaren_hr19, showpoints = FALSE)
```
...
```{r}
# Vat resultaten Hazard-rate model samen
summary_results_dens_jaren_hr0 <- bind_rows(
  summary_dens_jaren_hr0$dht$individuals$D %>%
    mutate(variable = "density",
           type = "Aantal broedparen Gele kwikstaart per 100 ha"),
  summary_dens_jaren_hr0$dht$individuals$N %>%
    mutate(variable = "abundance",
...
```{r}
summary_results_dens_jaren_hr0 %>%
...
We visualiseren de absolute verschillen.
