...
Lukt niet om te repliceren in met `ggplot()`. Herschaling onduidelijk. Deze link geeft mogelijks een relatief eenvoudige oplossing:  
...
# Vector of distances used to re-create the detection function (from 0 out to truncation distance of 300 m)
distances <- seq(0 ,300, length.out = 100)

# Using a hazard-rate model with two (dichotomous, 1/0) factor variables that only affects the scale
# (not the shape) of the detection function:

# Shape parameter
key.shape <- scale.value(model$ddf$ds$aux$ddfobj$shape$parameters, 
                         matrix(1, nrow = 100, 1))
...
mat0[, 2:3] <- 0
key.scale0 <- scale.value(model$ddf$ds$aux$ddfobj$scale$parameters, mat0)

# Factor = 1: De Moeren - buiten
mat1 <- matrix(1, nrow = 100, ncol = 3)
mat1[, 2] <- 0
key.scale1 <- scale.value(model$ddf$ds$aux$ddfobj$scale$parameters, mat1)

# Factor = 2: Oostelijke leemstreek - binnen
mat2 <- matrix(1, nrow = 100, ncol = 3)
mat2[, 3] <- 0
key.scale2 <- scale.value(model$ddf$ds$aux$ddfobj$scale$parameters, mat2)

# Factor = 3: Oostelijke leemstreek - buiten
mat3 <- matrix(1, nrow = 100, ncol = 3)
key.scale3 <- scale.value(model$ddf$ds$aux$ddfobj$scale$parameters, mat3)
...
# If you inspect the internal functions of the mrds package, you will find how the number of histogram breaks is calculated. In my case, the value is 15
h1 <- hist(dist.data[dist.data$distance <= 300,]$distance, 
           breaks = seq(0, 300, 300 / 15))

# Calculate expected counts for each distance value
expected.counts <- apply(t(as.matrix(c(0,300))), 1, bindata, 
                         r = (0:1000) * 300 / 1001,
                         breaks = seq(0, 300, 300 / 15))
