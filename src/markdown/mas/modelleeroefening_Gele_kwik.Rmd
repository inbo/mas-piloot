...
We lezen de data van 2018-2021 in en voegen variabelen toe m.b.t. openheid landschap en sbp.
...
We willen ook een dataframe met de bezochte plots 2018-2022 met variabelen en overzicht welke plots in welk jaar zijn bezocht.
...
Voor de nieuwe plots (2022) die vervangen zijn door een bestaand punt (2018-2021) moeten de variabelen overgenomen worden van het bestaande punt, omdat er anders mismatch mogelijk is (zie issues_mismatch_strata.Rmd).

```{r omzeilen-probleemgevallen}
# Which plot visited in which year (2018-2021)
plot_per_jaar <- bezoekstippen_avimap_2021 %>%
  st_drop_geometry() %>%
  distinct(plotnaam, jaar)

# Voeg jaren toe aan plots
# plots 2018-2021
design_2018_2021_jaar <- design_2018_2021 %>%
  inner_join(plot_per_jaar, by = "plotnaam")
# plots 2022 die al bestonden in 2018-2021 (overnemen variabelen)
design_2018_2021_jaar_bis <- design_2018_2021 %>%
  filter(plotnaam %in% design2022_raw$plotnaam) %>%
  mutate(jaar = 2022)
# plots 2022 die niew zijn
design2022_jaar_bis <- design2022_raw %>%
  filter(!(plotnaam %in% design_2018_2021_jaar$plotnaam)) %>%
  mutate(jaar = 2022)

# Voeg 2018-2021 en 2022 samen
design_2018_2022 <- bind_rows(
  design_2018_2021_jaar,
  design_2018_2021_jaar_bis,
  design2022_jaar_bis) %>%
  
  # Bilzen valt onder Oostelijke leemstreek
  mutate(regio = ifelse(regio ==  "Bilzen", "Oostelijke leemstreek", regio)) %>%
  distinct() %>%
  arrange(plotnaam, jaar)

# Hebben we nog duplicaten?
is_duplo <- duplicated(data.frame(design_2018_2022$plotnaam, design_2018_2022$jaar))
...
We hebben geen mismatches meer.
