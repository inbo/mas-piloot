...
Voor 2018-2021 moeten we er vanuit gaan dat als er geen waarnemingen zijn voor een bepaald telpunt, het telpunt niet geteld werd.
...
We lezen de covariaten data van 2018-2021 in.
...
telpunten_avimap_2021_amersfoort <- telpunten_avimap_2021 %>%
  st_drop_geometry() %>%
  st_as_sf(coords = c("st_x", "st_y"), crs = 28992)

telpunten_avimap_2021 <- st_transform(
  st_geometry(telpunten_avimap_2021_amersfoort), "EPSG:31370",
  pipeline = chosen_pipeline_definition) %>% 
  st_sf(st_drop_geometry(telpunten_avimap_2021), geometry = .) %>% 
  as_tibble %>% 
  st_as_sf
...
Voeg covariaten toe aan design van 2018-2021 en 2022 en voeg samen in 1 dataframe.
...
design_2018_2021 <- design_2018_2021 %>%
  full_join(plot_per_jaar %>%
              filter(jaar != 2022),
            by = "plotnaam")  %>%
  mutate(regio = ifelse(regio == "Bilzen", "Oostelijke leemstreek", regio)) %>%
  filter(regio %in% c("Oostelijke leemstreek", "De Moeren"))

# problem in 2022 data with position of some VL points
design2022_fixed <- design2022_raw %>%
  filter(grepl("VL", .$plotnaam)) %>%
  # replace geom by those from design_2018_2021
  left_join(design_2018_2021 %>%
              as_tibble() %>%
              filter(grepl("VL", .$plotnaam)) %>%
              select(plotnaam, geometry_correct = geometry) %>%
              distinct()) %>%
  st_set_geometry("geometry_correct") %>%
  select(-geometry) %>%
  rename(geometry = geometry_correct)

# Recalculate covariates
design2022 <- add_openheid_landschap_to_frame(
  path = path_to_openheid_landschap(),
  punten_sf = design2022_fixed,
  gebied = vlaanderen,
  cutlevels = c(1.25, 1.35, 1.51),
  class_labels = c("GL", "HGL", "HOL", "OL")) %>%
  select(-c(openheid, openheid_waarde, sbp, stratum)) %>%
  ...
  mutate(sbp = st_intersects(.,
           ...
           as.logical(),
         sbp = ifelse(sbp, "binnen", "buiten"),
         stratum = paste0(openheid, "\n", sbp, " plan")
  ) %>%
  select(plotnaam, regio, openheid, geometry, sbp, stratum, jaar)

design_2018_2022_full <- bind_rows(
  design_2018_2021,
  design2022) %>%
  distinct() %>%
  arrange(plotnaam)
...
probleemgevallen <- design_2018_2022_full %>%
  group_by(plotnaam) %>%
...
```{r}
design_2018_2022 <- design_2018_2022_full %>%
  st_drop_geometry()

design2022 <- design2022 %>%
  st_drop_geometry()
```

We hebben geen data over deze plots?

```{r}
design_2018_2022 %>% filter(is.na(openheid))
design_2018_2022 %>% filter(is.na(jaar))
```
...
# Fit model
dsmodelhr8_new <- ds(data = gele_kwik_2022_ds_new, key = "hr", 
                 formula = ~ regio + sbp + regio:sbp,
                 adjustment = NULL, truncation = 300, transect = "point", 
                 dht_group = FALSE,
                 convert_units = conversion_factor)
...
# Samenvatting model
summary_hr8_new <- summary(dsmodelhr8_new)

summary_results_hr8_new <- bind_rows(
  summary_hr8_new$dht$clusters$D %>%
    mutate(variable = "density",
           type = "clusters"),
  summary_hr8_new$dht$clusters$N %>%
    mutate(variable = "abundance",
           type = "clusters"),
  summary_hr8_new$dht$individuals$D %>%
    mutate(variable = "density",
           type = "individuals"),
  summary_hr8_new$dht$individuals$N %>%
    mutate(variable = "abundance",
           type = "individuals")
  )
...
# Effort = 4 model
dist_model_hr8_new <- summary_results_hr8_new %>%
  filter(type == "individuals", 
         variable == "density",
         Label != "Total") %>%
...
grid.arrange(dist_model_hr8, dist_model_hr8_new, ncol = 2)
```
...
# Fit model  
dsmodelhr8_presence <- ds(data = gele_kwik_2022_ds_presence, key = "hr", 
                 formula = ~ regio + sbp + regio:sbp,
                 adjustment = NULL, truncation = 300, transect = "point", 
...
# Samenvatting model
summary_results_hr8_presence <- summary(dsmodelhr8_presence)

summary_results_hr8_presence <- bind_rows(
  summary_results_hr8_presence$dht$clusters$D %>%
    mutate(variable = "density",
           type = "clusters"),
  summary_results_hr8_presence$dht$clusters$N %>%
    mutate(variable = "abundance",
           type = "clusters"),
  summary_results_hr8_presence$dht$individuals$D %>%
    mutate(variable = "density",
           type = "individuals"),
  summary_results_hr8_presence$dht$individuals$N %>%
    mutate(variable = "abundance",
           type = "individuals")
  )

# Model vanuit presence data
dist_model_hr8_presence <- summary_results_hr8_presence %>%
...
grid.arrange(dist_model_hr8_presence, dist_model_hr8_new, ncol = 2)
```
...
In de output van **territoria** krijgen we de centroide van de clusters (gemiddelde coordinaten van waarnemingen per cluster). Om niet te veel te moeten joinen berekenen we deze coordinaten opnieuw met de **sf** package. We maken een kolom `Sample.Label` met de het id van de plot, `size` is het aantal broedparen per cluster (overal 1), `regio`, `sbp` en `openheid` zijn covariaten en `Region.Label` is een label voor elke regio (zelfde als `regio`-kolom). We hebben ook een dataframe die we als optie `region_table` meegeven met `Region.Label` en `Area` en een dataframe die we als optie `sample_table` meegeven met `Sample.Label`, `Region.Label` en `Effort`. `Effort` is het aantal keer dat een plot is bezocht (overal 1). Deze laatste tabel bevat alle plotnamen, ook deze waar geen Gele kwikstaart werd gezien.
