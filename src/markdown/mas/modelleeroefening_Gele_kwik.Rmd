...
gele_kwik %>%
  ...
  ggplot(aes(x = aantal, y = n, fill = periode_in_jaar)) +
    ...
    theme(panel.grid.minor.x = element_blank(),
          panel.grid.major.x = element_blank()) + 
    facet_wrap(regio~jaar, scales = "free_y") +
    labs(y = "", x = "Aantal broedparen Gele kwikstaart per plot") +
  theme(legend.position = "bottom")
```
...
Een waarneming is een telling van een aantal individuen op een bepaalde afstand in een bepaalde telcirkel in een bepaalde telperiode in een bepaald jaar.
...
gele_kwik_presences %>%
  ...
  full_join(plots_per_jaar, by = "jaar") %>%
  select(jaar, aantal_plots, aantal_waarnemingen) %>%
  kable()
```
...
- plots per jaar
- waarnemingen per telperiode per jaar
- zie haas verkenning (Hans)
- distances verdeling per jaar en regio
- ...
