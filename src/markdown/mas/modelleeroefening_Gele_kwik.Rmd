...
Er zijn nog steeds veel punten waarvoor de oorspronkelijke en herberekende afstanden niet overeenkomen.

```{r}
# Zoek punten waarbij oorspronkelijke en nieuwe afstanden niet overeenkomen
intersect <- dist_df %>%
  dplyr::filter(distance2plot == floor(distance) | 
...
dist_df %>%
  ggplot() +
    geom_abline(intercept = 0, slope = 1, colour = "red") + 
    geom_point(data = dist_df, aes(x = distance2plot, y = distance), 
               colour = "blue") +
    geom_point(data = intersect, aes(x = distance2plot, y = distance))
```


```{r}
perimeters %>% mutate(Area = as.numeric(st_area(geom)) / 1e6) %>%
