...
We zien duidelijk zero-inflation en een slechte fit door Poisson.
...
gele_kwik_2022 %>%
  ...
  mutate(nmod = n/sum(n),
         pois = dpois(aantal, lambda = mean(aantal)),
         nbinom = dnbinom(aantal, mu = mean(aantal), size = 1)) %>%
  ggplot(aes(x = aantal, y = nmod)) +
    geom_histogram(stat = "identity") +
    theme(panel.grid.minor.x = element_blank(),
          panel.grid.major.x = element_blank()) + 
    geom_line(aes(y = pois, col = "Poisson"), lwd = 1) + 
    geom_line(aes(y = nbinom, col = "Negative\nbinomial"), lwd = 1) +
...
```{r}
model1p <- glm(aantal ~ regio, family = poisson(link = log), 
              data = gele_kwik_2022)
summary(model1p)
```

```{r}
model1zip <- zeroinfl(aantal ~ regio|regio, data = gele_kwik_2022, 
                      link = "logit", dist = "poisson")
summary(model1zip)
```

```{r}
model1nb <- glm.nb(aantal ~ regio, data = gele_kwik_2022)
summary(model1nb)
```

```{r}
model1zinb <- zeroinfl(aantal ~ regio|regio, data = gele_kwik_2022, 
                       link = "logit", dist = "negbin")
summary(model1zinb)
```

```{r}
cbind(AIC(model1p),AIC(model1nb),AIC(model1zip),AIC(model1zinb))
```

```{r}
estobsk <- function(k){
  cbind(
    sum(dpois(k, fitted(model1p))),
    sum(dnbinom(k, mu = fitted(model1nb),size = model1nb$theta)),
    sum(predict(model1zip,type = "prob")[,1 + k]),
    sum(predict(model1zinb,type = "prob")[,1 + k])
    )
}

estimatedobs <- sapply(min(gele_kwik_2022$aantal):max(gele_kwik_2022$aantal), 
                       estobsk)
row.names(estimatedobs) <- c("Poisson", "Negative binomial", "ZIP", "ZINB")
colnames(estimatedobs) <- min(gele_kwik_2022$aantal):max(gele_kwik_2022$aantal)

predictions <- estimatedobs %>%
  as.data.frame() %>%
  mutate(models = c("Poisson", "Negative\nbinomial", "ZIP", "ZINB")) %>%
  pivot_longer(cols = as.character(0:6), names_to = "aantal", values_to = "pred") %>%
...
gele_kwik_2022 %>%
  group_by(aantal) %>%
  summarise(n = n(), .groups = "drop_last") %>%
  full_join(predictions, by = "aantal") %>%
  ggplot(aes(x = aantal, y = n)) +
    geom_histogram(stat = "identity") +
    geom_point(data = predictions, aes(x = as.numeric(aantal), y = pred,
                                       colour = models)) +
    geom_line(data = predictions, aes(x = as.numeric(aantal), y = pred,
                                       colour = models)) +
    labs(y = "", x = "Aantal Gele kwikstaarten") +
  facet_wrap(~models)
```
