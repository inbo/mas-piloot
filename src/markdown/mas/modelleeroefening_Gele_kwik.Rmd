...
```{r}
names(um_model_hn2)
backTransform(um_model_hn2, type = "state")

# De Moeren
backTransform(linearComb(um_model_hn2["det"], c(1, 0)))

# Oostelijke leemstreek
backTransform(linearComb(um_model_hn2["det"], c(1, 1)))
```


```{r}
df <- data.frame(regio = c("De Moeren", "Oostelijke leemstreek"))

Elambda <- predict(um_model_hn2, type = "state", newdata = df,
    appendData = TRUE)
Esigma <- predict(um_model_hn2, type="det", newdata = df,
    appendData = TRUE)
```

```{r}
with(Elambda, {
    x <- barplot(Predicted, names=regio, xlab="Regio",
        ylab="Density (animals / sqkm)", ylim=c(0, 25), cex.names=0.7,
...
with(Esigma, {
    x <- barplot(Predicted, names=regio, xlab="Regio",
        ylab=expression(paste("Half-normal scale parameter (", sigma, ")")), ylim=c(0, 150), cex.names=0.7,
        cex.lab=0.7, cex.axis=0.7)
    arrows(x, Predicted, x, Predicted+SE, code=3, angle=90, length=0.05)
    box()
    })
...
```{r}
# Detection function
plot(function(x) gxhn(x, sigma = 117.28053), 0, 300, xlab = "Distance (m)",
    ylab = "Detection probability", las = 1, col = "blue")
plot(function(x) gxhn(x, sigma = 99.17182), 0, 300, add = TRUE, col = "green")
legend('topright', c("De Moeren", "Oostelijke leemstreek"),
...
```{r}
# Null modellen
um_model_hn1 <- gdistsamp(~1~1, data = umf, keyfun = "halfnorm", output = "density",
         unitsOut = "kmsq")
um_model_hr1 <- gdistsamp(formula = ~1~1, data = umf, keyfun = "hazard", output = "density",
         unitsOut = "kmsq")

# Detectiekans verschilt per regio
um_model_hn2 <- gdistsamp(~regio, ~1,~1, data = umf, keyfun = "halfnorm", output = "density",
         unitsOut = "kmsq")
um_model_hr2 <- distsamp(formula = ~regio~1, data = umf, keyfun = "hazard", output = "density",
         unitsOut = "kmsq")

# Densiteit verschilt per regio
um_model_hn3 <- distsamp(formula = ~1~regio, data = umf, keyfun = "halfnorm", output = "density",
         unitsOut = "kmsq")
um_model_hr3 <- distsamp(formula = ~1~regio, data = umf, keyfun = "hazard", output = "density",
         unitsOut = "kmsq")

# Detectiekans en densiteit verschillen per regio
um_model_hn4 <- distsamp(formula = ~regio~regio, data = umf, keyfun = "halfnorm", output = "density",
         unitsOut = "kmsq")
um_model_hr4 <- distsamp(formula = ~regio~regio, data = umf, keyfun = "hazard", output = "density",
         unitsOut = "kmsq")
```
