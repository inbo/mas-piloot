...
breaks <- seq(0, 300, 300 / 15)
hist_hr <- hist(dist_data[dist_data$distance <= 300,]$distance, breaks = breaks)
...
# Use the ggplot2 package to plot the histogram (as rectangles) and overlay the 
# detection functions
ggplot() + 
  geom_rect(aes(xmin = breaks[1:length(breaks) - 1], 
                xmax = breaks[2:length(breaks)], 
                ymin = rep(0,length(breaks) - 1), 
                ymax = hist_hr$counts), 
