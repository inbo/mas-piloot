...
De CRS moet aangepast worden naar Lambert 72. Dit doen we via een specifieke pipeline, zie: https://inbo.github.io/tutorials/tutorials/spatial_transform_crs_2/
...
Voor 2018-2021 moeten we er _voorlopig_ vanuit gaan dat als er geen waarnemingen zijn voor een bepaald telpunt, het telpunt niet geteld werd.
...
Ja, in `r nrow(pjb)` telpunten werd niets waargenomen. Dus het is aannemelijk dat dit ook in de eerdere jaren soms het geval zal geweest zijn.
...
Opgepast! Er zijn een aantal telpunten die in geen enkel jaar werden bezocht. Deze worden in deze stap verwijderd.
...
Hebben we nog mismatches?

```{r probleemgevallen}
# probleemgevallen met verschillende classificatie?
probleemgevallen <- design_2018_2022_sf %>%
  group_by(plotnaam) %>%
...
We hebben geen mismatches meer. We moeten wel nog enkele aanpassingen doen zodat we de rest van het script probleemloos kunnen runnen:
...
Controle op ontbrekende gegevens:

```{r}
stopifnot(all(complete.cases(design_2018_2022)))
```

```{r overzicht-data, eval=FALSE}
head(mas_31370_pipeline1)
summary(mas_31370_pipeline1)
glimpse(mas_31370_pipeline1)
```

Voor de waarnemingen verwijderen we ook dubbeltellingen. Indien de soort gezien was op meerdere teldagen binnen eenzelfde telperiode, wordt de laatste teldag genomen.
...
```{r sanity-check-on-distances}
mas_controle_afstand <- mas_clean %>%
  select(plotnaam, distance2plot, naam, jaar, waarnemer = waarneme) %>%
  inner_join(design_2018_2022_sf %>% as_tibble(),
...
# Gele kwikstaart 2022{#gele-kwik-2022}
...
gele_kwik_2022_presences <- mas_clean %>% 
  ...
  inner_join(design_2022, by = "plotnaam") # Moeren en Leemstreek

# Totale lijst van bezochte plots in 2022
bezoekenlijst_2022 <- design_2022 %>% 
...
De evidence ratio is groot wat er op duidt dat er gemiddeld gezien een significant hoger aantal broedparen Gele kwikstaart in De Moeren aanwezig zijn. De evidence ratio is de verhouding tussen posterior probability a < b t.o.v. posterior probability a > b. De schatting is `r round(hyp1$hypothesis$Estimate, digits = 3)`. Dit is een verschil in de log-schaal, dus als we dit exponentiëren bekomen we 0.63 keer minder broedparen Gele kwikstaart in de Oostelijke leemstreek dan in De Moeren.
...
# Comparison among models
comparisons2 <- loo_compare(fit_brmpois1, fit_brmpois2, 
...
Het Poisson model is opnieuw het simpelste model met relatief de beste fit. We controleren voor overdispersie (waarom hier onderdispersie?):
...
http://distancesampling.org/downloads/dist_encyc_env.pdf
...
Op basis van deze redenering kiezen we voor model 7 met de Hazard-rate functie en zonder covariaten. Dit model geeft de volgende scale parameter:
...
Op basis van model fit kiezen we opnieuw voor model 7 met de Hazard-rate functie en zonder covariaten. Dit model geeft de volgende scale parameter die gelijkaardig is als voordien:
...
We vergelijken de AIC van de verschillende modellen. Als het verschil tussen AIC’s kleiner is dan 2, kiezen we het eenvoudigste van deze modellen. Nog altijd is het Hazard-rate model zonder variabelen het best. 
...
**Spelen met sampling effort**   
Krijgen we dezelfde resultaten als we geen rekening houden met telperiode en effort = 4 zetten?

```{r}
# Data per regio met effort = 4
gele_kwik_2022_ds_new <- gele_kwik_2022_ds %>%
  mutate(Effort = 4,
         Region.Label = regio)
  
# Fit model
dsmodelhr1_new <- ds(data = gele_kwik_2022_ds_new, key = "hr", 
                 formula = ~ regio + sbp + regio:sbp,
                 adjustment = NULL, truncation = 300, transect = "point", 
                 dht_group = FALSE,
                 convert_units = conversion_factor)
...
# Samenvatting model
summary_hr1_new <- summary(dsmodelhr1_new)

summary_results_hr1_new <- bind_rows(
  summary_hr1_new$dht$clusters$D %>%
    mutate(variable = "density",
           type = "clusters"),
  summary_hr1_new$dht$clusters$N %>%
    mutate(variable = "abundance",
           type = "clusters"),
  summary_hr1_new$dht$individuals$D %>%
    mutate(variable = "density",
           type = "individuals"),
  summary_hr1_new$dht$individuals$N %>%
    mutate(variable = "abundance",
           type = "individuals")
  )
...
dist_model_hr1 <- summary_results %>%
  ...
  ggplot(aes(x = regio, y = Estimate)) + 
    ...
    scale_y_continuous(breaks = seq(0, 11, 1), limits = c(0, 11)) +
    labs(x = "", y = "Aantal broedparen Gele kwikstaart per 100 ha", 
         title = "Per periode en effort = 1") +
    theme(legend.position = "bottom")

# Effort = 4 model
dist_model_hr1_new <- summary_results_hr1_new %>%
  filter(type == "individuals", 
         variable == "density",
         Label != "Total") %>%
  ggplot(aes(x = Label, y = Estimate)) + 
    geom_point(aes(colour = Label), size = 4, 
               position = position_dodge(width = 0.5)) +
    geom_errorbar(aes(ymin = lcl, ymax = ucl, colour = Label), 
                  width = 0.25, position = position_dodge(width = 0.5)) + 
    scale_y_continuous(breaks = seq(0, 11, 1), limits = c(0, 11)) +
    labs(x = "", y = "", 
         title = "Per regio en effort = 4") +
    theme(legend.position = "bottom")


grid.arrange(dist_model_hr1, dist_model_hr1_new, ncol = 2)
```

Ja dit lijkt goed overeen te komen.

**Beschouwing absences**

Moeten we absences toevoegen? Krijgen we zelfde resultaten met enkel presences en als we de opties `region_table` en `sample_table` gebruiken?

```{r}
# Select presence data
gele_kwik_2022_ds_presence <- gele_kwik_2022_ds_new %>%
  filter(!is.na(distance))

# Maak dataframes met regio (stratum) info en plot info
reg_table <- reg_tab %>%
  rename(Region.Label = regio)
samp_table <- design_2022 %>%
  select(-c(stratum, openheid, sbp)) %>%
  rename(Sample.Label = plotnaam, 
         Region.Label = regio) %>%
  mutate(Effort = 4)

# Fit model  
dsmodelhr1_presence <- ds(data = gele_kwik_2022_ds_presence, key = "hr", 
                 formula = ~ regio + sbp + regio:sbp,
                 adjustment = NULL, truncation = 300, transect = "point", 
                 dht_group = FALSE, convert_units = conversion_factor,
                 region_table = reg_table, sample_table = samp_table)

# Samenvatting model
summary_results_hr1_presence <- summary(dsmodelhr1_presence)

summary_results_hr1_presence <- bind_rows(
  summary_results_hr1_presence$dht$clusters$D %>%
    mutate(variable = "density",
           type = "clusters"),
  summary_results_hr1_presence$dht$clusters$N %>%
    mutate(variable = "abundance",
           type = "clusters"),
  summary_results_hr1_presence$dht$individuals$D %>%
    mutate(variable = "density",
           type = "individuals"),
  summary_results_hr1_presence$dht$individuals$N %>%
    mutate(variable = "abundance",
           type = "individuals")
  )

# Model vanuit presence data
dist_model_hr1_presence <- summary_results_hr1_presence %>%
  filter(type == "individuals", variable == "density", Label != "Total") %>%
  ggplot(aes(x = Label, y = Estimate)) + 
    geom_point(aes(colour = Label), size = 4, 
               position = position_dodge(width = 0.5)) +
    geom_errorbar(aes(ymin = lcl, ymax = ucl, colour = Label), 
    ...
    scale_y_continuous(breaks = seq(0, 11, 1), limits = c(0, 11)) +
    labs(x = "", y = "Aantal broedparen Gele kwikstaart per 100 ha", 
         title = "Model vanuit presence data") +
    theme(legend.position = "bottom")

# Model vanuit presence-absence data
dist_model_hr1_new <- summary_results_hr1_new %>%
  filter(type == "individuals", 
         variable == "density",
         Label != "Total") %>%
  ggplot(aes(x = Label, y = Estimate)) + 
    ...
    scale_y_continuous(breaks = seq(0, 11, 1), limits = c(0, 11)) +
    labs(x = "", y = "", 
         title = "Model vanuit presence-absence data") +
    theme(legend.position = "bottom")


grid.arrange(dist_model_hr1_presence, dist_model_hr1_new, ncol = 2)
```

We hebben exact dezelfde resultaten.

**Conclusies**

- Zowel binned als continuous distances kunnen gebruikt worden, maar het lijkt onnodig om het moeilijker te maken door zelf cut points te bepalen voor de bins. Daarom raden we continuous aan
- Het is belangrijk covariaten toe te voegen die de detectiekans kunnen beïnvloeden
- Interpretatie sampling effort is van belang
- We kunnen perfect met presence data werken en apart een tabel van het studiedesign meegeven waarbij rekening wordt gehouden met afwezigheden. Dit zal eenvoudigere code geven en kans op fouten tijdens data preparatie (joinen van dataframes) reduceren
...
Het half-normal model waarbij detectiekans verschilt per soortbeschermingsplan en regio en densiteit verschilt per soortbeschermingsplan en openheid landschap scoort het best (eenvoudigst model met verschil in AIC < 2)
...
Is het nuttig om een interactie toe te voegen?

```{r, cache=TRUE}
um_model_hn19 <- distsamp(formula = ~regio * sbp~sbp * openheid,
                          data = umf, keyfun = "halfnorm", output = "density",
         unitsOut = "kmsq")
um_model_hr19 <- distsamp(formula = ~regio * sbp~sbp * openheid,
                          data = umf, keyfun = "hazard", output = "density",
         unitsOut = "kmsq")

um_model_hn20 <- distsamp(formula = ~regio * sbp~sbp + openheid,
                          data = umf, keyfun = "halfnorm", output = "density",
         unitsOut = "kmsq")
um_model_hr20 <- distsamp(formula = ~regio * sbp~sbp + openheid, 
                          data = umf, keyfun = "hazard", output = "density",
         unitsOut = "kmsq")

um_model_hn21 <- distsamp(formula = ~regio + sbp~sbp * openheid, 
                          data = umf, keyfun = "halfnorm", output = "density",
         unitsOut = "kmsq")
um_model_hr21 <- distsamp(formula = ~regio + sbp~sbp * openheid, 
                          data = umf, keyfun = "hazard", output = "density",
         unitsOut = "kmsq")
```
...
# Densiteit
# Half-open landschap binnen sbp
backTransform(linearComb(um_model_hn21["state"], c(1, 0, 0, 0)))

# Open landschap binnen sbp
backTransform(linearComb(um_model_hn21["state"], c(1, 0, 1, 0)))

# Half-open landschap buiten sbp
backTransform(linearComb(um_model_hn21["state"], c(1, 1, 0, 0)))

# Open landschap buiten sbp
backTransform(linearComb(um_model_hn21["state"], c(1, 1, 1, 1)))

# Detectiekans
# Binnen soortbeschermingsplan De Moeren
backTransform(linearComb(um_model_hn21["det"], c(1, 0, 0)))

# Binnen soortbeschermingsplan Oostelijke leemstreek
backTransform(linearComb(um_model_hn21["det"], c(1, 1, 0)))

# Buiten soortbeschermingsplan De Moeren
backTransform(linearComb(um_model_hn21["det"], c(1, 0, 1)))

# Buiten soortbeschermingsplan Oostelijke leemstreek
backTransform(linearComb(um_model_hn21["det"], c(1, 1, 1)))

# Predicties
new_df_dens <- expand.grid(sbp = c("buiten", "binnen"),
                           openheid = unique(covs_unmarked$openheid),
                           regio = unique(covs_unmarked$regio))
new_df_detect <- expand.grid(sbp = c("buiten", "binnen"),
                           regio = unique(covs_unmarked$regio))
...
We kunnen visualiseren hoe de detectiefunctie verschilt binnen en buiten soortbeschermingsplan per regio:

```{r}
# Detection function
plot(function(x) gxhn(x, sigma = 95.48460), 0, 300, xlab = "Afstand (m)",
    ylab = "Detection probability", las = 1, col = "blue")
plot(function(x) gxhn(x, sigma = 108.52155), 0, 300, add = TRUE, col = "green")
plot(function(x) gxhn(x, sigma = 92.44877), 0, 300, add = TRUE, col = "brown")
plot(function(x) gxhn(x, sigma = 105.07122), 0, 300, add = TRUE, col = "orange")
legend('topright', c("Buiten sbp - De Moeren", "Binnen sbp - De Moeren",
                     "Buiten sbp - Oostelijke leemstreek", 
                     "Binnen sbp - Oostelijke leemstreek"),
    col = c("blue", "green", "brown", "orange"), lty = 1)
```
...
```{r}
Elambda %>%
 mutate(categorie = paste(sbp, openheid, sep = " - ")) %>%
 ggplot(aes(x = categorie, y = Predicted)) + 
...
```{r}
design_2022 %>% count(regio, openheid) %>%
...
um_model_GDS3 <- gdistsamp(~sbp * openheid, ~1, ~regio + sbp, data = umf2, 
                           keyfun = "halfnorm", output = "density", 
                           unitsOut = "kmsq", mixture = "P")
um_model_GDS4 <- gdistsamp(~sbp * openheid, ~1, ~regio + sbp, data = umf2, 
                           keyfun = "halfnorm", output = "density", 
...
# Densiteit
# Half-open landschap binnen sbp
backTransform(linearComb(um_model_GDS4["lambda"], c(1, 0, 0, 0)))

# Open landschap binnen sbp
backTransform(linearComb(um_model_GDS4["lambda"], c(1, 0, 1, 0)))

# Half-open landschap buiten sbp
backTransform(linearComb(um_model_GDS4["lambda"], c(1, 1, 0, 0)))

# Open landschap buiten sbp
backTransform(linearComb(um_model_GDS4["lambda"], c(1, 1, 1, 1)))

# Detectiekans
# Binnen soortbeschermingsplan De Moeren
backTransform(linearComb(um_model_GDS4["det"], c(1, 0, 0)))

# Binnen soortbeschermingsplan Oostelijke leemstreek
backTransform(linearComb(um_model_GDS4["det"], c(1, 1, 0)))

# Buiten soortbeschermingsplan De Moeren
backTransform(linearComb(um_model_GDS4["det"], c(1, 0, 1)))

# Buiten soortbeschermingsplan Oostelijke leemstreek
backTransform(linearComb(um_model_GDS4["det"], c(1, 1, 1)))
...
```{r}
# Detection function
plot(function(x) gxhn(x, sigma = 99.16160), 0, 300, xlab = "Afstand (m)",
    ylab = "Detection probability", las = 1, col = "blue")
plot(function(x) gxhn(x, sigma = 112.61232), 0, 300, add = TRUE, col = "green")
plot(function(x) gxhn(x, sigma = 91.62027), 0, 300, add = TRUE, col = "brown")
plot(function(x) gxhn(x, sigma = 104.04805), 0, 300, add = TRUE, col = "orange")
legend('topright', c("Buiten sbp - De Moeren", "Binnen sbp - De Moeren",
                     "Buiten sbp - Oostelijke leemstreek", 
                     "Binnen sbp - Oostelijke leemstreek"),
    col = c("blue", "green", "brown", "orange"), lty = 1)
...
brmpois_openheid1 <- brm(bf(aantal ~ regio + periode_in_jaar + sbp * openheid + 
                              (1 | plotnaam)), 
                        data = gele_kwik_2022, family = poisson(), 
                        chains = nchains, warmup = burnin, iter = niter, 
                        cores = nparallel,
...
gele_kwik_2022_ds %>% mutate(dist_bin = case_when(
  ...
  as_tibble() %>%
  full_join(design_2022, by = c("Sample.Label" = "plotnaam")) %>%
  group_by(regio ) %>%
...
plot_terr <- gele_kwik_2022 %>%
  ...
  # alle plots (som)
  inner_join(design_2022, by = "plotnaam") %>%
...
grid.arrange(GLMM_model, dist_model_hr1, plot_terr, 
...
grid.arrange(GLMM_model, dist_model_hr1, plot_terr2, 
             layout_matrix = rbind(c(1, 2), c(3)))
```
...
In de output van **territoria** krijgen we de centroide van de clusters (gemiddelde coordinaten van waarnemingen per cluster). Om niet te veel te moeten joinen berekenen we deze coordinaten opnieuw met de **sf** package. We maken een kolom `Sample.Label` met de het id van de plot, `size` is het aantal broedparen per cluster (overal 1), `regio`, `sbp` en `openheid` zijn covariaten en `Region.Label` is een label voor elke regio (zelfde als `regio`-kolom). We hebben ook een dataframe die we als optie `region_table` meegeven met `Region.Label` en `Area` en een dataframe die we als optie `sample_table` meegeven met `Sample.Label`, `Region.Label` en `Effort`. `Effort` is het aantal keer dat een plot is bezocht (overal 1). Deze laatste tabel bevat alle plotnamen, ook deze waar geen Gele kwikstaart werd gezien. We gebruiken enkel afstanden <= 300 m
...
# Voeg regio label toe en filter op distance
...
ds_scen1_unif1 <- ds(data = df_dist_terr, key = "unif", truncation = 300, 
                     transect = "point", dht_group = FALSE,
                     convert_units = conversion_factor,
                     region_table = reg_table, sample_table = samp_table2)
```
...
Het Hazard-rate model waarbij de detectiekans afhankelijk is van soortbeschermingsplan is het best.  
  
Als oefening gaan we verder met de modellen waarbij de detectiekans afhankelijk is van regio en soortbeschermingsplan.
...
```{r}
breaks <- seq(0, 300, 300 / 15)

# Hazard-rate in functie van regio en sbp
par(mfrow = c(1, 2))
gof_ds(ds_scen1_hr5, main = "ds_scen1_hr5")

plot(ds_scen1_hr5, pdf = FALSE, showpoints = FALSE, breaks = breaks,
     main = "P(detectie) ~ regio + sbp")
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "blue",
  data = data.frame(regio = "De Moeren", sbp = "binnen"))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "darkgreen", 
  data = data.frame(regio = "De Moeren", sbp = "buiten"))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "brown", 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "binnen"))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "salmon", 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "buiten"))

par(mfrow = c(1, 1))
plot(ds_scen1_hr5, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     main = paste0("ds_scen1_hr5", " - ", "AIC: ",
                   as.character(round(summary(ds_scen1_hr5)$ds$aic, digits = 2))
                   )
     )
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "blue",
  data = data.frame(regio = "De Moeren", sbp = "binnen"), pdf = TRUE)
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "darkgreen", 
  data = data.frame(regio = "De Moeren", sbp = "buiten"), pdf = TRUE)
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "brown", pdf = TRUE, 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "binnen"))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "salmon", pdf = TRUE, 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "buiten"))
legend("topright", legend = c("De Moeren - binnen", "De Moeren - buiten", 
       "Oostelijke leemstreek - binnen", "Oostelijke leemstreek - buiten"),
       lwd = 3, lty = 1, col = c("blue", "darkgreen", "brown", "salmon"))

par(mfrow = c(2, 2))
plot(ds_scen1_hr5, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     subset = regio == "De Moeren" & sbp == "binnen",
     main = "De Moeren - binnen", pl.col = alpha("blue", 0.5))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "blue",
  data = data.frame(regio = "De Moeren", sbp = "binnen"), pdf = TRUE)


plot(ds_scen1_hr5, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     subset = regio == "De Moeren" & sbp == "buiten",
     main = "De Moeren - buiten", pl.col = alpha("darkgreen", 0.5))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "darkgreen",
  data = data.frame(regio = "De Moeren", sbp = "buiten"), pdf = TRUE)


plot(ds_scen1_hr5, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     subset = regio == "Oostelijke leemstreek" & sbp == "binnen",
     main = "Oostelijke leemstreek - binnen", pl.col = alpha("brown", 0.5))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "brown",
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "binnen"), 
  pdf = TRUE)

plot(ds_scen1_hr5, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     subset = regio == "Oostelijke leemstreek" & sbp == "buiten",
     main = "Oostelijke leemstreek - buiten", pl.col = alpha("salmon", 0.5))
add.df.covar.line(ds_scen1_hr5, lwd = 3, lty = 1, col = "salmon",
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "buiten"), 
  pdf = TRUE)
...
plot(ds_scen1_hn5, pdf = FALSE, showpoints = FALSE, breaks = breaks, 
     main = "P(detectie) ~ regio + sbp")
add.df.covar.line(ds_scen1_hn5, lwd = 3, lty = 1, col = "blue",
  data = data.frame(regio = "De Moeren", sbp = "binnen"))
add.df.covar.line(ds_scen1_hn5, lwd = 3, lty = 1, col = "darkgreen", 
  data = data.frame(regio = "De Moeren", sbp = "buiten"))
add.df.covar.line(ds_scen1_hn5, lwd = 3, lty = 1, col = "brown", 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "binnen"))
add.df.covar.line(ds_scen1_hn5, lwd = 3, lty = 1, col = "salmon", 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "buiten"))

par(mfrow = c(1, 1))
plot(ds_scen1_hn5, pdf = TRUE, showpoints = FALSE, breaks = breaks,
...
add.df.covar.line(ds_scen1_hn5, lwd = 3, lty = 1, col = "darkgreen", 
  data = data.frame(regio = "De Moeren", sbp = "buiten"), pdf = TRUE)
add.df.covar.line(ds_scen1_hn5, lwd = 3, lty = 1, col = "brown", pdf = TRUE, 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "binnen"))
add.df.covar.line(ds_scen1_hn5, lwd = 3, lty = 1, col = "salmon", pdf = TRUE, 
  data = data.frame(regio = "Oostelijke leemstreek", sbp = "buiten"))
legend("topright", legend = c("De Moeren - binnen", "De Moeren - buiten", 
       "Oostelijke leemstreek - binnen", "Oostelijke leemstreek - buiten"),
       lwd = 3, lty = 1, col = c("blue", "darkgreen", "brown", "salmon"))
...
Kunnen we deze plots repliceren in `ggplot()`? Herschaling onduidelijk. Combinatie van volgende links geeft de oplossing:  
https://groups.google.com/g/distance-sampling/c/Xf9KnmPdXuo/m/7PhjBe3YAAAJ  
https://rdrr.io/cran/mrds/src/R/plot.ds.R  
  
We hebben een aantal functies nodig van het **mrds** package:
...
We berekenen detectiekansen:

```{r}
dist_data <- df_dist_terr
hr_model <- ds_scen1_hr5
hn_model <- ds_scen1_hn5

# Vector of distances used to re-create the detection function (from 0 out to 
# truncation distance of 300 m)
distances <- seq(0, 300, length.out = 100)

# The models have two dichotomous factor variables that only affect the scale 
# (not the shape) of the detection function:

# Shape parameter
key_shape_hr <- scalevalue(hr_model$ddf$ds$aux$ddfobj$shape$parameters, 
                           matrix(1, nrow = 100, 1))

# Scale parameter - Hazard-rate
# Factor = 0: De Moeren - binnen
mat0 <- matrix(1, nrow = 100, ncol = 3)
mat0[, 2:3] <- 0
key.scale0 <- scalevalue(hr_model$ddf$ds$aux$ddfobj$scale$parameters, mat0)

# Factor = 1: De Moeren - buiten
mat1 <- matrix(1, nrow = 100, ncol = 3)
mat1[, 2] <- 0
key.scale1 <- scalevalue(hr_model$ddf$ds$aux$ddfobj$scale$parameters, mat1)

# Factor = 2: Oostelijke leemstreek - binnen
mat2 <- matrix(1, nrow = 100, ncol = 3)
mat2[, 3] <- 0
key.scale2 <- scalevalue(hr_model$ddf$ds$aux$ddfobj$scale$parameters, mat2)

# Factor = 3: Oostelijke leemstreek - buiten
mat3 <- matrix(1, nrow = 100, ncol = 3)
key.scale3 <- scalevalue(hr_model$ddf$ds$aux$ddfobj$scale$parameters, mat3)

# Calculate detection probability values
y.val0 <- keyfct.hz(distances, key.scale0, key_shape_hr)
y.val1 <- keyfct.hz(distances, key.scale1, key_shape_hr)
y.val2 <- keyfct.hz(distances, key.scale2, key_shape_hr)
y.val3 <- keyfct.hz(distances, key.scale3, key_shape_hr)

df_hazard <- rbind(data.frame(dist = distances, y_val = y.val0, 
                   Legende = "De Moeren - binnen"),
               data.frame(dist = distances, y_val = y.val1, 
                   Legende = "De Moeren - buiten"),
               data.frame(dist = distances, y_val = y.val2, 
                   Legende = "Oostelijke leemstreek - binnen"),
               data.frame(dist = distances, y_val = y.val3, 
                   Legende = "Oostelijke leemstreek - buiten"))

# Scale parameter - Half-normal
# Factor = 0: De Moeren - binnen
mat0 <- matrix(1, nrow = 100, ncol = 3)
mat0[, 2:3] <- 0
key.scale0 <- scalevalue(hn_model$ddf$ds$aux$ddfobj$scale$parameters, mat0)

# Factor = 1: De Moeren - buiten
mat1 <- matrix(1, nrow = 100, ncol = 3)
mat1[, 2] <- 0
key.scale1 <- scalevalue(hn_model$ddf$ds$aux$ddfobj$scale$parameters, mat1)

# Factor = 2: Oostelijke leemstreek - binnen
mat2 <- matrix(1, nrow = 100, ncol = 3)
mat2[, 3] <- 0
key.scale2 <- scalevalue(hn_model$ddf$ds$aux$ddfobj$scale$parameters, mat2)

# Factor = 3: Oostelijke leemstreek - buiten
mat3 <- matrix(1, nrow = 100, ncol = 3)
key.scale3 <- scalevalue(hn_model$ddf$ds$aux$ddfobj$scale$parameters, mat3)

# Calculate detection probability values
y.val0 <- keyfct.hn(distances, key.scale0)
y.val1 <- keyfct.hn(distances, key.scale1)
y.val2 <- keyfct.hn(distances, key.scale2)
y.val3 <- keyfct.hn(distances, key.scale3)

df_halfnorm <- rbind(data.frame(dist = distances, y_val = y.val0, 
                     Legende = "De Moeren - binnen"),
                 data.frame(dist = distances, y_val = y.val1, 
                     Legende = "De Moeren - buiten"),
                 data.frame(dist = distances, y_val = y.val2, 
...
Nu kunnen we beginnen plotten:

```{r}
# Re-create the histogram: Hazard-rate

# Detection probability for each fitted value & Nhat estimate
selected <- rep(TRUE, nrow(hr_model$ddf$ds$aux$ddfobj$xmat))
if (length(hr_model$ddf$fitted) == 1) {
  pdot <- rep(hr_model$ddf$fitted, sum(as.numeric(selected)))
} else {
  pdot <- hr_model$ddf$fitted[selected]
  Nhat <- sum(1 / pdot)
}

# Create a dummy histogram
# dist_data is the data.frame object of the distance data
# Right-truncating here at 300 m
# If you inspect the internal functions of the mrds package, you will find how 
# the number of histogram breaks is calculated. In our case, the value is 15
n_breaks <- 15
trunc <- 300
breaks <- seq(0, trunc, trunc / n_breaks)
hist_hr <- hist(dist_data[dist_data$distance <= trunc,]$distance, 
...
# Re-scale the counts
hist_hr$counts <- hist_hr$counts / expected.counts

# Calculate the fitted average detection probability
ddfobj <- hr_model$ddf$ds$aux$ddfobj
...
df_gemiddelde_hr <- data.frame(dist = distances, lineval = linevalues)

# Use the ggplot2 package to plot the histogram as a barplot and overlay the 
# detection functions
hist_df_hr <- data.frame(mids = hist_hr$mids, counts = hist_hr$counts)

ggplot(hist_df_hr) + 
  geom_bar(aes(x = mids, y = counts), stat = "identity",
           width = trunc / n_breaks, fill = "white", color = "black") + 
  geom_line(data = df_hazard, aes(x = dist, y = y_val, colour = Legende), 
            size = 1) +
  geom_line(data = df_gemiddelde_hr, aes(x = dist, y = lineval), size = 1, 
            linetype = "dashed") +
  labs(title = "Hazard-rate model", x = "Afstand (m)", y = "Detectiekans") +
...
```{r}
# Re-create the histogram: Half-normal

# Detection probability for each fitted value & Nhat estimate
selected <- rep(TRUE, nrow(hn_model$ddf$ds$aux$ddfobj$xmat))
if (length(hn_model$ddf$fitted) == 1) {
  pdot <- rep(hn_model$ddf$fitted, sum(as.numeric(selected)))
} else {
  pdot <- hn_model$ddf$fitted[selected]
  Nhat <- sum(1 / pdot)
}
...
trunc <- 300
breaks <- seq(0, trunc, trunc / n_breaks)
hist_hn <- hist(dist_data[dist_data$distance <= trunc,]$distance, 
                breaks = breaks, plot = FALSE)
...
# Re-scale the counts
hist_hn$counts <- hist_hn$counts / expected.counts

# Calculate the fitted average detection probability
ddfobj <- hn_model$ddf$ds$aux$ddfobj
finebr <- seq(0, trunc, length.out = 101)
xgrid <- NULL
linevalues <- NULL
newdat <- ddfobj$xmat
...
df_gemiddelde_hn <- data.frame(dist = distances, lineval = linevalues)

# Use the ggplot2 package to plot the histogram as a barplot and overlay the 
# detection functions
hist_df_hn <- data.frame(mids = hist_hn$mids, counts = hist_hn$counts)

ggplot(hist_df_hn) + 
  geom_bar(aes(x = mids, y = counts), stat = "identity",
           width = trunc / n_breaks, fill = "white", color = "black") + 
  geom_line(data = df_halfnorm, aes(x = dist, y = y_val, colour = Legende), 
            size = 1) +
  geom_line(data = df_gemiddelde_hn, aes(x = dist, y = lineval), size = 1, 
            linetype = "dashed") +
  labs(title = "Half-normal model", x = "Afstand (m)", y = "Detectiekans") +
  theme(legend.position = c(1, 1),
        legend.justification = c(1, 1),
        legend.background = element_rect(fill = "white", color = "black"))
```
...
- Hoe kunnen we hypotheses testen? Effect class package Thierry?
- Oppervalkte $A$ moet enkel HOL en OL zijn om abundanties te schatten. Nu zit daar ook nog stedelijk gebied in enzo.


# Gele kwikstaart 2018-2022{#gele-kwik}

Nu kunnen we overgaan naar een dataset van Gele kwikstaart over de jaren heen (2018-2022). We selecteren de data voor Gele kwikstaart van 2018-2022 met broedcode > 0 en afstanden <= 300 m. Opmerking: wat met *Gele kwikstaart (spec)*?  
  
Er is een discrepantie in de variabelen van de plots tussen 2018-2021 en 2022 (zie \@ref(inlezen-data)). Daardoor zijn er in de volgende data geen plots die zowel in 2018-2021 en 2022 zijn bezocht. Bovendien zullen de resultaten voor 2022 nu dan ook verschillen van deze in \@ref(gele-kwik-2022).

```{r}
# design 2018-2022 Oostelijke leemstreek en De Moeren
design_langterm <- design_2018_2022 %>%
  filter(regio %in% c("De Moeren", "Oostelijke leemstreek"))

# Selecteer tellingen van Gele kwikstaart in 2018-2022 in de twee regio's
# met broedcode > 0, op minder dan 300 m en binnen telrondes
gele_kwik_presences <- mas_clean %>%
  filter(naam == "Gele Kwikstaart", !is.na(periode_in_jaar), wrntype > 0L,
         distance2plot <= 300) %>%  
  inner_join(design_langterm, by = c("plotnaam", "jaar")) %>%
  mutate(Region.Label = paste(regio, jaar, sep = " - "))

# Totale lijst van bezochte plots in 2018-2022
bezoekenlijst_2018_2022 <- design_langterm %>% 
  expand_grid(distinct(mas_clean, periode_in_jaar)) %>% # voeg telperiodes toe
  filter(!is.na(periode_in_jaar)) %>%
  arrange(plotnaam, jaar, periode_in_jaar)

# Voeg afwezigheden toe door te mergen met alle bezoeken
gele_kwik <- gele_kwik_presences %>%
  st_drop_geometry() %>%
...
## Data exploratie{#exploratie-lang}

```{r}
gele_kwik_presences %>%
  st_drop_geometry() %>%
  distinct(plotid, regio, periode_in_jaar, doy, jaar) %>%
  ggplot(aes(x = periode_in_jaar, y = doy, colour  = factor(jaar))) + 
  geom_point() +
  geom_line(aes(group = interaction(plotid, factor(jaar)))) +
  labs(y = "Dag in het jaar", x = "Telperiode") +
  facet_wrap(~regio)
```

**Aantallen**

Enkele samenvattende statistieken van de aantallen broedparen Gele kwikstaart:
...
Een waarneming is een telling van een aantal individuen op een bepaalde afstand in een bepaalde telcirkel in een bepaalde telperiode in een bepaald jaar. Het is vooral opvallend dat in 2022 veel minder waarnemingen waren terwijl er meer plots waren bezocht. 

```{r}
plots_per_jaar <- gele_kwik %>%
  group_by(regio, jaar) %>%
...
gele_kwik_presences %>%
  st_drop_geometry() %>%
  group_by(regio, jaar) %>%
  summarise(aantal_waarnemingen = n(), .groups = "drop") %>%
  full_join(plots_per_jaar, by = c("regio", "jaar")) %>%
  select(regio, jaar, aantal_plots, aantal_waarnemingen) %>%
  kable()
```
...
gele_kwik_presences %>%
  ...
  select(regio, jaar, aantal_plots, aantal_waarnemingen) %>%
  ggplot() +
    geom_bar(aes(x = jaar, y = aantal_plots, fill = aantal_waarnemingen), stat = "identity") +
    scale_fill_gradientn(colours = rainbow(2)) +
    facet_grid(~regio)
```

In telperiode 1 worden opvallend minder Gele kwikstaarten gezien in beide regio's. 

```{r}
gele_kwik_presences %>%
  ggplot() + 
  geom_bar(aes(x = periode_in_jaar, fill = factor(jaar))) +
  labs(y = "Aantal waarnemingen van Gele kwikstaart", x = "Telperiode") +
  facet_wrap(~regio) +
  theme(legend.position = "bottom")
```
...
## Afbakenen territoria

Het aantal koppels wordt bepaald o.b.v.:

1. Uitsluitende waarnemingen  
Waarnemingen tijdens éénzelfde telling worden met volledige zekerheid als verschillende dieren beschouwd. Wij beschouwen vier telrondes per jaar.

2. Clustering op basis van fusieafstand  
Waarnemingen gedurende verschillende tellingen binnen hetzelfde jaar, maar binnen een bepaalde geografische afstand van elkaar (= fusieafstand) worden geclusterd. We gaan er dan van uit dat dit hetzelfde dier was.
...
# Selecteer coördinaten van broedkoppels waarvan er > 1 per telling
# (zelfde plot zelfde telperiode) werden gezien
broedkoppel_lang <- gele_kwik_presences %>% 
...
# Bereken paarsgewijze afstanden van broedkoppels per telling 
# (zelfde plot, zelfde telperiode, zelfde jaar)
dst_lang <- c()
for (group in broedkoppel_lang$group_id) {
  df_lang <- broedkoppel_lang %>% filter(group_id == group)
  dst_lang <- c(dst_lang, st_distance(df_lang))
}
```

```{r}
# Visualiseer in histogram (filter 0 m eruit)
plot_dist_terr_lang <- dst_lang %>% as_tibble() %>% 
  filter(value != 0) %>%
  ggplot() +
    geom_histogram(aes(x = value, y = ..density..), fill = "yellow", 
                   colour = "black", binwidth = 5, alpha = 0.5) +
    labs(x = "Kleinste afstand tot een ander koppel in dezelfde telling (m)") +
    geom_vline(xintercept = mean(dst_lang[dst_lang != 0]), colour = "darkgreen", 
               size = 1.2) + 
    geom_vline(xintercept = quantile(dst_lang[dst_lang != 0], 
               probs = c(0.25, 0.5, 0.75)),colour = "darkblue", size = 1.2) +
    geom_density(aes(x = value, y = ..density..), size = 1)
plot_dist_terr_lang

# Extract pieken
hist_piek_lang <- ggplot_build(plot_dist_terr_lang)$data[[1]] %>% 
  filter(y == max(y)) %>% 
  pull(x)
dens_piek_lang <- ggplot_build(plot_dist_terr_lang)$data[[4]] %>% 
  filter(y == max(y)) %>% 
  pull(x)
```

Blauw: Q1-Q3. Groen: gemiddelde.  
De top van de densiteitscurve van de koppelafstanden lag op `r round(dens_piek_lang, digits = 2)` m, de top van de histogram op `r hist_piek_lang` m, de mediaan op `r round(median(dst_lang[dst_lang != 0]), digits = 2)` m en het gemiddelde op `r round(mean(dst_lang[dst_lang != 0]), digits = 2)` m.

Johannes liet ons weten dat dit niet goed overeenkomt met de biologie van de soort. Bij SOVON gebruiken ze een clusterafstand van 50 m, dus hier gaan we verder met deze afstand.

```{r}
fusie_afstand_lang <- 50
```


### Data preparatie

We hebben voor elke waarneming hun `x`- en `y`-coördinaten nodig in een geprojecteerd coördinatensysteem. `survey` is een geheel getal id voor elke telling. Een survey is dus een unieke combinatie van een plot en een telperiode. `status` is een geheel getal dat de broedcode aangeeft. Een hogere waarde veronderstelt meer zekerheid over een broedgeval. `id` is een unieke id voor iedere observatie. Een observatie is een waarneming van één enkel individu op een bepaalde locatie in een bepaalde survey met een bepaalde status. Voor elk jaar doen we een aparte clustering.

```{r}
make_territoria_df <- function(dataframe, j) {
  # Voorbereiden dataframe
  df_territoria_lang <- dataframe %>%
    filter(jaar == j) %>%
    group_by(plotnaam, periode_in_jaar) %>%
    mutate(survey = cur_group_id(),
           status = as.integer(wrntype)) %>%
  ...
  df_territoria_lang <- 
    tibble(
      df_territoria_lang[rep(seq_len(dim(df_territoria_lang)[1]), 
                    df_territoria_lang$aantal), , 
                    drop = FALSE], 
      row.names = NULL
    ) %>%
    mutate(aantal = 1)
  ...
  # Gedupliceerde rijen hebben zelfde oid --> nieuw id maken voor elke waarneming
  df_territoria_lang %>% 
    mutate(id = as.numeric(paste0(row_number(), j)))
}
```

We maken een dataframe voor elk jaar.

```{r}
for (j in unique(gele_kwik_presences$jaar)) {
  assign(paste0("df_territoria_", j), make_territoria_df(gele_kwik_presences, j))
}
```
...
Zodra we een dataframe met de waarnemingen hebben, maken we verbinding met een SQLite-database en importeren de waarnemingen. Deze kent elke waarneming toe aan een eigen cluster. Vervolgens moeten we de afstandsmatrix berekenen. Dit is niet de volledige afstandsmatrix. We laten alle irrelevante afstanden weg, b.v. tussen waarnemingen van dezelfde telling of met een afstand groter dan tweemaal de maximale clusterafstand. Nu kunnen we beginnen met het clusteren. De clustering houdt rekening met alle waarnemingen met een broedcode hoger dan of gelijk aan de ingestelde broedcode. Herhaal de clustering voor elk broedcodeniveau. Merk op dat het overslaan van niveaus impliceert dat we ze combineren met het lagere niveau.

```{r}
territoria_cluster <- function(dataframe, fusie_afstand) {
  # Maak database connectie en importeer waarnemingen
  conn <- connect_db()
  import_observations(observations = dataframe, conn = conn, 
                      max_dist = fusie_afstand)
  
  # Bereken afstandsmatrix
  distance_matrix(conn = conn, max_dist = fusie_afstand)
  
  # Broedcodes
  broedcodes <- sort(unique(dataframe$status))
  
  # Cluster van meest zekere broedcode naar minder zeker
  result <- vector(mode = "list", length = length(broedcodes))
  for (broedcode in rev(broedcodes)) {
    print(paste0("Clusteren broedcode ", broedcode))
    cluster_observation(conn = conn, status = broedcode, 
                        max_dist = fusie_afstand)
    result[[broedcode]] <- get_cluster(conn = conn)
  }
  dbDisconnect(conn = conn)
  
  # Output clusternummers
  result[[1]]$observations$cluster
}
```

```{r}
set.seed(20221202)
df_territoria_2018$cluster <- territoria_cluster(df_territoria_2018, 
                                                 fusie_afstand_lang)
df_territoria_2019$cluster <- territoria_cluster(df_territoria_2019,
                                                 fusie_afstand_lang)
df_territoria_2020$cluster <- territoria_cluster(df_territoria_2020,
                                                 fusie_afstand_lang)
df_territoria_2021$cluster <- territoria_cluster(df_territoria_2021,
                                                 fusie_afstand_lang)
df_territoria_2022$cluster <- territoria_cluster(df_territoria_2022,
                                                 fusie_afstand_lang)

df_territoria_lang <- rbind(df_territoria_2018, df_territoria_2019, 
                            df_territoria_2020, df_territoria_2021, 
                            df_territoria_2022) %>%
  st_as_sf()
```
...
In totaal hadden we `r nrow(df_territoria_lang)` observaties (waarneming van een bepaald individu in een plot in een bepaalde telperiode in een bepaald jaar). Nu hebben we `r length(unique(df_territoria_lang$cluster))` clusters (= broedparen). We vergelijken het aantal territoria met het gemiddelde en maximum over de telperiodes per plot per jaar. Bij deze laatste is ook met absensies rekening gehouden (maakt voor maximum niet uit).

```{r}
# Bereken aantal plots per regio per jaar
plots_per_regiojaar <- design_langterm %>% 
  group_by(regio, jaar) %>% 
  summarise(n_plots = n_distinct(plotnaam), .groups = "drop") %>%
  mutate(Region.Label = paste(regio, jaar, sep = " - "))

# Voeg aantal plots per regio toe en bereken aantal clusters per plots
territoria_per_plot_lang <- df_territoria_lang %>% 
  st_drop_geometry() %>%
  full_join(plots_per_regiojaar, by = "Region.Label") %>%
  group_by(Region.Label, n_plots) %>%
  summarise(broedparen = n_distinct(cluster), .groups = "drop") %>%
  mutate(Estimate = broedparen / n_plots,
         categorie = "territoria")
...
gele_kwik %>%
  ...
  # Voeg territoria data toe
  select(Region.Label, Estimate, categorie) %>%
  rbind(territoria_per_plot_lang[, -c(2, 3)]) %>%
  
  # Splits Region.Label
  separate(Region.Label, into = c("regio", "jaar"), sep = " - ") %>%
...
In tegenstelling tot de eerdere analyses met de functie `ds()` uit de **Distance** package, gaan we hier expliciet distance sampling in 2 delen opsplitsen:

1. Schatting detectiekans
    - Fit detectiecurve op afstanden van waarneming tot telpunt (ruwe, ongeclusterde data)
    - Detectiekans is area under the curve
    - `ddf()` functie **mrds** package
2. Schatting abundanties en densiteiten
    - Extrapoleer geclusterde waarnemingen (territoria) m.b.v. detectiekans bekomen in 1
    - Bereken densiteiten door de gesamplede oppervlakte in beschouwing te nemen
    - `dht()` functie **mrds** package


### Data preparatie

Een eerste belangrijke stap in data preparatie is het herberekenen van de afstanden tussen de waarnemingen en de telpunten.

```{r}
# Lees coördinaten telpunten in 
telpunten_2018_2021 <- read_sf(
  here("data", "mas", 
       "avimap_514_0_MAS_Werkgroep_Grauwe_Kiekendief_Belgi__telpunten_xy.shp")
  ) %>%
...
telpunten_2022 <- read_sf(
  here("data/processed/steekproef_piloot_avimap.geojson")
  ) %>%
  select(plotnaam = definitief_punt)

# Voeg telpunten samen en verwijder duplicaten
telpunten_sf <- rbind(telpunten_2018_2021, telpunten_2022) %>%
  distinct()

# Bereken afstanden van waarneming tot telpunt
dist_df <- inner_join(df_territoria_lang %>% as.data.frame(), 
                      telpunten_sf %>% as.data.frame(), 
                      by = "plotnaam") %>%
  mutate(distance = as.numeric(
    st_distance(
      .data[["geometry.x"]] %>% st_sf(), 
...
Inderdaad, er is een mismatch met de afstanden uit de app en de herberekende afstanden. We zien vooral mismatches in 2022 en de mismatches van vòòr 2022 zijn enkel van plot VL0075 of VL0073.

```{r}
dist_df %>%
  mutate(mismatch = ifelse(distance2plot == floor(distance) |
                           distance2plot == ceiling(distance),
                           "no", "yes")) %>%
  ggplot(aes(x = distance2plot, y = distance)) +
    geom_abline(intercept = 0, slope = 1, colour = "black") + 
    geom_point(aes(colour = mismatch)) +
    labs(x = "Originele afstand (app SOVON)", 
         y = "Herberekende afstand waarneming tot telpunt") +
...
In een tweede stap maken we de dataset klaar om de detectiecurve te fitten. We hebben een kolom `object` nodig: volgnummer voor elke waarneming; een kolom `distance`: afstand tot waarnemer voor elke waarneming; kolommen met covariaten: `regio`, `jaar`, `sbp` en `openheid`. 

```{r}
detection_df <- dist_df %>%
  st_drop_geometry() %>%
  rename(object = id) %>%
  select(object, distance, regio, jaar, sbp, openheid) %>%
  mutate(jaar = as.character(jaar))
```

Ten slotte maken we de dataframes en parameters klaar om de densiteiten te berekenen. `Region_table` met de oppervlaktes per regio-jaar (totale oppervlakte strata), `sample_table` geeft aan welke samples in welke regio's voorkomen en met welke effort (effort is overal 1 bij ons omdat we met territoria al de verschillende periodes hebben beschouwd) en `obs_table` die aangeeft welke waarnemingen in welke plots en strata (regio-jaar) zitten. Ten slotte zet de conversiefactor de afstanden om van meter naar vierkante kilometer.

```{r}
# Bereken oppervlaktes van de regio's
areas <- perimeters %>% mutate(Area = as.numeric(st_area(geom)) / 1e6) %>%
            select(regio = Naam, Area) %>%
            st_drop_geometry()

region_table <- design_langterm %>%
  distinct(regio, jaar) %>%
  full_join(areas, by = "regio") %>%
  mutate(Region.Label = paste(regio, jaar, sep = " - ")) %>%
  select(Region.Label, Area)

sample_table <- design_langterm %>%
...
Voor de `obs_table` wordt het iets complexer omdat we nu enkel met de clusters willen werken en niet met alle waarnemingen. Bovendien overlappen sommige plots binnen de fusieafstand waardoor sommige waarnemingen geclusterd zijn die vanaf verschillende plots zijn geobserveerd. Voor deze clusters moeten we `Sample.Label` zelf kiezen. Het maakt niet uit of een cluster verbonden is met de een of andere plot zolang die dezelfde covariaten hebben (zelfde regio, sbp en openheid). Dit moet dus wel gecontroleerd worden.

```{r}
# Welke clusters behoren tot meerdere plots?
clusters_dubble <- dist_df %>%
  group_by(cluster) %>%
  mutate(n = n_distinct(plotnaam)) %>%
  filter(n > 1) %>%
  distinct(cluster) %>%
  pull()
...
dist_df %>% 
  st_drop_geometry() %>%
  filter(cluster %in% clusters_dubble) %>%
  arrange(cluster, plotnaam) %>%
  select(cluster, plotnaam, regio, sbp, openheid) %>% 
  kable()
```

Dit is ok, dus we geven aan deze clusters de eerste `Sample.Label` mee volgens het alfabet (dus voor cluster 1222021 geven we VL0110 mee en niet VL0118).

```{r}
obs_table <- dist_df %>%
  st_drop_geometry() %>%
  select(object = cluster, Region.Label, Sample.Label = plotnaam) %>%
  
  # Selecteer eerste plot voor clusters die tot verschillende plots behoren
  arrange(object, Sample.Label) %>%
  group_by(object) %>%
  mutate(Sample.Label = first(Sample.Label)) %>%
  distinct() %>%
  as_tibble()
```
...
**1. Bereken detectiekans**  
We fitten half-normal en hazard-rate key functions waarbij we alle combinaties van covariaten beschouwen en alle combinaties van interacties.

```{r}
# We maken formules uit combinaties van alle variabelen
match_variables <- c("regio", "jaar", "sbp", "openheid")
...
We fitten alle detectiefuncties met een for-loop. Niet alle interacties kunnen gefit worden omdat niet elke categorie van openheid en sbp in elke regio en/of jaar aanwezig is (opgevangen door `tryCatch()`). We beschouwen de half-normal en hazard-rate key functions.
...
# Null models
detection_hn0 <- ddf(data = detection_df, method = "ds",
  dsmodel = ~mcds(key = "hn", formula = ~1),
  meta.data = list(width = 300, point = TRUE))

detection_hr0 <- ddf(data = detection_df, method = "ds",
  dsmodel = ~mcds(key = "hr", formula = ~1),
  meta.data = list(width = 300, point = TRUE))

# Sla namen van de gefitte models op
fitted_models <- c("detection_hn0", "detection_hr0")
...
for (i in seq_len(length(forms))) {
  ...
  for (k in keys) {
    # Vang warnings of errors op
    tt <- tryCatch(
      ddf(data = detection_df, method = "ds",
        dsmodel = ~mcds(key = k, formula = f),
        meta.data = list(width = 300, point = TRUE)),
      error = function(e) e, warning = function(w) w)
    
    # Naam voor model
    mod_name <- paste0("detection_", k, i)
    
    # Sla het model enkel op als er geen problemen zijn
    if (is(tt, "warning")) {
      print(paste0("Warning: Model '", mod_name, "' did not converge. ", 
                   "Formula: '", f, "'. Key: '", k, "'."))
    } else if (is(tt, "error")) {
      print(paste0("Error: Model '", mod_name, "' not fitted. ", 
                   "Formula: '", f, "'. Key: '", k, "'."))
    } else {
      det_model <- ddf(data = detection_df, method = "ds",
        dsmodel = ~mcds(key = k, formula = f),
        meta.data = list(width = 300, point = TRUE))
      ...
      # Sla naam van gefitte model op
      fitted_models <- c(fitted_models, mod_name)
    }
  }
}
```

**2. Bereken densiteiten**  
Op basis van de detectiekansen berekend in **1.**, berekenen we nu de densiteiten in een for-loop. Op het einde brengen we **1.** en **2.** samen in een dsmodel zodat we gebruik kunnen maken van de functies in de **Distance** package.

```{r}
final_models <- vector(length = length(fitted_models))

# Loop over gefitte modellen
for (m in fitted_models) {
  dens_schat <- dht(model = get(m), region.table = region_table, 
      sample.table = sample_table, obs.table = obs_table,
      options = list(group = FALSE, convert.units = conversion_factor))
  
  # Maak dsmodel voor later gebruik functies in Distance package
  dsmodel <- list(ddf = get(m), dht = dens_schat)
  class(dsmodel) <- "dsmodel"
  
  # Sla het volledige 2-part model op
  suffix <- sub("^.+_", "", m)
  assign(paste0("dens_jaren_", suffix), dsmodel)
  
  # Sla naam van gefitte model op
  final_models[match(m, fitted_models)] <- paste0("dens_jaren_", suffix)
}
```
...
De namen van de dsmodels zijn opgeslaan in de variabele `final_models`, maar die kunnen we jammergenoeg niet gebruiken in combinatie met de `summarize_ds_models()` functie. We vergelijken de AIC van de verschillende modellen. Als het verschil tussen AIC’s kleiner is dan 2, kiezen we het eenvoudigste van deze modellen. Modellen met vergelijkbare AIC’s hebben vergelijkbare geschatte detectiekansen, dus in de praktijk is er weinig verschil in de keuze tussen deze modellen. Een half-normal model heeft een parameter minder dan een hazard-rate model met dezelfde variabelen.

```{r}
summarize_ds_models(dens_jaren_hn0, dens_jaren_hr0, dens_jaren_hn1, 
                    dens_jaren_hr1, dens_jaren_hn2, dens_jaren_hr2, 
                    dens_jaren_hn3, dens_jaren_hr3, dens_jaren_hn4, 
                    dens_jaren_hr4, dens_jaren_hn5, dens_jaren_hr5, 
                    dens_jaren_hn6, dens_jaren_hr6, dens_jaren_hn7, 
                    dens_jaren_hr7, dens_jaren_hn8, dens_jaren_hr8, 
                    dens_jaren_hn9, dens_jaren_hr9, dens_jaren_hn10, 
                    dens_jaren_hr10, dens_jaren_hn11, dens_jaren_hr11, 
                    dens_jaren_hn12, dens_jaren_hr12, dens_jaren_hn13, 
                    dens_jaren_hr13, dens_jaren_hn14, dens_jaren_hr14, 
                    dens_jaren_hn15, dens_jaren_hr15, dens_jaren_hn16, 
                    dens_jaren_hr16, dens_jaren_hn17, dens_jaren_hr17, 
                    dens_jaren_hn18, dens_jaren_hr18, dens_jaren_hn19, 
                    dens_jaren_hr19, dens_jaren_hn20, dens_jaren_hr20, 
                    dens_jaren_hn21, dens_jaren_hr21, dens_jaren_hr24) %>%
  kable()
```

We selecteren Hazard-rate model met variabelen jaar en sbp in interactie. Omdat we geen combinaties van interacties en variabelen niet in een interactie hebben gefit vergelijken we nog enkele modellen formules:

- `~jaar * sbp + regio`  
- `~jaar * sbp + regio`  
- `~jaar * sbp + openheid`  
- `~jaar * sbp + regio * openheid`  

```{r}
# Detectiekans
detection_hr27 <- ddf(data = detection_df, method = "ds",
        dsmodel = ~mcds(key = "hr", formula = ~jaar * sbp + regio),
        meta.data = list(width = 300, point = TRUE))
detection_hr28 <- ddf(data = detection_df, method = "ds",
        dsmodel = ~mcds(key = "hr", formula = ~jaar * sbp + openheid),
        meta.data = list(width = 300, point = TRUE))
detection_hr29 <- ddf(data = detection_df, method = "ds",
        dsmodel = ~mcds(key = "hr", formula = ~jaar * sbp + regio + openheid),
        meta.data = list(width = 300, point = TRUE))
detection_hr30 <- ddf(data = detection_df, method = "ds",
        dsmodel = ~mcds(key = "hr", formula = ~jaar * sbp + regio * openheid),
        meta.data = list(width = 300, point = TRUE))

# Densiteiten
dens_schat_hr27 <- dht(model = detection_hr27, region.table = region_table, 
    sample.table = sample_table, obs.table = obs_table,
    options = list(group = FALSE, convert.units = conversion_factor))
dens_schat_hr28 <- dht(model = detection_hr28, region.table = region_table, 
    sample.table = sample_table, obs.table = obs_table,
    options = list(group = FALSE, convert.units = conversion_factor))
dens_schat_hr29 <- dht(model = detection_hr29, region.table = region_table, 
    sample.table = sample_table, obs.table = obs_table,
    options = list(group = FALSE, convert.units = conversion_factor))
dens_schat_hr30 <- dht(model = detection_hr30, region.table = region_table, 
    sample.table = sample_table, obs.table = obs_table,
    options = list(group = FALSE, convert.units = conversion_factor))

# Maak dsmodel voor later gebruik functies in Distance package
dens_jaren_hr27 <- list(ddf = detection_hr27, dht = dens_schat_hr27)
class(dens_jaren_hr27) <- "dsmodel"

dens_jaren_hr28 <- list(ddf = detection_hr28, dht = dens_schat_hr28)
class(dens_jaren_hr28) <- "dsmodel"

dens_jaren_hr29 <- list(ddf = detection_hr29, dht = dens_schat_hr29)
class(dens_jaren_hr29) <- "dsmodel"

dens_jaren_hr30 <- list(ddf = detection_hr30, dht = dens_schat_hr30)
class(dens_jaren_hr30) <- "dsmodel"
```

```{r}
summarize_ds_models(dens_jaren_hr19, dens_jaren_hr27, dens_jaren_hr28, 
                    dens_jaren_hr29, dens_jaren_hr30) %>%
  kable()
```

We zien dat model 19 nog alijd het simpelste model is met een verschil in AIC’s kleiner dan 2.
...
par(mfrow = c(1, 2))
gof_ds(dens_jaren_hr19)
plot(dens_jaren_hr19, pdf = TRUE, showpoints = FALSE, breaks = breaks)
par(mfrow = c(1, 1))
plot(dens_jaren_hr19, showpoints = FALSE, breaks = breaks)
```

We zien een goede algemene fit. We kunnen dit ook visualiseren voor elke categorie.

```{r}
colours <- c("blue", "darkgreen", "brown", "salmon", "orange")
par(mfrow = c(1, 2))
for (j in as.character(2018:2022)) {
  col <- colours[match(j, as.character(2018:2022))]
  for (s in c("binnen", "buiten")) {
    test <- list(s)
    plot(dens_jaren_hr19, pdf = TRUE, showpoints = FALSE, breaks = breaks,
      subset = jaar == j & sbp == test,
      main = paste(j, s, sep = " - "), pl.col = alpha(col, 0.5))
    add.df.covar.line(dens_jaren_hr19, lwd = 3, lty = 1, col = col,
      data = data.frame(jaar = j, sbp = s), pdf = TRUE)
  }
}
par(mfrow = c(1, 1))
```
...
We kunnen de detectiecurve plotten voor elke categorie zoals eerder in`ggplot()`. Dit zouden we best eens in een fuctie gieten zodat we dat hier ook eenvoudig kunnen gebruiken. Voorlopig gebruiken we dit niet. We maken enkele samenvattende tabellen.

```{r}
summary_dens_jaren_hr19 <- summary(dens_jaren_hr19)

# Vat resultaten Hazard-rate model samen
summary_results_dens_jaren_hr19 <- bind_rows(
  summary_dens_jaren_hr19$dht$individuals$D %>%
    mutate(variable = "density",
           type = "Aantal broedparen Gele kwikstaart per 100 ha"),
  summary_dens_jaren_hr19$dht$individuals$N %>%
    mutate(variable = "abundance",
           type = "Totaal aantal broedparen Gele kwikstaart")
  ) %>%
  filter(Label != "Total") %>%
...
```{r}
summary_results_dens_jaren_hr19 %>%
...
We visualiseren de absolute verschillen in densiteit over de jaren. De resultaten in 2022 moeten herbekeken worden. De data van dit jaar zijn nog niet goed ingelezen.
...
summary_results_dens_jaren_hr19 %>%
  ...
  ggplot(aes(x = jaar, y = Estimate, colour = regio)) + 
    ...
    labs(x = "", y = "Aantal broedparen per 100 ha", 
         title = "Gele kwikstaart - densiteit") +
    theme(legend.position = "")
```

Wat zijn de detectiekansen per categorie?

```{r}
cbind(dens_jaren_hr19$ddf$data, 
      det_prob = predict(dens_jaren_hr19, se.fit = TRUE)$fitted, 
      det_prob_se = predict(dens_jaren_hr19, se.fit = TRUE)$se) %>%
  select(jaar, sbp, det_prob, det_prob_se) %>%
  distinct() %>%
  arrange(jaar, sbp) %>%
  kable()
```

We kunnen de relatieve toename en afname vergelijken over de jaren met de **effectclass** package.  
https://effectclass.netlify.app/  
  
Dit wordt ook gedaan bij de trends in de Algemene broedvogelatlas.
...
Opgelet! De volgende omzettingen zijn niet correct. De foutenvlaggen moeten groter zijn en berekend o.b.v. verschil tussen het jaar en het referentiejaar (verschil is som varianties dus grotere foutenvlaggen). Nu is puur een opschuiving en herschaling gedaan. De relatieve verschillen kunnen berekend worden door de log van de fractie te nemen en dan wordt dit het verschil van de logs. Als je ervan uitgaat dat de confidence limits lognormaal zijn kan je ook herberekenen tot de variantie en dit aanpassen (som van varianties en terug exponentieren).

```{r}
# Functie om relatieve verschillen te berekenen
RPD <- function(reference, value, limit=NULL) {
  if (is.null(limit)) {
    return(((value - reference) / mean(c(reference, value))) * 100)
  } else {
    return(((limit - reference) / mean(c(reference, value))) * 100)
  }
}
```
...
summary_results_dens_jaren_hr19 %>%
  ...
  arrange(jaar) %>%
  group_by(regio) %>%
  mutate(ref = first(Estimate)) %>%
  ungroup() %>%
  filter(jaar != 2018) %>%
  rowwise() %>%
  mutate(rel_est = RPD(ref, Estimate),
         rel_lcl = RPD(ref, Estimate, lcl),
         rel_ucl = RPD(ref, Estimate, ucl), 
         reference = reference,
         lower_thresh = thresholds[1], 
         upper_thresh = thresholds[2]) %>%
  ggplot(aes(x = jaar, y = rel_est, ymin = rel_lcl, ymax = rel_ucl, colour = regio)) + 
    geom_hline(aes(yintercept = reference), linetype = 2,
               size = 1, colour = alpha("firebrick", 0.8)) +
    geom_hline(aes(yintercept = lower_thresh), linetype = 3,
               size = 1, colour = alpha("firebrick", 0.8)) +
    geom_hline(aes(yintercept = upper_thresh), linetype = 3,
             size = 1, colour = alpha("firebrick", 0.8)) +
    geom_errorbar(width = 0.25) +
    labs(x = "", y = "procentuele wijziging t.o.v. 2018", 
       title = "Gele kwikstaart - Aantal broedparen per 100 ha") +
    scale_y_continuous(sec.axis = sec_axis(~(100 + .) / 100, 
                       name = "relatieve wijziging t.o.v. 2018")) +
    stat_effect(reference = reference, threshold = thresholds, size = 4) +
    scale_effect("classificatie") +
    facet_wrap(~regio) +
    guides(colour = "none")
```

De fout in de symbolen is omdat de package nog in maintenance is.  

We kunnen ook paarsgewijze verschillen tussen de jaren berekenen zoals bij de trends in de Algemene broedvogelatlas.


## Discussie

Er zijn nog een aantal discussiepunten:  
  
**Mismatch afstanden**  
De herberekende afstanden komen niet overeen met de afstanden meegekregen door SOVON. Link met volgende punt?
  
**Probleem met inladen data 2022**  
Zie begin code. Zelfde plots hebben verschillende variabelen (sbp, openheid) 2022 vs 2018-2021?
  
**Visibiliteit**  
  
- visibiliteit verschilt van plot tot plot
- we willen niet extrapoleren naar de volledige plot als een deel niet kan gezien worden (bv omdat er huizen staan of een stuk bos)
- niet mogelijk met conventional distance sampling: assumptie uniforme verdeling van dieren  
  
Stel dat we nu de detectiefunctie laten afhangen van visibiliteit. 
Hoe lager de visibiliteit, hoe lager de detectiekans.
Dat wil zeggen dat we voor een lagere visibiliteit gaan overcompenseren voor gemiste dieren ten opzichte van hogere visibiliteit. Stel dat we een plot hebben waarbij 30% visibiliteit is verloren door een bos/huizen.
De detectiekans zal lager zijn voor deze plot, maar dat wil zeggen dat we hierdoor juist sterker gaan compenseren.
Stel dat we bij de huizen maar 1 vogel zien, maar de detectiekans is ook super laag, zal het "model" daar juist voor compenseren en bv. zeggen dat er in werkelijkheid 10 zitten, terwijl dat waarschijnlijk niet realistisch is. Het probleem is dat we de assumptie van uniforme verdeling schaden want in de 30 % bos/huizen zitten niet evenveel dieren als in de 70% akker. Of je detectiekans nu laag of hoog is, ze zal compenseren zodat de volledige abundantie is geschat over de plot. Dingen als bos, huizen ... zijn belangrijke elementen in het Vlaamse landschap dus ervan uitgaan dat de dieren uniform verdeeld zijn in plots waar er overlap is met dergelijke elementen zou de impact van bos, huizen ... op densiteiten van akkervogels verwaarlozen.  
  
**Clustering**  
De clustermethode van SOVON wordt niet voor MAS gebruikt wel voor gebiedsdekkende karteringen:  
*"Tijdens een BMP-bezoek worden doorgaans niet alle individuen opgemerkt, met andere woorden de trefkans is onvolledig. Maar na 5, 6 of 7 bezoeken en clustering kan een betrouwbare schatting van het aantal aanwezige broedparen worden gemaakt. Tijdens een MAS-bezoek van vijf of tien minuten is de trefkans beduidend lager dan bij een BMPbezoek (zie bijvoorbeeld fig. 27a) en zal een kleiner aandeel van de aanwezige individuen worden opgemerkt. Daardoor heeft het bepalen van territoria met behulp van clustering geen zin. Na 2 of 3 MAS-bezoeken zit je met meerdere van deze onvolledige tellingen, waaruit een schatting van het aantal aanwezige individuen moet worden gemaakt."*  
Zij clusteren o.b.v. 3 criteria die verschillen van soort tot soort:

- datumgrenzen
- aantal keer een soort moet waargenomen worden gegeven een aantal bezoeken
- fusieafstand  
  
Bovendien werkt dit enkel bij honkvaste soorten.  
  
Een ander probleem bij clustering is dat je detectiekans afhangt van het aantal bezoeken als je gaat beginnen clusteren. Bij vier bezoeken wordt je detectiekans: $p_{new} = 1-(1-p_{old})^4$. Dit geeft een groot verschil. Neem bijvoorbeeld $p = 0.3$ dan is dit na 4 bezoeken $p = 1-(1-0.3)^4 = 0.76$. Het is ook onduidelijk hoe de onzekerheid op $p$ dan moet aangepast worden. Als je enkel met waarnemingen werkt, dan verandert je detectiekans niet, maar wel je bezochte oppervalk $a$. Dan zal het enkel je densiteitsschattingen beïnvloeden.


## Conclusie

- Communicatie met SOVON over data en methodiek belangrijk
- Targets moeten gecontroleerd worden, fout in data/code?
- Clusteren van individuen mogelijks geen goed idee. Gebruik van ruwe data met `Effort = 4` zal een onderschatting geven maar is een robuustere methode.
- Voorlopig is het niet belangrijk om hypotheses te testen. We willen enkel een goede schatting van de abundantie met onzekerheid. Als later toch gevraagd wordt, kunnen we eventueel herberkeningen doen en abstracties zodat we van **effectclass** kunnen gebruikmaken.
