...
We hebben voor elke waarneming hun `x`- en `y`-coördinaten nodig in een geprojecteerd coördinatensysteem. `survey` is een geheel getal id voor elke telling. Een survey is dus een unieke combinatie van een plot, een telperiode en een jaar. `status` is een geheel getal dat de broedcode aangeeft. Een hogere waarde veronderstelt meer zekerheid over een broedgeval. `id` is een unieke id voor iedere observatie. Een observatie is een waarneming van één enkel individu op een bepaalde locatie in een bepaalde survey met een bepaalde status.

```{r}
# Voorbereiden dataframe
df_territoria_lang <- gele_kwik_presences %>% 
  group_by(plotnaam, periode_in_jaar, jaar) %>%
  mutate(survey = cur_group_id(),
         status = as.integer(wrntype)) %>%
  ungroup() %>%
...
# Gedupliceerde rijen hebben zelfde oid --> nieuw id maken voor elke waarneming
df_territoria_lang <- df_territoria_lang %>% mutate(id = row_number())
```
...
conn <- connect_db()
import_observations(observations = df_territoria_lang, conn = conn, 
                    max_dist = fusie_afstand_lang)
```

Vervolgens moeten we de afstandsmatrix berekenen. Dit is niet de volledige afstandsmatrix. We laten alle irrelevante afstanden weg, b.v. tussen waarnemingen van dezelfde telling of met een afstand groter dan tweemaal de maximale clusterafstand.

```{r distmatrix-territoria}
distance_matrix(conn = conn, max_dist = fusie_afstand_lang)
```
...
```{r cluster-territoria}
# Broedcodes
broedcodes <- sort(unique(df_territoria_lang$status))
...
# voeg clusternummer toe aan de dataset
df_territoria_lang$cluster <- result[[1]]$observations$cluster
```


### Resultaten

In totaal hadden we `r nrow(result[[1]]$observations)` observaties (waarneming van een bepaald individu in een plot in een bepaalde telperiode in een bepaald jaar). Nu hebben we `r nrow(result[[1]]$cluster)` clusters (= broedparen). We vergelijken het aantal territoria met het gemiddelde en maximum over de telperiodes per plot per jaar. Bij deze laatste is ook met absensies rekening gehouden (maakt voor maximum niet uit). Voor het 
