...
vvi_out <- dm_25mpoly %>%
  group_by(pointid) %>%
  tidyr::nest() %>%
  mutate(cvvi = purrr::map_dbl(data, function(x) {
    vvi_from_sf(
    observer = x,
    spacing = 10,
    cores = 4,
...
Berekening op volledige steekproefkader (Moeren en Leemstreek) zou `r round(nrow(steekproefkader) * 25 * 30 * 1e-3 /3600, 1)` uur duren bij resolutie van 5 m en spacing van 10 m.
