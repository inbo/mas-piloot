...
chosen_pipeline_definition <- pipelines %>% slice(1) %>% pull(definition)
mas_31370_pipeline1 <- 
...
mas_controle_afstand <- mas_31370_pipeline1 %>%
  ...
  mutate(correct_distance = st_distance(.$geometry.stip, .$geometry.telpunt,
            by_element = TRUE),
...
mas_controle_afstand %>%
  ...
  ggplot() +
  geom_point(aes(y = mean_abs_verschil, x = jaar, colour = set),
             alpha = 0.3,
             position = position_dodge(0.5)) +
  geom_line(aes(y = mean_abs_verschil, x = jaar, group = plotnaam))
```

Mmmh, lijkt wel geclusterd te zijn
...
design2022 %>% 
  ...
  mapview(zcol = "mean_abs_verschil")
```


```{r kaart-verplaatste-telpunten}
probleemgevallen2022 <- mas_controle_afstand %>%
  filter(jaar == 2022, abs_verschil > 1)

dist2plotcirkels <- probleemgevallen2022 %>%
  st_buffer(dist = .$distance2plot)

design2022 %>% 
  inner_join(mas_controle_afstand %>%
              filter(jaar == 2022,
                     abs_verschil > 1) %>%
              distinct(plotnaam)
  ) %>%
  mapview() +
  mapview(probleemgevallen2022, zcol = "plotnaam", legend = FALSE) +
  mapview(dist2plotcirkels, zcol = "plotnaam", legend = FALSE,
          alpha.regions = 0)
```


```{r}
probleem_plot_2022 <- probleemgevallen2022 %>% distinct(plotnaam) %>%
  pull(plotnaam)

issue_list_raw %>%
  filter(pointid %in% probleem_plot_2022 |
           vervanging_id %in% probleem_plot_2022) %>%
  kable()
```

Iets misgelopen met de coördinaten van "VL" (bestaande) telpunten in 2022. 

```{r}
design_2018_2022 %>%
  mutate(heeft_conversieprobleem = plotnaam %in% probleem_plot_2022) %>%
  filter(str_detect(plotnaam, "VL"), !is.na(jaar), heeft_conversieprobleem) %>%
  mapview(zcol = "jaar")
```
...
```{r}
issue_list_raw %>%
  mutate(probleemgeval = pointid %in% probleem_plot_2022 |
           vervanging_id %in% probleem_plot_2022) %>%
  ggplot() +
  geom_bar(aes(x = probleemgeval, fill = openheid_sbp))
issue_list_raw %>%
  mutate(probleemgeval = pointid %in% probleem_plot_2022 |
           vervanging_id %in% probleem_plot_2022) %>%
  ggplot() +
  geom_bar(aes(x = probleemgeval, fill = wie))
issue_list_raw %>%
  mutate(probleemgeval = pointid %in% probleem_plot_2022 |
           vervanging_id %in% probleem_plot_2022) %>%
  ggplot() +
  geom_bar(aes(x = probleemgeval, fill = beslissing))
```

# Bepalen van de strata oppervlakte

```{r inlezen-steekproefkaderpuntenset}
...
perimeters_strata <- perimeters_strata %>%
  mutate(stratum = paste(
    abbreviate(Naam, 2), openheid_klasse, ifelse(is_sbp, "binnen", "buiten")))

mapview(perimeters_strata, zcol = "stratum")
```
