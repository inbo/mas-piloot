...
We maken een grid met cellen van met dezelfde oppervlakte als de telcirkels en leggen deze over het steekproefkader (= telpunten steekproefkader met buffer van 300 m).
...
Dit visualiseren we via `geom_tile()`.
...
p_veldleeuwerik2 <- plot_veldleeuwerik %>%
  rename(gemiddelde = Estimate) %>%
  pivot_longer(cols = c("gemiddelde",  "Q2.5", "Q97.5"), names_to = "statistiek",
               values_to = "waarde") %>%
  mutate(statistiek = factor(statistiek, levels = c("Q2.5", "gemiddelde", "Q97.5"))) %>%
  ggplot() +
    geom_sf(data = perimeter_ol, fill = "lightgrey", colour = "darkgrey",
            linewidth = 1) +
