...
niter <- 2000         # number of iterations (incl. burn-in)
...
conditional_effects(test)
plot(conditional_effects(test, offset = FALSE), points = F)
```
...
test_fitted <- fitted(test, new_df, re_formula = NA, offset = FALSE)

new_df %>%
  bind_cols(test_fitted) %>%
  st_as_sf(coords = c("x_plot", "y_plot"), crs = 31370) %>%
  st_buffer(300) %>%
  ggplot() +
    geom_sf(aes(fill = Estimate), colour = alpha("white", 0)) +
    scale_fill_gradient(low = "red", high = "blue",
                         limits = c(0, ceiling(max(test_fitted[, 1]))))
```
...
steekproefkader_buf <- steekproefkader %>%
  filter(Naam == "Oostelijke leemstreek",
         openheid_klasse %in% c("OL", "HOL")) %>%
  st_buffer(300)
grid1 <- st_make_grid(steekproefkader_buf, 100) %>%
  as_tibble() %>%
  rownames_to_column(var = "id") %>%
  st_as_sf(crs = 31370)

ids <- st_intersection(grid1, steekproefkader_buf) %>%
  pull(id)
...
test_fitted2 <- fitted(test, new_df2, re_formula = NA, offset = FALSE)

new_df2 %>%
  bind_cols(test_fitted2) %>%
  ...
  ggplot(aes(x_plot, y_plot)) +
    ...
    scale_fill_gradient(low = "red", high = "blue",
                        limits = c(0, ceiling(max(test_fitted2[, 1])))) +
    coord_sf(crs = 31370)
```
...
new_df2 %>%
  ...
  ggplot(aes(x_plot, y_plot)) +
    ...
    metR::geom_contour2(
      ...
      breaks = seq(0, 16, 3),
      label.placer = metR::label_placer_fraction(frac = 0.3, rot_adjuster = 0))
```
...
new_df2 %>%
  ...
  ggplot(aes(x_plot, y_plot)) +
    ...
    metR::geom_contour2(
      aes(z = Estimate, label = after_stat(level)),
      breaks = seq(0, 16, 3),
      label.placer = metR::label_placer_fraction(frac = 0.5, rot_adjuster = 0))
```
