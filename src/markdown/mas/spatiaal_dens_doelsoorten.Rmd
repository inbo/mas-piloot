...
$$
\ln(\lambda_{j,t}) = \beta_0 + f(x_j, y_j) + b_{0,j} + b_{1,j:t}
$$
...
We maken een grid met cellen van 100x100 m en leggen deze over het steekproefkader (met buffer van 300 m).
...
```{r}
p_veldleeuwerik <- plot_veldleeuwerik %>%
  ggplot() +
  geom_sf(data = perimeter_ol, fill = "lightgrey", colour = "darkgrey",
          linewidth = 1) +
    geom_raster(aes(x = x_plot, y = y_plot, fill = Estimate),
...
ggsave(here("media", "spatiale_densiteiten", "veldleeuwerik_dens_ol.png"),
       p_veldleeuwerik,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "veldleeuwerik_dens_ol.pdf"),
       p_veldleeuwerik,
       dpi = 300,
       width = 8,
       height = 4)
...
```{r}
p_gelekwikstaart <- plot_gelekwikstaart %>%
  ggplot() +
  geom_sf(data = perimeter_ol, fill = "lightgrey", colour = "darkgrey",
          linewidth = 1) +
    geom_raster(aes(x = x_plot, y = y_plot, fill = Estimate),
...
```{r}
dir.create(here("media", "spatiale_densiteiten"), showWarnings = FALSE)
ggsave(here("media", "spatiale_densiteiten", "gelekwikstaart_dens_ol.png"),
       p_gelekwikstaart,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "gelekwikstaart_dens_ol.pdf"),
       p_gelekwikstaart,
       dpi = 300,
       width = 8,
       height = 4)
...
```{r}
dir.create(here("media", "spatiale_densiteiten"), showWarnings = FALSE)
ggsave(here("media", "spatiale_densiteiten", "geelgors_dens_ol.png"),
       p_geelgors,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "geelgors_dens_ol.pdf"),
       p_geelgors,
       dpi = 300,
       width = 8,
       height = 4)
