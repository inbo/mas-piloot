...
We maken een hexagonaal grid met cellen van met gelijkaardige grootte als de telcirkels en leggen deze over het steekproefkader (met buffer van 300 m).
Voor de centroïde van elke grid cell maken we een predictie o.b.v. het model.

```{r}
radius <- 300

# Calculate side length of hexagon with the same area as the circle
side_length <- sqrt((2 * pi * radius^2) / (3 * sqrt(3)))

# Function to calculate hexagon vertices with adjusted side length
hexagon_vertices <- function(side_length) {
  angles <- seq(0, 2 * pi, length.out = 7)
  x <- side_length * cos(angles)
  y <- side_length * sin(angles)
  return(data.frame(x, y))
}

# Calculate hexagon vertices with adjusted side length
hexagon_data <- hexagon_vertices(side_length)
...
circle_data <- circle_points(radius)

# Plot hexagon with adjusted side length and circumscribed circle
ggplot() +
  geom_polygon(data = hexagon_data, aes(x, y),
  ...
  xlim(-400, 400) +
  ylim(-400, 400) +
  ggtitle("Hexagon with Adjusted Side Length and Circumscribed Circle")
```
...
Dit doen we aan de hand van de helft van het 95 % credible interval.
De helft van deze lengte heet de foutmarge.
De relatieve foutenmarge is dan deze foutmarge gedeeld door de schatting van de densiteit in die grid cell.
...
p_veldleeuwerik3 <- plot_veldleeuwerik %>%
  ggplot() +
    geom_sf(data = perimeter_ol, fill = "lightgrey", colour = "darkgrey",
            linewidth = 1) +
    geom_raster(aes(x = x_plot, y = y_plot, fill = rel_fout),
                interpolate = TRUE) +
    scale_fill_gradientn(colours = c("darkblue", "lightblue", "white",
                                     "gold", "red"),
                         breaks = c(0, 0.4, 0.6, 0.8, 1.2),
                         limits = c(0, 1.2)) +
    coord_sf(crs = 31370) +
    labs(x = "", y = "", fill = "Rel. foutenmarge aantal\nbroedparen per 100 ha",
         title = "Veldleeuwerik") +
    theme(legend.position = "bottom")
p_veldleeuwerik3
```
...
ggsave(here("media", "spatiale_densiteiten", "veldleeuwerik_fout_ol1.png"),
       p_veldleeuwerik3,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "veldleeuwerik_fout_ol1.pdf"),
       p_veldleeuwerik3,
       dpi = 300,
       width = 8,
       height = 4)

ggsave(here("media", "spatiale_densiteiten", "veldleeuwerik_fout_ol2.png"),
       p_veldleeuwerik4,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "veldleeuwerik_fout_ol2.pdf"),
       p_veldleeuwerik4,
       dpi = 300,
       width = 8,
       height = 4)
...
ggsave(here("media", "spatiale_densiteiten", "gelekwikstaart_fout_ol1.png"),
       p_gelekwikstaart3,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "gelekwikstaart_fout_ol1.pdf"),
       p_gelekwikstaart3,
       dpi = 300,
       width = 8,
       height = 4)

ggsave(here("media", "spatiale_densiteiten", "gelekwikstaart_fout_ol2.png"),
       p_gelekwikstaart4,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "gelekwikstaart_fout_ol2.pdf"),
       p_gelekwikstaart4,
       dpi = 300,
       width = 8,
       height = 4)
...
ggsave(here("media", "spatiale_densiteiten", "geelgors_fout_ol1.png"),
       p_geelgors3,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "geelgors_fout_ol1.pdf"),
       p_geelgors3,
       dpi = 300,
       width = 8,
       height = 4)

ggsave(here("media", "spatiale_densiteiten", "geelgors_fout_ol2.png"),
       p_geelgors4,
       dpi = 300,
       width = 8,
       height = 4)
ggsave(here("media", "spatiale_densiteiten", "geelgors_fout_ol2.pdf"),
       p_geelgors4,
       dpi = 300,
       width = 8,
       height = 4)
...
mapview(grid_veldleeuwerik,
        zcol = "rel_fout",
        alpha = 0,
        layer.name = "Rel. foutenmarge aantal broedparen per 100 ha",
        map.types = "OpenStreetMap")
```
