...
We moeten de functie `osmrTrip()` lichtjes aanpassen omdat de punten vaak niet op harde wegen liggen en dichtbij elkaar waardoor er een probleem is bij de output van de functie. Daarom outputten we geen geometry, maar enkel de afstand van het kortste traject.
...
```{r}
closest_dist_grouped <- closest_dist %>%
  st_drop_geometry() %>%
  group_by(regio) %>%
  summarise(som_intra_afstanden = sum(afstand)) %>%
  mutate(eenheid = "km")
closest_dist_grouped %>%
  kable()
```

De som van alle kortste afstanden binnen elke groep telpunten is `r sum(closest_dist_grouped$som_intra_afstanden)` km.

# Routeberekening naar clusters
...
- alles x2 voor heen en terug
  - wordt automatisch berekend door `osrmTrip()` voor afstanden tussen twee punten
...
We berekenen voor elke stad de korste afstand naar de centroïde van elke groep telpunten. Daarna selecteren we per groep de stad die het dichtstbij ligt. Die laatste afstand is dan de korste afstand heen en terug tussen de centroïde van een groep en de dichtste stad.
...
afstanden_df <- do.call(rbind, datalist) %>%
  group_by(groep) %>%
...
afstanden_df_min <- afstanden_df %>%
  mutate(
    ...
    tot_dist = dist * factor)
```

De som van alle korste afstanden van een stad naar elke groep telpunten is `r sum(afstanden_df_min$tot_dist)` km.

```{r}
afstanden_df_min_sum <- afstanden_df_min %>%
  st_drop_geometry() %>%
  group_by(regio) %>%
  summarise(teller_afstanden = sum(tot_dist)) %>%
  full_join(closest_dist_grouped, by = "regio") %>%
  select(regio, intra_afstanden = som_intra_afstanden, teller_afstanden) %>%
  rowwise() %>%
  mutate(totaal = sum(intra_afstanden, teller_afstanden))
afstanden_df_min_sum %>%
  kable()
```

`intra_afstanden` toont de afstanden binnen groepen van telpunten. `teller_afstanden` de afstanden heen en terug van de teller (stad) naar de centroïde van elke groep punten waarbij we gecorrigeerd hebben voor de grootte van de groep punten. `totaal` is de som. De totale afstand die in dit scenario zal worden afgelegd is `r sum(afstanden_df_min_sum$totaal)` km.  
...
De som van alle korste afstanden van een stad naar elke groep telpunten is `r sum(afstanden_df_max$tot_dist)` km.
...
De totale afstand die in dit scenario zal worden afgelegd is `r sum(afstanden_df_max_sum$totaal)` km.
...
bind_rows(afstanden_df_min_sum %>% mutate(scenario = "minimum"), 
  ...
  select(scenario, everything(), totaal_per_regio = totaal) %>%
  group_by(scenario) %>%
  mutate(totaal_per_scenario = sum(totaal_per_regio),
         eenheid = "km") %>%
  kable(digits = 2)
```

In het minimumscenario wordt in totaal (afstand binnen clusters + afstand van teller naar cluster (heen en terug)) `r sum(afstanden_df_min_sum$totaal)` km afgelegd. In het maximumscenario wordt in totaal `r sum(afstanden_df_max_sum$totaal)` km afgelegd.  
