...
Is er een significant verschil in het aantal soorten gezien door de professionele vrijwilliger vs. de vrijwilligers?
...
$\text{H}_0$: Er is geen verschil in het aantal soorten gezien door de professionele vrijwilliger vs. de vrijwilligers.  
$\text{H}_a$: Er is een verschil in het aantal soorten gezien door de professionele vrijwilliger vs. de vrijwilligers.
...
```{r}
fit <- nlme::lme(n ~ status, random = ~1|plaatstijd, 
                 data = dubbels_2022_df_soortenrijkdom)

summary(fit)
```

Zelfde verschil als met t-test. 3.2 soorten minder gemiddeld voor vrijwilliger.

We hebben discrete data ... Poisson?

Doe bovenstaande opnieuw met brms
