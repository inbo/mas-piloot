...
Zien we gelijkaardige trends als we ervaring mee in rekening brengen?
...
relfreqs_ervaring_diffs <- relfreqs_ervaring %>%
  ...
  ungroup() %>%
  rowwise() %>%
  mutate(max_val = pmax(`professioneel - ervaren`, `vrijwilliger - beginner`, 
                         `vrijwilliger - ervaren`),
         "grootste gem. rel.freq." = case_when(
           max_val == `professioneel - ervaren` ~ "professioneel - ervaren",
           max_val == `vrijwilliger - beginner` ~ "vrijwilliger - beginner",
           max_val == `vrijwilliger - ervaren` ~ "vrijwilliger - ervaren"
         )) %>%
  select(-max_val) %>%
  arrange(desc(`professioneel - ervaren`)) 

relfreqs_ervaring_diffs %>%
  kable(digits = 2)
```

Het is moeilijk om de figuren te interpreteren, maar we zien meestal wel dat de relatieve frequentie per soort hoger is bij beginnende vrijwilligers dan bij ervaren vrijwilligers.
