...
```{r}
diversiteit <- dubbels_2022_df %>%
  group_by(plotnaam, periode_in_jaar, status) %>%
  summarize(aantal_soorten = n_distinct(naam),
            .groups = "drop")
  

diversiteit %>%
  ggplot() +
    geom_boxplot(aes(x = status, y = aantal_soorten,
                     fill = periode_in_jaar)) +
    facet_wrap(~periode_in_jaar, scales = "free_x") +
    labs(y = "aantal soorten", x = "status", fill = "Telperipode")
```
...
aantal_wnm %>%
  ggplot() +
    geom_boxplot(aes(x = status, y = n,
                     fill = periode_in_jaar)) +
    facet_wrap(~periode_in_jaar, scales = "free_x") +
    labs(y = "aantal waarnemingen", x = "status", fill = "Telperipode")
```
...
aantal_dieren %>%
  ggplot() +
    geom_boxplot(aes(x = status, y = n,
                     fill = periode_in_jaar)) +
    facet_wrap(~periode_in_jaar, scales = "free_x") +
    labs(y = "aantal individuen", x = "status", fill = "Telperipode")
```
