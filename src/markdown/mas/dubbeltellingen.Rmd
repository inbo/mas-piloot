...
De correlatie tussen observaties binnen een zelfde plot is relatief hoog (ICC = `r round(icc_plot, 3)`) en de correlatie tussen observaties binnen eenzelfde periode in eenzelfde plot niet ((ICC = `r round(icc_plotperiode, 3)`)). We testen of we de random effects kunnen weglaten. Hiervoor fitten we een model zonder random effect van plot en zonder enig random effect.
...
dubbel_waarnemingen_norm13 <- brm(
  bf(n ~ status), 
  data = dubbels_2022_df_waarnemingen, family = gaussian(), seed = 1234,
  chains = nchains, warmup = burnin, iter = niter, cores = nparallel,
  file = paste0(model_path, "dubbel_waarnemingen_norm13"),
  file_refit = "on_change")
...
if (!file.exists(paste0(model_path, "fit_dubbel_waarnemingen_norm12.rds"))) {
  ...
  fit_dubbel_waarnemingen_norm13 <- add_criterion(dubbel_waarnemingen_norm13, 
    criterion = c("loo"), 
    file = paste0(model_path, "fit_dubbel_waarnemingen_norm13"))
} else {
  fit_dubbel_waarnemingen_norm12 <- readRDS(paste0(model_path, 
                                             "fit_dubbel_waarnemingen_norm12.rds"))
  fit_dubbel_waarnemingen_norm13 <- readRDS(paste0(model_path, 
                                             "fit_dubbel_waarnemingen_norm13.rds"))
}
...
models_norm <- data.frame(
  naam = c("geen random effects", 
           "plotnaam:periode_in_jaar", 
           "plotnaam en plotnaam:periode_in_jaar"),
  model = c("fit_dubbel_waarnemingen_norm13", 
...
loo_compare(
    fit_dubbel_waarnemingen_norm1, 
    fit_dubbel_waarnemingen_norm12, 
    fit_dubbel_waarnemingen_norm13, 
    criterion = "loo") %>%
  as.data.frame() %>%
  rownames_to_column("model") %>%
  left_join(models_norm, by = "model") %>%
...
We beschouwen het finale model met random effects.
...
https://discourse.mc-stan.org/t/using-brms-to-create-generated-quantities-stan-code-block/11766/10
...
dubbel_soorten_pois23 <- brm(
  bf(n ~ status*broedcode), 
  data = dubbels_2022_df_broed_soortenrijkdom, family = gaussian(), seed = 1234,
  chains = nchains, warmup = burnin, iter = niter, cores = nparallel,
  file = paste0(model_path, "dubbel_soorten_pois23"),
  file_refit = "on_change")
...
if (!file.exists(paste0(model_path, "fit_dubbel_soorten_pois22.rds"))) {
  ...
  fit_dubbel_soorten_pois23 <- add_criterion(dubbel_soorten_pois23, 
    criterion = c("loo"), 
    file = paste0(model_path, "fit_dubbel_soorten_pois23"))
} else {
  fit_dubbel_soorten_pois22 <- readRDS(paste0(model_path, 
                                             "fit_dubbel_soorten_pois22.rds"))
  fit_dubbel_soorten_pois23 <- readRDS(paste0(model_path, 
                                             "fit_dubbel_soorten_pois23.rds"))
}
...
models_norm <- data.frame(
  naam = c("geen random effects", 
           "plotnaam:periode_in_jaar", 
           "plotnaam en plotnaam:periode_in_jaar"),
  model = c("fit_dubbel_soorten_pois23", 
...
We controleren kwantificeren dit effect ook voor aantal waarnemingen ...
