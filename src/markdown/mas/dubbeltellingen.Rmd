...
# 1. Normal
dubbel_soorten_norm1 <- brm(bf(n ~ status + (1 | plaatstijd)), 
...
# 2. Poisson
dubbel_soorten_pois1 <- brm(bf(n ~ status + (1 | plaatstijd)), 
...
# 3. Negative binomial
dubbel_soorten_negbin1 <- brm(bf(n ~ status + (1 | plaatstijd)), 
...
comparisons_soorten <- loo_compare(fit_dubbel_soorten_norm1, fit_dubbel_soorten_pois1, 
...
```{r}
rbind(fit_dubbel_soorten_norm1$criteria$loo$estimates[, "Estimate"],
  fit_dubbel_soorten_pois1$criteria$loo$estimates[, "Estimate"],
  fit_dubbel_soorten_negbin1$criteria$loo$estimates[, "Estimate"]
  ) %>%
  as.data.frame() %>%
  mutate(model = c("Normal", "Poisson", "Negative binomial")) %>%
  full_join(comparisons_soorten, by = "model") %>%
...
pp_check(fit_dubbel_soorten_pois1, type = "bars_grouped", ndraws = 100, group = "status",
         facet_args = list(ncol = 1, scales = "free_y"))

pp_check(fit_dubbel_soorten_negbin1, type = "bars_grouped", ndraws = 100, group = "status",
         facet_args = list(ncol = 1, scales = "free_y"))
```
...
```{r}
sum_soorten <- summary(fit_dubbel_soorten_norm1)
sum_soorten
```
...
```{r, include=FALSE}
schatting_soorten <- round(sum_soorten$fixed["statusvrijwilliger", "Estimate"], 3)
ll_soorten <- round(sum_soorten$fixed["statusvrijwilliger", "l-95% CI"], 3)
ul_soorten <- round(sum_soorten$fixed["statusvrijwilliger", "u-95% CI"], 3)
sign <- ifelse(schatting_soorten > 0, "meer", "minder")
```

We besluiten dat de vrijwillige tellers gemiddeld `r schatting_soorten` soorten `r sign` zien dan de professionele teller (95% credible interval: `r ll_soorten` - `r ul_soorten`).
...
We besluiten dat de vrijwillige tellers gemiddeld `r schatting_waarnemingen` soorten `r sign` zien dan de professionele teller (95% credible interval: `r ll_waarnemingen` - `r ul_waarnemingen`).
