...
Kan een verschil in aantal soorten of aantal waarnemingen tussen professionele en vrijwillige tellers verschillen tussen ervaringsklasses?  
  
$\text{H}_0$: Het verschil in het aantal soorten gezien door de professionele teller vs. de vrijwilligers is even groot voor beginners als voor ervaren tellers.  
$\text{H}_a$: Het verschil in het aantal soorten gezien door de professionele teller vs. de vrijwilligers is niet even groot voor beginners als voor ervaren tellers.  
  
Het zal moeilijk zijn hiervoor een interactiemodel te fitten omdat we maar één ervaringscategorie bij de professionele teller hebben (ervaren) en twee bij de vrijwilligers (beginner en ervaren). We passen de hypotheses aan.
...
Zowel voor broedcode 0 als broedcode groter dan 0 zien we dat de professionele teller meer soorten ziet dan de vrijwilligers (95% CI resp. `r ll_soorten2_broed_nul * 100` tot `r ul_soorten2_broed_nul * 100` % en `r ll_soorten2_broed_groot` tot `r ul_soorten2_broed_groot * 100` % verschil). Het verschil in het aantal soorten gezien door de professionele teller vs. de vrijwilligers is niet significant verschillend voor broedcode 0 en broedcode groter dan 0 (95% CI `r ll_soorten2_broed_nul * 100` tot `r ul_soorten2_broed_nul * 100` % verschil).  
...
We testen het verschil in aantal soorten en waarnemingen voor ervaren vs. beginnende vrijwilligers.

```{r}
test <- brm(
    bf(n ~ ervaring  + (1 | plotnaam)), 
    data = dubbels_2022_df_soortenrijkdom %>% filter(status == "vrijwilliger"), 
...
pp_check(test, type = "dens_overlay_grouped", ndraws = 100, 
         group = "ervaring")

pp_check(test2, type = "bars_grouped", ndraws = 100, 
         group = "ervaring", facet_args = list(ncol = 1, scales = "free_y"))
```
