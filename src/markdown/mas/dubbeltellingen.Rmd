...
# 1. Normal
dubbel_fit_norm <- brm(bf(n ~ status + (1 | plaatstijd)), 
                data = dubbels_2022_df_soortenrijkdom, family = gaussian(), 
                chains = nchains, warmup = burnin, iter = niter, 
                cores = nparallel)

# 2. Poisson
dubbel_fit_pois <- brm(bf(n ~ status + (1 | plaatstijd)), 
                data = dubbels_2022_df_soortenrijkdom, family = poisson(), 
                chains = nchains, warmup = burnin, iter = niter, 
                cores = nparallel)

# 3. Negative binomial
dubbel_fit_negbin <- brm(bf(n ~ status + (1 | plaatstijd)), 
                data = dubbels_2022_df_soortenrijkdom, family = negbinomial(), 
                chains = nchains, warmup = burnin, iter = niter, 
                cores = nparallel)
```

```{r}
fit_dubbel_fit_norm <- add_criterion(dubbel_fit_norm, c("kfold"), K = 5, 
                                     cores = 5, folds = "stratified", 
                                     group = "status")
fit_dubbel_fit_pois <- add_criterion(dubbel_fit_pois, c("kfold"), K = 5, 
                                     cores = 5, folds = "stratified", 
                                     group = "status")
fit_dubbel_fit_negbin <- add_criterion(dubbel_fit_negbin, c("kfold"), K = 5, 
                                     cores = 5, folds = "stratified", 
                                     group = "status")

# Comparison among models
comparisons <- loo_compare(fit_dubbel_fit_norm, fit_dubbel_fit_pois, 
                           fit_dubbel_fit_negbin, 
...
```{r}
rbind(fit_dubbel_fit_norm$criteria$kfold$estimates[, "Estimate"],
  fit_dubbel_fit_pois$criteria$kfold$estimates[, "Estimate"],
  fit_dubbel_fit_negbin$criteria$kfold$estimates[, "Estimate"]
  ) %>%
  as.data.frame() %>%
  mutate(model = c("Normal", "Poisson", "Negative binomial")) %>%
...
```{r}
pp_check(fit_dubbel_fit_norm, type = "dens_overlay_grouped", ndraws = 100, 
         group = "status")

pp_check(fit_dubbel_fit_pois, type = "bars_grouped", ndraws = 100, group = "status",
         facet_args = list(ncol = 1, scales = "free_y"))

pp_check(fit_dubbel_fit_negbin, type = "bars_grouped", ndraws = 100, group = "status",
         facet_args = list(ncol = 1, scales = "free_y"))
```
...
```{r, include=FALSE}
sum <- summary(fit_dubbel_fit_norm)
schatting_soorten <- round(sum$fixed["statusvrijwilliger", "Estimate"], 3)
ll_soorten <- round(sum$fixed["statusvrijwilliger", "l-95% CI"], 3)
ul_soorten <- round(sum$fixed["statusvrijwilliger", "u-95% CI"], 3)
sign <- ifelse(schatting_soorten > 0, "meer", "minder")
```

We besluiten dat de professionele teller gemiddeld `r schatting_soorten` soorten `r sign` ziet dan de vrijwilligers (95% credible interval: `r ll_soorten` - `r ul_soorten`).
...
# 1. Normal
dubbel_waarnemingen_norm <- brm(bf(n ~ status + (1 | plaatstijd)), 
                data = dubbels_2022_df_waarnemingen, family = gaussian(), 
                chains = nchains, warmup = burnin, iter = niter, 
                cores = nparallel)

# 2. Poisson
dubbel_waarnemingen_pois <- brm(bf(n ~ status + (1 | plaatstijd)), 
                data = dubbels_2022_df_waarnemingen, family = poisson(), 
                chains = nchains, warmup = burnin, iter = niter, 
                cores = nparallel)

# 3. Negative binomial
dubbel_waarnemingen_negbin <- brm(bf(n ~ status + (1 | plaatstijd)), 
...
```{r}
fit_dubbel_waarnemingen_norm <- add_criterion(dubbel_waarnemingen_norm, c("kfold"), K = 5, 
                                     cores = 5, folds = "stratified", 
                                     group = "status")
fit_dubbel_waarnemingen_pois <- add_criterion(dubbel_waarnemingen_pois, c("kfold"), K = 5, 
                                     cores = 5, folds = "stratified", 
                                     group = "status")
fit_dubbel_waarnemingen_negbin <- add_criterion(dubbel_waarnemingen_negbin, c("kfold"), K = 5, 
                                     cores = 5, folds = "stratified", 
                                     group = "status")

# Comparison among models
comparisons <- loo_compare(fit_dubbel_waarnemingen_norm, fit_dubbel_waarnemingen_pois, 
                           fit_dubbel_waarnemingen_negbin, 
                           criterion = "kfold")[, c(1, 2)] %>%
                as.data.frame() %>%
                mutate(model = c("Normal", "Poisson", "Negative binomial"))
```
...
```{r}
rbind(fit_dubbel_waarnemingen_norm$criteria$kfold$estimates[, "Estimate"],
  fit_dubbel_waarnemingen_pois$criteria$kfold$estimates[, "Estimate"],
  fit_dubbel_waarnemingen_negbin$criteria$kfold$estimates[, "Estimate"]
  ) %>%
  as.data.frame() %>%
  mutate(model = c("Normal", "Poisson", "Negative binomial")) %>%
  full_join(comparisons, by = "model") %>%
...
```{r}
pp_check(fit_dubbel_waarnemingen_norm, type = "dens_overlay_grouped", ndraws = 100, 
         group = "status")

pp_check(fit_dubbel_waarnemingen_pois, type = "bars_grouped", ndraws = 100, group = "status",
         facet_args = list(ncol = 1, scales = "free_y"))

pp_check(fit_dubbel_waarnemingen_negbin, type = "bars_grouped", ndraws = 100, group = "status",
         facet_args = list(ncol = 1, scales = "free_y"))
```
...
```{r}
plot(fit_dubbel_waarnemingen_norm)
```

En gelijkaardige resultaten als de t-test. Met een duidelijk verschil tussen de professionele teller en de vrijwilligers.

```{r}
summary(fit_dubbel_waarnemingen_norm)
```

```{r}
hyp1 <- hypothesis(
  x = fit_dubbel_waarnemingen_norm,
  "statusvrijwilliger = 0",
  class = "b", alpha = 0.05)
hyp1
plot(hyp1)
...
```{r, include=FALSE}
sum <- summary(fit_dubbel_waarnemingen_norm)
schatting_waarnemingen <- round(sum$fixed["statusvrijwilliger", "Estimate"], 3)
ll_waarnemingen <- round(sum$fixed["statusvrijwilliger", "l-95% CI"], 3)
ul_waarnemingen <- round(sum$fixed["statusvrijwilliger", "u-95% CI"], 3)
sign <- ifelse(schatting_waarnemingen > 0, "meer", "minder")
```

We besluiten dat de professionele teller gemiddeld `r schatting_waarnemingen` soorten `r sign` ziet dan de vrijwilligers (95% credible interval: `r ll_waarnemingen` - `r ul_waarnemingen`).
