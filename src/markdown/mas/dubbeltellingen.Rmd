...
Op het eerste zicht zien we geen ovallend grote verschillen in aantal soorten gezien tenzij voor broedcode 3.
...
Als we de broedcodes > 0 samen nemen zien we ook geen groot verschil in aantal soorten tussen professionelen en vrijwilligers.
...
diversiteit_df_broed %>%
  mutate(broedcode = factor(ifelse(wrntype == 0, "0", "> 0"),
                            ...
                            ordered = TRUE)) %>%
  ggplot() +
    geom_boxplot(aes(x = broedcode, y = n)) +
    scale_y_continuous(breaks = seq(0, 100, 2), 
                       limits = c(0, NA)) +
    geom_hline(yintercept = 0, linetype = 2, colour = "grey") +
    facet_wrap(~status) +
...
We zien gelijkaardige trends als bij het aantal soorten.
