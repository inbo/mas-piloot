  ...
  p <- doel_df %>%
    ...
    mutate(wrntype_omschrijving2 = paste(wrntype, wrntype_omschrijving, 
           ...
           wrntype_omschrijving2 = reorder(wrntype_omschrijving2, wrntype,
                                           decreasing = TRUE)) %>%
    filter(naam == spec) %>%
    select(jaar, regio, wrntype_omschrijving2) %>%
    group_by(jaar, regio, wrntype_omschrijving2) %>%
    summarise(N = n(), .groups = "drop") %>%
    full_join(strata_df %>% select(-stratum) %>% distinct(),
              by = join_by(regio, jaar)) %>%
    mutate(regio = ifelse(regio == "Oostelijke leemstreek",
                          gsub("\\s", "\n", regio),
                          regio)) %>%
    ggplot(aes(x = factor(jaar), y = N, fill = wrntype_omschrijving2)) +
      geom_hline(yintercept = 0, alpha = 0.2, linetype = "dashed") +
...
p_summ <- gridExtra::grid.arrange(
  p1 +
    theme(plot.margin = unit(c(5.5, -2, 5.5, 5.5), "pt")),
  p2 + 
    labs(y = "") +
    theme(plot.margin = unit(c(5.5, -2, 5.5, -2), "pt")),
  p3 +
    labs(y = "") +
    theme(plot.margin = unit(c(5.5, 5.5, 5.5, -2), "pt")),
  p4 +
    theme(plot.margin = unit(c(5.5, -2, 5.5, 5.5), "pt")),
  p5 + 
    labs(y = "") +
...
ggsave(here(media_folder, "broedcode_mas_2022_2023.pdf"),
       p_summ,
       dpi = 300,
       width = 6,
       height = 10)
...
p <- doel_df %>% 
  st_drop_geometry() %>%
  mutate(plotnaam = paste(plotnaam, jaar, sep = "_")) %>%
...
ggsave(here(media_folder, "soortenfreqs_doel_per_strat.pdf"),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
ggsave(here(media_folder, "soortenfreqs_zoogdieren_2022_2023.pdf"),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
p <- zoogdieren %>%
  ...
  summarise(totaal_aantal = sum(aantal),
            .groups = "drop") %>%
  ggplot(aes(x = periode_in_jaar, y = totaal_aantal, fill = factor(jaar))) +
    geom_hline(yintercept = 0, alpha = 0.2, linetype = "dashed") +
...
ggsave(here(media_folder, "aantallen_haas.pdf"),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
p <- zoogdieren %>%
  ...
  ggplot() +
    ...
    facet_grid(~regio, scales = "free_x") +
    theme(legend.position = "top")
p

ggsave(here(media_folder, "soortenfreqs_haas_per_strat.pdf"),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
ggsave(here(media_folder, "tellingen_mas_2018_2023.pdf"),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
ggsave(here(media_folder, "waarnemingen_mas_2018_2023.pdf"),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
ggsave(here(media_folder, "soorten_mas_2018_2023.pdf"),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
ggsave(here(media_folder, "soortenfreqs_mas_2018_2023.pdf"),
       p + theme(plot.margin = unit(c(5.5, 15.5, 5.5, -10), "pt")),
       dpi = 300,
       width = 6,
       height = 10)
```
...
```{r}
freq_to_plot2 <- relfreqs_vogels %>%
  filter(jaar %in% 2018:2021) %>%
  mutate(naam = reorder(naam, relfreq, FUN = "mean",
                        decreasing = FALSE)) %>%
  filter(as.numeric(naam) > length(unique(naam)) - 20 |
           naam %in% doelsoorten)

p_freq_to_plot2 <- freq_to_plot2 %>%
  ggplot() +
  geom_vline(xintercept = 0, alpha = 0.2, linetype = "dashed") +
  geom_hline(yintercept = length(unique(freq_to_plot2$naam)) - 20 + 0.5,
             alpha = 0.2, linetype = "dashed") +
  geom_point(aes(x = relfreq, y = naam, colour = regio),
             size = 1) +
  labs(y = "", x = "Relatieve frequentie", colour = "Regio") +
...
ggsave(here(media_folder, "soortenfreqs_mas_2018_2021.pdf"),
       p_freq_to_plot2,
       dpi = 300,
       width = 8,
       height = 8)

freq_to_plot3 <- relfreqs_vogels %>%
  filter(jaar %in% 2022:2023) %>%
  mutate(naam = reorder(naam, relfreq, FUN = "mean",
                        decreasing = FALSE)) %>%
  filter(as.numeric(naam) > length(unique(naam)) - 20 |
...
p_freq_to_plot3 <- freq_to_plot3 %>%
  ggplot() +
  geom_vline(xintercept = 0, alpha = 0.2, linetype = "dashed") +
  geom_hline(yintercept = length(unique(freq_to_plot3$naam)) - 20 + 0.5,
             alpha = 0.2, linetype = "dashed") +
  geom_point(aes(x = relfreq, y = naam, colour = regio),
             size = 1) +
  labs(y = "", x = "Relatieve frequentie", colour = "Regio") +
...
ggsave(here(media_folder, "soortenfreqs_mas_2022_2023.pdf"),
       p_freq_to_plot3,
       dpi = 300,
       width = 8,
       height = 6)
...
relfreq_diff_dm <- relfreqs_vogels %>%
  select(-n) %>%
  pivot_wider(names_from = regio,
              values_from = relfreq,
              values_fill = 0) %>%
...
p <- relfreq_diff_dm %>%
  ggplot() +
    geom_vline(xintercept = 0,
               colour = "lightgrey") +
    geom_hline(yintercept = length(unique(relfreq_diff_dm$naam)) - 20 + 0.5,
               alpha = 0.2, linetype = "dashed") +
    ...
    facet_wrap(~jaar, nrow = 3) +
    scale_y_discrete(labels = function(x) highlight(x, doelsoorten, "red")) +
    coord_cartesian(xlim = c(0 - abs(max(relfreq_diff_dm$verschilfreq)),
                             0 + abs(max(relfreq_diff_dm$verschilfreq)))) +
...
ggsave(here(media_folder, "verschilfreq_dm_mas_2018_2023.pdf"),
       p + theme(plot.margin = unit(c(5.5, 15.5, 5.5, -10), "pt")),
       dpi = 300,
       width = 6,
       height = 10)
...
```{r, fig.height = 200/25.4}
relfreq_diff_ol <- relfreqs_vogels %>%
  select(-n) %>%
  pivot_wider(names_from = regio,
              values_from = relfreq,
              values_fill = 0) %>%
...
p <- relfreq_diff_ol %>%
  ggplot() +
    geom_vline(xintercept = 0,
               colour = "lightgrey") +
    geom_hline(yintercept = length(unique(relfreq_diff_ol$naam)) - 20 + 0.5,
               alpha = 0.2, linetype = "dashed") +
    geom_point(aes(x = verschilfreq, y = naam),
               size = 1) +
    labs(y = "", x = "Verschil in relatieve frequentie") +
    facet_wrap(~jaar, nrow = 3) +
    scale_y_discrete(labels = function(x) highlight(x, doelsoorten, "red")) +
    coord_cartesian(xlim = c(0 - abs(max(relfreq_diff_ol$verschilfreq)),
                             0 + abs(max(relfreq_diff_ol$verschilfreq)))) +
    scale_x_continuous(breaks = seq(-1, 1, 0.25)) +
    theme(legend.position = "top",
          axis.text = element_text(size = 8),
          axis.text.y = element_markdown(),
...
ggsave(here(media_folder, "verschilfreq_ol_mas_2018_2023.pdf"),
       p + theme(plot.margin = unit(c(5.5, 15.5, 5.5, -10), "pt")),
       dpi = 300,
       width = 6,
       height = 10)
...
Top tien van beide kanten in 1 plot.
...
p <- relfreq_diff_total %>%
  ggplot() +
    ...
    guides(colour = guide_legend(override.aes = list(size = 3))) +
    annotate("segment",
             x = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
             y = c(length(unique(relfreq_diff_total$naam)) - 10 + 1),
             xend = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
    ...
    annotate("segment",
           ...
           arrow = arrow(type = "closed", length = unit(0.015, "npc")),
           colour = "black") +
    annotate("text", label = c("De Moeren"),
             x = c(abs(max(relfreq_diff_dm$verschilfreq)) + 0.03),
             y = c(length(unique(relfreq_diff_total$naam)) - 10 + 1),
             size = 2, hjust = "left", colour = "black", angle = 90) +
    annotate("text", label = c("Oostelijke leemstreek"),
           x = c(abs(max(relfreq_diff_dm$verschilfreq)) + 0.03),
           y = c(10.5 - 0.5),
           size = 2, hjust = "right", colour = "black", angle = 90)
p

ggsave(here(media_folder, "verschilfreq_total_mas_2018_2023.pdf"),
       p + theme(plot.margin = unit(c(5.5, 15.5, 5.5, -10), "pt")),
       dpi = 300,
       width = 6,
       height = 10)
...
```{r}
relfreq_diff_total2 <- relfreqs_vogels %>%
  select(-n) %>%
  filter(jaar %in% 2018:2021) %>%
  pivot_wider(names_from = regio,
              values_from = relfreq,
...
p_relfreq_diff_total2 <- relfreq_diff_total2 %>%
  ggplot() +
    geom_vline(xintercept = 0,
               colour = "lightgrey") +
    geom_hline(yintercept = c(10.5,
                length(unique(relfreq_diff_total2$naam)) - 10 + 0.5),
               alpha = 0.2, linetype = "dashed") +
    geom_point(aes(x = verschilfreq, y = naam),
               size = 1) +
    labs(y = "", x = "Verschil in relatieve frequentie") +
    ...
    annotate("segment",
             x = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
             y = c(length(unique(relfreq_diff_total2$naam)) - 10 + 1),
             xend = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
             yend = c(length(unique(relfreq_diff_total2$naam)) - 10 + 3),
             arrow = arrow(type = "closed", length = unit(0.015, "npc")),
             colour = "black") +
    annotate("segment",
           x = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
...
ggsave(here(media_folder, "verschilfreq_total_mas_2018_2021.pdf"),
       p_relfreq_diff_total2 +
         theme(plot.margin = unit(c(5.5, 15.5, 5.5, -10), "pt")),
       dpi = 300,
       width = 8,
       height = 8)

relfreq_diff_total3 <- relfreqs_vogels %>%
  select(-n) %>%
  filter(jaar %in% 2022:2023) %>%
  pivot_wider(names_from = regio,
              values_from = relfreq,
...
p_relfreq_diff_total3 <- relfreq_diff_total3 %>%
  ggplot() +
    geom_vline(xintercept = 0,
               colour = "lightgrey") +
    geom_hline(yintercept = c(10.5,
                length(unique(relfreq_diff_total3$naam)) - 10 + 0.5),
               alpha = 0.2, linetype = "dashed") +
    geom_point(aes(x = verschilfreq, y = naam),
               size = 1) +
    labs(y = "", x = "Verschil in relatieve frequentie") +
    ...
    annotate("segment",
             x = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
             y = c(length(unique(relfreq_diff_total3$naam)) - 10 + 1),
             xend = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
             yend = c(length(unique(relfreq_diff_total3$naam)) - 10 + 3),
             arrow = arrow(type = "closed", length = unit(0.015, "npc")),
             colour = "black") +
    annotate("segment",
           x = c(abs(max(relfreq_diff_dm$verschilfreq)) - 0.03),
...
ggsave(here(media_folder, "verschilfreq_total_mas_2022_2023.pdf"),
       p_relfreq_diff_total3 +
         theme(plot.margin = unit(c(5.5, 15.5, 5.5, -10), "pt")),
       dpi = 300,
       width = 8,
       height = 6)
...
ggsave(here(media_folder,
            paste0("soortbeschermingsmaatregelen1_2018_2023.pdf")),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
```{r}
palette <- colorRampPalette(RColorBrewer::brewer.pal(9, name = 'Set1'))(length(unique(mas_vogels$plotnaam)))

p <- mas_vogels %>%
  distinct(plotnaam, jaar, regio, sbp, openheid, stratum, area_prop_sb) %>%
  ggplot() +
    geom_boxplot(aes(x = factor(jaar), y = area_prop_sb)) +
    geom_line(aes(x = factor(jaar), y = area_prop_sb, group = plotnaam,
                  colour = plotnaam), alpha = 0.2) +
    facet_wrap(~regio) +
    scale_colour_manual(values = palette) +
    labs(x = "", y = "Oppervlakte-aandeel soorbeschermingsmaatregelen") +
    theme(legend.position = "",
          legend.title = element_blank())
p

ggsave(here(media_folder, paste0("soortbeschermingsmaatregelen2_2018_2023.pdf")),
       p,
       dpi = 300,
       width = 8,
       height = 6)
...
p <- mas_vogels %>%
  distinct(plotnaam, jaar, regio, sbp, openheid, stratum, area_prop_sb) %>%
  mutate(sbp = paste(sbp, "sbp")) %>%
  ggplot() +
    geom_boxplot(aes(x = factor(jaar), y = area_prop_sb)) +
    geom_line(aes(x = factor(jaar), y = area_prop_sb, group = plotnaam,
                  colour = plotnaam), alpha = 0.2) +
    facet_grid(regio~sbp) +
    scale_colour_manual(values = palette) +
    labs(x = "", y = "Oppervlakte-aandeel soorbeschermingsmaatregelen") +
    theme(legend.position = "",
          legend.title = element_blank())
...
ggsave(here(media_folder, paste0("soortbeschermingsmaatregelen3_2018_2023.pdf")),
       p,
       dpi = 300,
       width = 8,
       height = 6)
