...
> nog niet finaal! wacht op Johannes week van 31 juli

We lezen de data van 2023 in en voegen ze toe aan de eerdere nieuwe export.

```{r}
file_geojson_2023 <- file.path(data_path, 
                               "20230731_qgis_export_sovon_wfs_2023.geojson")
mas_geojson_2023 <- read_sf(file_geojson_2023)

mas_geojson_2018_2023 <- bind_rows(mas_geojson_new,
                                   mas_geojson_2023)
```
...
mas_31370_pipeline <- 
  st_transform(st_geometry(mas_geojson_2018_2023), "EPSG:31370",
               pipeline = chosen_pipeline_definition) %>% 
  st_sf(st_drop_geometry(mas_geojson_2018_2023), geometry = .) %>% 
  as_tibble() %>% 
  st_as_sf() %>%
...
De dataframe `telpunten_2018_2021` bevat alle MAS telpunten die in de periode 2018-2021 zijn doorgegeven aan SOVON. Het is echter niet zo dat al deze telpunten in elk jaar geteld zijn. Het kan zelfs zijn dat een telpunt uit deze lijst in geen enkel jaar werd geteld (= helemaal niet bezocht geweest). Voor 2022-2023 kennen we alle telpunten die geteld/bezocht moesten worden. Wel zijn de locaties in 2022-2023 verkeerd voor VL plots. Dit zijn reeds bestaande plots en zitten dus al in de lijst van 2018-2021. We selecteren voor 2022-2023 dus enkel de nieuwe plots.
...
# 2022-2023
design_2022_2023_raw <- read_csv(file.path(
    "data",
    "processed",
...
design_2022_2023 <- design_2022_2023_raw %>%
...
telpunten_2018_2023 <- bind_rows(design_2018_2021, design_2022_2023) %>%
...
mas_perimeters <- mas_telperiodes %>%
  filter(plotnaam %in% telpunten_2018_2023$plotnaam)
```
...
mas_design <- mas_perimeters %>%
  ...
  inner_join(telpunten_2018_2023, by = "plotnaam") %>%
  st_as_sf()
```
...
Voor de waarnemingen verwijderen we dubbeltellingen. Indien binnen een jaar binnen eenzelfde telperiode op meerdere teldagen OF op eenzelfde teldag door verschillende waarnemers werd geteld, wordt de laatste teldag genomen tenzij er ook door een professionele teller is geteld. Dan nemen we de telling van de professionele teller.
...
Ol_2008.5 testje
maak extra variabele van persoon meest geteld in jaar in plot
als die een dubbeltelling geeft toewijzen, anders prof, anders laatste teldag
...
# Toevoegen specifieke variabelen
## Status teller
...
We maken ook een variabele met per plot, per telperiode, per jaar het aantal predatoren (individuen en soorten).

```{r}
mas_predators <- mas_profs %>%
  mutate(predator = ifelse(naam %in% predatoren, "ja", "nee"),
         predator_num = ifelse(naam %in% predatoren, aantal, 0),
         predator_spec = ifelse(naam %in% predatoren, naam, NA)) %>%
  group_by(plotnaam, jaar, periode_in_jaar) %>%
  mutate(n_predators = sum(predator_num),
         n_predators_spec = n_distinct(predator_spec, na.rm = TRUE)) %>%
  ungroup() %>%
  select(-c(predator_num, predator_spec))
```

## Plotspecifieke variabelen

`inner_join()` met verklarende variabelen berekend in `verzameling_verklarende_variabelen.Rmd`. 
Dit zijn variabelen op plot- en jaarniveau.

```{r}
verklarende_variabelen_plotjaar <- read_csv(
  file.path("data", "processed", "verklarende_variabelen_2018_2022.csv")
  )

mas_finaal_df <- mas_predators %>%
  inner_join(verklarende_variabelen_plotjaar, 
             by = join_by(plotnaam, jaar, regio))
```

# Controle ontbrekende gegevens

Enkel ontbrekende gegevens waar verwacht.

```{r}
apply(mas_finaal_df, 2, function(x) any(is.na(x)))
```
...
```{r}
glimpse(mas_finaal_df)
```

Aantal telpunten:

```{r}
plots_per_telperiode <- mas_finaal_df %>%
  st_drop_geometry() %>%
  group_by(jaar, periode_in_jaar) %>%
  summarise(n_per_telperiode = n_distinct(plotnaam)) %>%
  ungroup()

plots_per_telperiode %>%
  arrange(jaar, periode_in_jaar) %>%
  kable()
```

```{r}
plots_per_jaar <- mas_design %>%
  st_drop_geometry() %>%
  group_by(jaar) %>%
  summarise(n_per_jaar = n_distinct(plotnaam)) %>%
  ungroup()
...
Niet elke plot in elke periode geteld?

```{r}
plots_per_telperiode %>%
  full_join(plots_per_jaar, by = join_by(jaar)) %>%
  arrange(jaar) %>%
  kable()
```
...
mas_finaal_df %>%
  ...
  summarise(n_per_telperiode = n_distinct(periode_in_jaar),
            .groups = "drop") %>%
  ggplot(aes(x = n_per_telperiode)) +
    geom_bar() +
    geom_text(stat = "count", aes(label = after_stat(count)), vjust = 0,
              colour = "black") +
    facet_wrap(~jaar, scales = "free_y") +
    labs(x = "aantal bezoeken van eenzelfde telpunt per jaar", y = "frequentie")
```

We bekijken enkele voorbeelden. 
Sommige plots zijn niet in elke telperiode geteld.
Anderen zijn niet binnen de correcte datumintervallen geteld waardoor ze worden weggefilterd.
Nog anderen zijn tweemaal in eenzelfde telperiode geteld en niet meer in een andere één van de twee tellingen wordt behouden.

VL0039:  
3x geteld in 2018: niets van data in periode 4  
4x geteld in 2019  
3x geteld in 2020: geteld op 20 maart voor periode 1 -> weggefilterd  
4x geteld in 2021

VL0050:  
2x geteld in 2018: geteld tussen telperiodes 2 en 3 op 16 juni, niets van data in in periode 4  
3x geteld in 2019: dubbeltelling telperiode 4  
3x geteld in 2020: geteld buiten range telperiode 4 op 18 juni -> weggefilterd

VL0004:  
4x geteld in 2018  
1x geteld in 2020: effectief maar data voor 1 telperiode

VL0297:  
2x geteld in 2018: niets van data in periodes 2 en 3  
2x geteld in 2019: niets van data in periodes 1 en 2  
4x geteld in 2020  
3x geteld in 2021: niets van data in periode 1  
4x geteld in 2022

VL0538:  
3x geteld in 2021: geteld buiten range telperiode 4 op 11 juni -> weggefilterd  
2x geteld in 2022: niets van data in periode 1, geteld buiten range telperiode 4 op 16 juli -> weggefilterd
	
Ol_9862.14:  
3x geteld in 2022: geteld tussen telperiodes 2 en 3 op 12 juni -> weggefilterd

Ol_12609.15:  
3x geteld in 2022: dubbelgeteld in periode 1, niet geteld in periode 2

DM_257.2:  
3x geteld in 2022: geteld tussen telperiodes 2 en 3 op 17 juni -> weggefilterd
...
```{r}
write_csv(st_drop_geometry(mas_finaal_df), 
          file.path("data", "processed", "MAS_data_full_2018_2022.csv"))
```
