...
test2 <- filter(mas_controle_afstand2, plotnaam == unique(probleempunten$plotnaam)) %>% 
  select(-jaar.telpunt) %>% 
  distinct()
...
mapview::mapview(probleempunten$geometry.stip) +
  mapview::mapview(st_buffer(test2, 
                             test2$distance2plot),
                   color = "black", alpha.regions = 0) +
  mapview::mapview(probleempunten$geometry.telpunt, 
                   col.regions = "red") +
  mapview::mapview(st_buffer(probleempunten$geometry.telpunt, 300), 
                   color = "red", alpha.regions = 0) +
  mapview::mapview(test2, col.regions = "green")
```
...
> Waarom kwam dit punt niet eerder op? In de eerdere export waren de afstande wel t.o.v. correct punt gemeten.
