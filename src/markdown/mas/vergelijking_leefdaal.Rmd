...
Continue variabelen worden gecentreerd (trek gemiddelde af per kolom) en geschaald (deel door standaard deviatie per kolom): `scale()`. Voor compositionele data (kwantitatieve beschrijvingen van de delen van een geheel, i.e. proporties tussen 0 en 1 die sommeren tot 1) moeten de gegevens worden getransformeerd naar de gebruikelijke Euclidische geometrie. Hiervoor voeren we de gecentreerde log-ratio transformatie uit: `compositions::clr()`.  
...
x <- landgebruik_leefdaal_df_unscaled %>%
      select_if(is.numeric)
D <- ncol(x)
n <- nrow(x)
V <- matrix(nrow = D, ncol = n)

for (l in seq_len(D)) {
  # The ilr transformation of x(l) results in z(l)
  df <- x %>%
    select(l, everything()) %>%
    compositions::ilr() %>%
    as_tibble()
  
  # The first ilr variable z(l) explains all the relative information
  # about the original compositional part x(l)
  V[l,] <- svd(df)$v[1,]
}
...
Deze resultaten zijn niet in overeenstemming met onze eerdere conclusies in \@ref(landgebruik). Daar besloten we dat Kortenberg iets meer gelijkaardig was aan Zavelberg dan Lovenjoel, maar dat er geen groot verschil is. Toen beschouwden we echter geen relatieve proporties, maar absolute aantallen. Inderdaad, als we kijken naar de verschillen in ratio's met Zavelberg zien we dat Lovenjoel vaker een kleiner verschil in log-ratio heeft met Zavelberg dan Kortenberg en dat de opgetelde verschillen ook kleiner zijn voor Lovenjoel.
...
```{r}
conclusie_df <- rbind(conclusie_df, 
                      c("landgebruiksamenstelling log-ratio", "-", "+", 
                        "correctie na PCA o.b.v. relatieve data"))
```
...
Deze resultaten zijn in overeenstemming met onze conclusie in \@ref(openheid). Daar besloten we dat Kortenberg veel gelijkaardiger is aan Zavelberg in termen van openheid van het landschap dan Lovenjoel. Hier zien we dezelfde trend hoewel het verschil niet zo uitgesproken is als verwacht.
