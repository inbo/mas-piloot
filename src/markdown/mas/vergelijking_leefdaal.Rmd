...
conclusie_df <- rbind(conclusie_df, c("perceelsgrootte", "0", "0", 
                        "voordeel Lovenjoel niet super groot"))
```
...
De eerste PC verklaart ruwweg 2/3 van de variatie en de tweede as 1/3.
...
pca_fit %>%
  ...
  ggplot(aes(PC, percent)) +
    ...
    geom_text(aes(label = round(percent, digits = 3)), vjust = -0.5)
```
...
```{r}
pca_fit %>%
  broom::augment(leefdaal_multivar) %>%
  ggplot(aes(.fittedPC1, .fittedPC2, colour = .rownames)) + 
    geom_point(size = 3) +
    scale_y_continuous(limits = c(-7.5, 7.5)) +
    scale_x_continuous(limits = c(-7.5, 7.5)) +
    geom_text(aes(label = .rownames), vjust = -1) +
    geom_text(aes(label = paste0("(", round(.fittedPC1, digits = 2), ", ", 
              round(.fittedPC2, digits = 2), ")")), vjust = 1.5) +
    geom_hline(yintercept = 0, colour = "grey", linetype = "dashed") +
    geom_vline(xintercept = 0, colour = "grey", linetype = "dashed") +
    theme(legend.position = "") +
    labs(x = "PC1", y = "PC2")
