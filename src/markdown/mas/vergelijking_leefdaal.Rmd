...
zie file:///C:/R/git_repositories/mbag/src/markdown/mas/afbakening_steekproefkader.html#162_Oppervlakte-aandeel_hoofdteelt

```{r, fig.height=15}
hfdteelten_leefdaal %>%
  ggplot(aes(x = naam, y = area_prop)) +
    geom_bar(aes(fill = naam), stat = "identity") +
    facet_wrap(~LBLHFDTLT, ncol = 3, scales = "free_y") +
    labs(x = "", y = "oppervlakte-aandeel", title = "Hoofdteelten") +
    theme(legend.position = "", 
          axis.text.x = element_text(angle = 90, vjust = 0.5))
```
