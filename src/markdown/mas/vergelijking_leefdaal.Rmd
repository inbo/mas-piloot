...
Deze scores zijn tot op een zekere hoogte subjectief, maar ze kunnen ons op het einde een overzicht bieden van de voor- en nadelen tussen de verschillende gebieden. De finale keuze moet wel nog steeds afhangen van een meer genuanceerde discussie. Sommige categoriën kunnen groter doorwegen dan andere.
...
Landgebruiksamenstelling 2019 -> nieuwe file? om de 3 jaar  
...
kleur <- legende_landgebruik %>%
  pull(kleur)
mapview(leefdaal, zcol = "naam", layer.name = "Deelgebied") +
  mapview(raster, layer.name = "Landgebruik", col.regions = kleur,
        att = "land_use")
```
...
Kortenberg heeft 1 keer vaker een kleiner verschil met Zavelberg dan Lovenjoel. Dit zegt wel niets over de grootte van het verschil. 
...
Kortenberg heeft vaker een kleiner verschil in proportie landgebruik met Zavelberg dan Lovenjoel en het totale verschil is ook kleiner.

```{r}
conclusie_df <- rbind(conclusie_df, c("landgebruiksamenstelling", "+", "-"))
```
...
We willen een overzicht van de verschillen met Zavelberg (categorie ‘Kortenberg/Lovenjoel’: verschil met Zavelberg gelijk voor controlegebieden).
...
Op vlak van oppervlakte-aandeel beheersovereenkomsten is er in Kortenberg bijna niets te vinden en Lovenjoel is in dat opzicht meer vergelijkbaar met Zavelberg. Desondanks wordt in Lovenjoel wel vaak op andere beheersvormen ingezet dan in Zavelberg. Hierdoor heeft Kortenberg wel vaker een kleiner verschil in oppervlakte beheersovereenkomst met Zavelberg.
...
Kortenberg heeft vaker een kleiner verschil met Zavelberg dan Lovenjoel (categorie 'Kortenberg/Lovenjoel': verschil met Zavelberg gelijk voor controlegebieden), maar dit is maar 16 keer ten opzichte van 15 dus dit is geen groot verschil.
...
Zowel Kortenberg als Lovenjoel hebben ongeveer even vaak een kleiner verschil met Zavelberg.

```{r}
conclusie_df <- rbind(conclusie_df, c("hoofdteelten", "0", "0"))
```
...
De verschillen lijken kleiner in Kortenberg. Inderdaad, Kortenberg heeft steeds een kleiner verschil met Zavelberg dan Lovenjoel (categorie ‘Kortenberg/Lovenjoel’: verschil met Zavelberg gelijk voor controlegebieden).
...
```{r}
conclusie_df <- rbind(conclusie_df, c("openheid", "+", "-"))
```
