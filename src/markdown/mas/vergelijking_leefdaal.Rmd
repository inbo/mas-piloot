...
Volgende code werkt niet: geeft GitRepositories en ik heb git_repositories.
`targets_store <- here("src", "targets", "mas_steekproef_pilootfase", "_targets")`
`tar_read(vito_lum_2019_file, store = targets_store)` 


```{r}
# Lees in landgebruik raster
landgebruik <- rast(here("data", "landgebruik", "vito", 
                      "lu_landgebruik_vlaa_2019_v2.tif"))
landgebruik_leefdaal <- crop(landgebruik, leefdaal) %>%
  raster::raster()

# Visualisatie
mapview(leefdaal, zcol = "naam", layer.name = "Deelgebied", 
        alpha.regions = 0.6, alpha = 1) +
  mapview(landgebruik_leefdaal, layer.name = "Landgebruik", 
        alpha.regions = 0.4)
```
...
De verschillen lijken kleiner in Lovenjoel. Inderdaad Lovenjoel heeft vaker een kleiner verschil met Zavelberg dan Kortenberg (categorie 'Kortenberg/Lovenjoel': verschil met Zavelberg gelijk voor controlegebieden).
...
We zien vaker kleinere verschillen met Zavelberg in oppervlakte-aandeel hoofdteelten in Lovenjoel dan in Kortenberg.

```{r}
conclusie_df <- rbind(conclusie_df, c("hoofdteelten", "-", "+"))
```
