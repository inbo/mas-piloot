...
```{r}
plot_df %>%
  select(plotnaam, regio, openheid_waarde_1000) %>%
  group_by(regio) %>%
  summarise(min = min(openheid_waarde_1000),
            mean = mean(openheid_waarde_1000),
            max = max(openheid_waarde_1000),
            Q1 = quantile(openheid_waarde_1000, 0.25),
            Q2 = quantile(openheid_waarde_1000, 0.5),
            Q3 = quantile(openheid_waarde_1000, 0.75)) %>%
  kable()
...
plotjaar_df <- mas_data_df %>%
  select(plotnaam, regio, jaar, all_of(plotyear_vars)) %>%
  distinct() %>%
