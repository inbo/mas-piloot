...
plot_brms_fit <- function(model, x, species = "", axis_label = NULL, FUN = mean,
  ...
  if (x %in% cat_vars) {
    ...
    if (length(cat_vars) == 1) {
      out <- out +
        facet_wrap(vars(!!sym(cat_vars)), scales = "free_x")
    } else if (length(cat_vars) > 2) {
      out <- out +
...
for (pred in cont_vars) {
  plot_brms_fit(mod_final_olpropsb_list$Veldleeuwerik, pred,
                interval = c(0, 0.95)) %>%
    print()
}

plot_brms_fit(mod_final_olpropsb_list$Veldleeuwerik, "area_prop_sb_cat",
              interval = c(0, 0.95))

plot_brms_fit(mod_final_olpropsb_list$Veldleeuwerik, "boom",
