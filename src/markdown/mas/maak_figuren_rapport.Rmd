...
df_obs_list <- readRDS(here(out_folder_distance, "df_obs_list.rds"))
veldleeuwerik_df <- df_obs_list$Veldleeuwerik

design <- bind_rows(
  ds_model2022$dht$clusters$bysample %>%
    distinct(Region, Sample) %>%
    mutate(jaar = 2022),
  ds_model2023$dht$clusters$bysample %>%
    distinct(Region, Sample) %>%
    mutate(jaar = 2023)) %>%
  rename("plotnaam" = Sample) %>% 
  separate(Region, into = c("regio", "openheid", "sbp"),
           sep = " - ", remove = TRUE)

plot_densiteit(ds_model2022, spec,
               show_data = TRUE,
               year = 2022:2023,
