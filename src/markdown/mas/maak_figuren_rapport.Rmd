...
```{r}
cont_vars <- mod_final_olpropsb_list$Veldleeuwerik$data %>%
    select(-aantal) %>%
    select(where(is.numeric)) %>%
    names()

for (pred in cont_vars) {
  p <- plot_brms_fit(mod_final_olpropsb_list$Veldleeuwerik, pred,
                interval = c(0, 0.95), show_data = TRUE)
