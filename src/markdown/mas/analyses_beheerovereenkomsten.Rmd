...
```{r}
formula_final1 <- bf(paste("aantal ~", paste0(predictors_final1_refmodel1[1:5], 
                                              collapse = " + ")))
...
We maken een simpel model met 

```{r}
plot_brms_fit <- function(model, x, species = "", axis_label = "",
  ...
  if (x == "sbp") {
    ...
    plot_df <- model$data %>%
      select(-aantal) %>%
      data_grid(sbp = c(unique(model$data$sbp)),
