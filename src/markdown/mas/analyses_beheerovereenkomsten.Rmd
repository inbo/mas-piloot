...
4 of 8?
8 is goed zie ook snelle methode zelfde resultaat ...
...
plot_brms_fit(final, x = "status_teller", show_data = FALSE,
              axis_label = "teller status")
```
...
plot_brms_fit(final, x = "groententeelt_hfdtlt", factor = 100, show_data = FALSE,
                    axis_label = "proportie hoofdteelt groententeelt (%)")
plot_brms_fit(final, x = "zomergranen_hfdtlt2", factor = 100, show_data = FALSE,
              axis_label = "proportie hoofdteelt zomergranen (%)")
plot_brms_fit(final, x = "boom", factor = 100, 
              show_data = FALSE,
              axis_label = "proportie bomen (%)")
plot_brms_fit(final, x = "aardappelen_hfdtlt", show_data = FALSE,
              axis_label = "proportie hoofdteelt aardappelen (%)")
```
...
plot_brms_fit(final, x = "openheid_waarde_1000", show_data = TRUE,
              axis_label = "openheid landschap (1000 m)")
plot_brms_fit(final, x = "groententeelt_hfdtlt", factor = 100, show_data = TRUE,
                    axis_label = "proportie hoofdteelt groententeelt (%)")
plot_brms_fit(final, x = "zomergranen_hfdtlt2", factor = 100, show_data = TRUE,
              axis_label = "proportie hoofdteelt zomergranen (%)")
plot_brms_fit(final, x = "boom", factor = 100, 
              show_data = TRUE,
              axis_label = "proportie bomen (%)")
plot_brms_fit(final, x = "aardappelen_hfdtlt", show_data = TRUE,
              axis_label = "proportie hoofdteelt aardappelen (%)")
```
...
```{r}
create_data_list <- function(doel_soorten_ol_pre, region, year = 2022) {
  df_count_list <- vector("list", length = length(doel_soorten_ol_pre))
  df_count_names <- vector(length = length(doel_soorten_ol_pre))

  for (i in seq_along(doel_soorten_ol_pre)) {
    spec <- sort(doel_soorten_ol_pre)[i]
    ...
    mas_data_df_full <- mas_data_total %>%
      ...
      distinct() %>%
      mutate(jaar.f = factor(jaar),
             zomergranen_hfdtlt2 = rowSums(across(c(graangewassen_hfdtlt, 
                                                    zomergranen_hfdtlt)))) %>%
      select(-c(graangewassen_hfdtlt, zomergranen_hfdtlt))
    ...
    mas_data_df_out <- mas_data_df_agg %>%
      ...
      mutate(area_prop_sb_cat = case_when(
        ...
        area_prop_sb_cat = relevel(factor(area_prop_sb_cat), "nulbeleid"))
...
```{r}
df_count_list <- create_data_list(doel_soorten_ol_pre, "Oostelijke leemstreek")
```
...
# Oostelijke leemstreek 2022: area_prop_sb
...
Voor de totaalanalyse beschouwen we alle plots van 2018-2021 en voegen enkel plots van 2022 toe als ze minstens eenmaal geteld zijn in een van de voorgaande jaren (cf. distance sampling analyses).
`jaar` zal een extra variabele zijn (als factor).
