...
We zijn vooral geïnteresseerd hoe de aantallen zich verhouden tot de verklarende variabelen (en dit per jaar per regio). Dit zijn: `r pred_vars`.
...
Beschouw $Y_{i}$ een observatie $i$ van een aantal broedparen in eenzelfde telcirkel $j$. Deze observaties beschouwen we afhankelijk van de telperiode (dummy variabele $X_{\text{periode_in_jaar}}$), teljaar ($X_{\text{jaar}}$), regio ($X_{\text{regio}}$) en de oppervlakte proportie soortbeschermingsplannen binnen de telcirkel ($X_{\text{area_prop_sb}}$). We voegen een interactie toe tussen regio en proportie sb zodat het effect kan verschillen per regio. We voegen ten slotte een random effect toe van telcirkel $b_{1}$. In een later stadium kunnen we testen of deze laatste noodzakelijk is en/of als er meer complexe correlatiestructuren moeten gespecifieerd worden (bv. autoregressive).
...
if (!file.exists(paste0(model_path, "fit_sbp_gelekwik_pois1"))) {
...
if (!file.exists(paste0(model_path, "fit_sbp_gelekwik_negbin1"))) {
...
if (!file.exists(paste0(model_path, "fit_sbp_gelekwik_ZIP1"))) {
...
if (!file.exists(paste0(model_path, "fit_sbp_gelekwik_ZINB"))) {
  fit_sbp_gelekwik_ZINB <- add_criterion(
    sbp_gelekwik_ZINB, c("kfold"), K = 5, cores = 5,
    folds = "stratified", group = "regio", 
    file = paste0(model_path, "fit_sbp_gelekwik_ZINB"))
} else {
  fit_sbp_gelekwik_ZINB <- readRDS(paste0(model_path, 
                                           "fit_sbp_gelekwik_ZINB.rds"))
}
```

```{r}
models <- c("Poisson", "Zero-inflated Negative binomial", 
...
rbind(fit_sbp_gelekwik_pois1$criteria$kfold$estimates[, "Estimate"],
...
Het Poisson model toont het beste evenwicht tussen model parsimonie en model fit. Dat zien we ook bij de posterior predictive check. We vergelijken de waargenomen tellingen met gesimuleerde tellingen van de posterior predictive distribution.
...
Het Poisson model kan de data goed beschrijven. We kijken nu welke random effecten we best toevoegen.

### Modellen fitten (1)

We fitten vier modellen met Poisson verdeling. Het fixed effect gedeelte is hetzelfde als voordien:
...
We beschouwen vier mogelijkheden voor de random effects:

1.  Geen random effecten
2.  Random intercept per telcirkel
3.  Autoregressive per jaar (orde 1)
4.  Random intercept en autoregressive (orde 1)
5.  Random intercept en random slope per telcirkel voor jaar effect (verwijder jaar effect uit fixed effects)
...
sbp_gelekwik_pois2 <- 
  brm(formula = bf(aantal ~ periode_in_jaar + jaar.f + regio*area_prop_sb,
                   autocor = ~ ar(time = jaar, gr = ar_id, p = 1)),
...
# 4. Random intercept en autoregressive
sbp_gelekwik_pois3 <- 
  brm(formula = bf(aantal ~ periode_in_jaar + jaar.f + regio*area_prop_sb + 
                   (1 | plotnaam),
                   autocor = ~ ar(jaar, gr = ar_id, p = 1)),
      data = gele_kwik_df_modAR, 
      family = poisson(), 
      chains = nchains, 
      warmup = burnin, 
      ...
      cores = nparallel, 
      backend = "cmdstanr",
      file = paste0(model_path, "sbp_gelekwik_pois3"),
      file_refit = "on_change")

summary(sbp_gelekwik_pois3)
plot(sbp_gelekwik_pois3)

# 5. Random intercept en random slope per telcirkel voor jaar effect
sbp_gelekwik_pois4 <- 
  brm(formula = bf(aantal ~ periode_in_jaar + regio*area_prop_sb + 
                   (1 + jaar | plotnaam)),
      data = gele_kwik_df_mod, 
      family = poisson(), 
      chains = nchains, 
      ...
      cores = nparallel, 
      backend = "cmdstanr",
      file = paste0(model_path, "sbp_gelekwik_pois4"),
      file_refit = "on_change")

summary(sbp_gelekwik_pois4)
plot(sbp_gelekwik_pois4)
```
...
```{r}
# Perform K-fold cross-validation
if (!file.exists(paste0(model_path, "fit_sbp_gelekwik_pois2"))) {
...
if (!file.exists(paste0(model_path, "fit_sbp_gelekwik_pois3"))) {
  fit_sbp_gelekwik_pois3 <- add_criterion(
    sbp_gelekwik_pois3, c("kfold"), K = 5, cores = 5,
    folds = "stratified", group = "regio", 
    file = paste0(model_path, "fit_sbp_gelekwik_pois3"))
} else {
  fit_sbp_gelekwik_pois3 <- readRDS(paste0(model_path, 
                                           "fit_sbp_gelekwik_pois3.rds"))
}
if (!file.exists(paste0(model_path, "fit_sbp_gelekwik_pois4"))) {
  fit_sbp_gelekwik_pois4 <- add_criterion(
    sbp_gelekwik_pois4, c("kfold"), K = 5, cores = 5,
    folds = "stratified", group = "regio", 
    file = paste0(model_path, "fit_sbp_gelekwik_pois4"))
} else {
  fit_sbp_gelekwik_pois4 <- readRDS(paste0(model_path, 
                                           "fit_sbp_gelekwik_pois4.rds"))
}
```
