...
```{r}
plot_brms_fit <- function(model, x, species = "", axis_label = "", FUN = mean,
                          factor = 1, n = 100, show_data = TRUE) {
...
```{r}
test_geelgors <- brm(formula = bf(aantal ~ sbp + 
                       meerjarige_fruitteelt_hfdtlt + 
                       houtkant + 
                       n_predators2 + 
                       landbouwinfrastructuur_hfdtlt + 
...
test_geelgors
conditional_effects(test_geelgors)
```
