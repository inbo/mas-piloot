...
```{r}
posterior_sample_r2 <- bayes_R2(ref_gelekwik_model1, summary = FALSE)
...
indices <- which(names(data_projpred) %in% c("id", "aantal", "plotnaam", 
                                             ...
                                             "x_plot", "y_plot",
                                             "openheid_klasse_300"))
predictors_refmodel1 <- names(data_projpred)[-indices]

forms <- get_search_terms(n_predictors_max = 3, predictors = predictors_refmodel1, 
                          keep_var = "sbp")
```

```{r}
# Fast varsel for finding a suitable value for argument nterms_max
cvvs_fast <- cv_varsel(ref_gelekwik_model1,
                       # for speed, better is "forward"
                       method = "L1",
                       nclusters_pred = 20, # speed
                       cv_method = "LOO",
                       nterms_max = length(predictors_refmodel1),
...
```{r}
cvvs <- cv_varsel(ref_gelekwik_model1,
                  method = "forward",
                  cv_method = "LOO",
                  nterms_max = 16,
