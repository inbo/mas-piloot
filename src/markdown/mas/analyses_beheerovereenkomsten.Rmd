...
Voor de regressiecoëfficienten ($\beta$-parameters) gebruiken we de 'R2-D2 shrinkage prior' [@zhang2016; @yanchenko2021]. Voor het intercept gebruiken we de basis priors van **brms**.
...
```{r}
posterior_sample_b0 <- as_draws_df(test, "b_Intercept")
...
```{r}
prior_check_samples <- prior_draws(test)
posterior_sample_r2 <- bayes_R2(test, summary = FALSE)
...
plot(cvvs, stats = "mlpd", deltas = TRUE, ranking_nterms_max = NA)
```
