...
Er zijn 3 tellingen in De Moeren van 2022 die op eenzelfde dag zijn geteld door 2 verschillende personen.

-  DM_137.2 op 2022-06-03 door JJNN16 en HMLE02
-  DM_18 op 2022-06-03 door JJNN16 en HMLE02
-  DM_39.22 op 2022-04-28 door JJNN16 en ATRR00

Voorlopig passen we we de variabele `teller_status` manueel aan naar 'professioneel' voor deze tellingen (we gaan ervan uit dat het een opleidingsmoment was).

> aan Johannes vragen

```{r}
mas_data_df <- mas_data_df %>%
  mutate(status_teller = case_when(
    plotnaam == "DM_137.2" & datum == "2022-06-03" ~ "professioneel",
    plotnaam == "DM_18" & datum == "2022-06-03" ~ "professioneel",
    plotnaam == "DM_39.22" & datum == "2022-04-28" ~ "professioneel",
    TRUE ~ status_teller))
```
...
library(tidybayes)
draws <- gele_kwik_df %>%
  distinct(plotnaam, regio, periode_in_jaar, jaar.f) %>%
  add_epred_draws(brmpois1)
  
draws %>%
  ggplot(aes(x = periode_in_jaar, y = .epred, colour = jaar.f)) +
  stat_halfeye() +
  facet_grid(regio~jaar.f)
```
