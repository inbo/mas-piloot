...
```{r}
tibble(beleid = c("nul", "laag", "middel", "hoog"),
       prop_area_sb = c("0", "<0.04", "0.04-0.1", ">0.1")) %>%
  kable()
```
...
plot_brms_fit <- function(model, x, species = "", axis_label = NULL, FUN = mean,
  ...
  if (x %in% cat_vars) {
    ...
    if (length(cat_vars)  == 2) {
      ...
      stop("Not implemented for more than 3 categorical predictors!", .call = FALSE)
    }
    ...
    if (length(cat_vars) == 1) {
      ...
      stop("Not implemented for more than 2 categorical predictors!", .call = FALSE)
    }
  }
  return(out)
}
