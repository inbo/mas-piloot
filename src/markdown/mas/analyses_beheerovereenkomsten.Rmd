...
Onze doelsoorten zijn Veldleeuwerik, Gele kwikstaart, Geelgors, Kievit, Grasmus, Ringmus, Witte kwikstaart, Patrijs, Haas.
Voor Patrijs en Ringmus hebben we bijna geen data.
We zullen deze soorten voorlopig niet opnemen in de analyses.

```{r}
doel_soorten <- c("Veldleeuwerik", 
                  "Gele Kwikstaart", 
                  "Geelgors", 
                  "Kievit", 
                  "Grasmus",
...
```{r}
create_data_list <- function(doel_soorten, region, year = 2022) {
  df_count_list <- vector("list", length = length(doel_soorten))
  df_count_names <- vector(length = length(doel_soorten))

  for (i in seq_along(doel_soorten)) {
    spec <- sort(doel_soorten)[i]
...
```{r}
df_count_list <- create_data_list(doel_soorten, "Oostelijke leemstreek")
```
...
```{r}
# Formula
indices_to_remove <- which(names(df_count_list$GeleKwikstaart) %in% c(
  "id", 
  "aantal", 
  "plotnaam", 
  "regio",
...
predictors_olsbp <- names(df_count_list$GeleKwikstaart)[-indices_to_remove]
