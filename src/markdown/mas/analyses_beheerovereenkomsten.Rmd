...
```{r}
create_data_list <- function(doel_soorten, region) {
...
# Oostelijke leemstreek 2022: totaal

Nu fitten we alle predictors zonder bepaalde restricties.

```{r}
# Formula
indices_to_remove3 <- which(names(df_count_list$GeleKwikstaart) %in% c(
  "id", 
  "aantal", 
  "plotnaam", 
  "regio",
...
predictors_oltotal <- names(df_count_list$GeleKwikstaart)[-indices_to_remove3]
...
```{r}
refmod_oltotal_list <- vector("list", length = length(df_count_list_scaled))
refmod_oltotal_names <- vector(length = length(df_count_list_scaled))

for (i in seq_along(df_count_list_scaled)) {
  spec <- names(df_count_list_scaled)[i]
  name <- paste0("refmod_oltotal_", spec)
  print(paste("Busy fitting", name, "..."))
  assign(name,
         brm(formula = form_refmod_oltotal,
             data = df_count_list_scaled[[i]], 
               family = poisson(),
               prior = R2D2_prior,
               chains = nchains, 
  ...
  refmod_oltotal_list[[i]] <- get(name)
  refmod_oltotal_names[i] <- spec
}

names(refmod_oltotal_list) <- refmod_oltotal_names
```

Niet altijd een goede fit:

```{r}
for (i in seq_along(refmod_oltotal_list)) {
  title <- paste("Fit referentiemodel", names(refmod_oltotal_list)[i])
  ref_mod <- refmod_oltotal_list[[i]]
  print(pp_check(ref_mod, type = "bars", ndraws = 200) +
          ggtitle(title))
}
```

Deze keer specificeren we niet dat bepaalde variabelen zeker in het model moeten aanwezig zijn.

```{r}
cvvs_oltotal_list <- vector("list", length = length(refmod_oltotal_list))
cvvs_oltotal_names <- vector(length = length(refmod_oltotal_list))

for (i in seq_along(refmod_oltotal_list)) {
  spec <- names(refmod_oltotal_list)[i]
  name <- paste0("cvvs_oltotal_", spec)
  print(paste("Busy with", name, "..."))
  ...
  cvvs_oltotal_list[[i]] <- get(name)
  cvvs_oltotal_names[i] <- spec
}

names(cvvs_oltotal_list) <- cvvs_oltotal_names
```

```{r}
for (i in seq_along(cvvs_oltotal_list)) {
  title <- names(cvvs_oltotal_list)[i]
  if (!title == "FALSE") {
    print(plot(cvvs_oltotal_list[[i]], stats = "mlpd", deltas = TRUE, 
               ranking_abbreviate = TRUE, 
               ranking_abbreviate_args = list(minlength = 4)) +
             ggtitle(title))
    
    smmry <- summary(cvvs_oltotal_list[[i]], stats = "mlpd", 
                 type = c("mean", "lower", "upper"),
                 deltas = TRUE)
    print(smmry, digits = 1)
    
    rk <- ranking(cvvs_oltotal_list[[i]])
    pr_rk <- cv_proportions(rk)
