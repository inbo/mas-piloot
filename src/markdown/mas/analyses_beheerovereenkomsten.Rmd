...
```{r}
plot_brms_fit <- function(model, x, axis_label = "",
                          factor = 1, n = 100, show_data = TRUE) {
  if (x == "sbp") {
    plot_df <- data_projpred %>%
      select(all_of(predictors_final2_refmodel1)) %>%
    ...
    out <- plot_df %>%
      ggplot(aes(x = paste(.data[[x]], x), y = aantal, fill = .data[[x]]))
    
    if (isTRUE(show_data)) {
      out <- out +
        geom_jitter(data = data_projpred, colour = "firebrick", alpha = 0.3,
                    width = 0.2, height = 0)
    } else {
      out <- out +
    ...
    plot_df <- data_projpred %>%
      data_grid(sbp = c(unique(data_projpred$sbp)),
                haag = ifelse(rep(x == "haag", n), 
                              seq_range(haag,
                                        n = n),
                              mean(haag)),
                houtkant = ifelse(rep(x == "houtkant", n), 
                                  seq_range(houtkant,
                                            n = n), 
                                  mean(houtkant)),
                wintergranen_hfdtlt = ifelse(rep(x == "wintergranen_hfdtlt", 
                                                 n), 
                                             seq_range(wintergranen_hfdtlt,
                                                       n = n), 
                                             mean(wintergranen_hfdtlt)),
                openheid_waarde_1000 = ifelse(rep(x == "openheid_waarde_1000", 
                                                  n), 
                                              seq_range(openheid_waarde_1000, 
                                                        n = n), 
                                              mean(openheid_waarde_1000))) %>%
        add_epred_draws(model)
    ...
    if (isTRUE(show_data)) {
      out <- out +
        geom_point(data = data_projpred, colour = "firebrick", alpha = 0.3) 
    } else {
      out <- out +
        scale_y_continuous(limits = c(0, NA))
    }
...
```{r}
p1 <- plot_brms_fit(final, x = "haag", factor = 100, show_data = FALSE,
                    axis_label = "proportie hagen (%)") +
  theme(axis.title = element_text(size = 12)) +
  coord_cartesian(ylim = c(0, 1.6))
p2 <- plot_brms_fit(final, x = "houtkant", factor = 100, show_data = FALSE,
  ...
  coord_cartesian(ylim = c(0, 1.6))
p3 <- plot_brms_fit(final, x = "wintergranen_hfdtlt", factor = 100, 
                    show_data = FALSE,
                    axis_label = "proportie hoofdteelt wintergranen (%)") +
  theme(axis.title = element_text(size = 12)) +
  coord_cartesian(ylim = c(0, 1.6))
p4 <- plot_brms_fit(final, x = "openheid_waarde_1000", show_data = FALSE,
...
```{r}
p5 <- plot_brms_fit(final, x = "haag", factor = 100,
                    axis_label = "proportie hagen (%)") +
  theme(axis.title = element_text(size = 12)) +
  coord_cartesian(ylim = c(0, 6))
p6 <- plot_brms_fit(final, x = "houtkant", factor = 100,
                    axis_label = "proportie houtkanten (%)") +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank(),
        axis.title = element_text(size = 12)) +
  coord_cartesian(ylim = c(0, 6))
p7 <- plot_brms_fit(final, x = "wintergranen_hfdtlt", factor = 100,
                    axis_label = "proportie hoofdteelt wintergranen (%)") +
  theme(axis.title = element_text(size = 12)) +
  coord_cartesian(ylim = c(0, 6))
p8 <- plot_brms_fit(final, x = "openheid_waarde_1000", 
                    axis_label = "openheid landschap (1000 m)") +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        axis.title.y = element_blank(),
        axis.title = element_text(size = 12)) +
  coord_cartesian(ylim = c(0, 6))
ggarrange(p5, p6, p7, p8, common.legend = TRUE, legend = "right")
```


Conclusie:
- geef dataframe door aan functie wegens onzekerheid welke variabelen
- projpred met sbp, met area_prop_sb_cat en met alles (algemeen effect)
  - voor OL en DM
...
Nu fitten we alle predictors zonder bepaalde restricties. We gebruiken de default priors.
