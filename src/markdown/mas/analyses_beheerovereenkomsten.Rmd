...
```{r}
search_terms_olsbp <- get_search_terms(
  n_predictors_max = 2, 
  predictors = predictors_olsbp[-match("sbp", predictors_olsbp)],
  keep_var = "sbp")
```

```{r}
cvvs_olsbp_list <- vector("list", length = length(refmod_olsbp_list))
cvvs_olsbp_names <- vector(length = length(refmod_olsbp_list))

for (i in seq_along(refmod_olsbp_list)) {
  spec <- names(refmod_olsbp_list)[i]
  name <- paste0("cvvs_olsbp_", spec)
  ...
  if (!file.exists(cvvs_file)) {
    assign(name, 
           cv_varsel(
             object          = refmod_olsbp_list[[i]],
             method          = "forward",
             cv_method       = "LOO",
             search_terms    = search_terms_olsbp,
             validate_search = TRUE))
  ...
  cvvs_olsbp_list[[i]] <- get(name)
  cvvs_olsbp_names[i] <- spec
}

names(cvvs_olsbp_list) <- cvvs_olsbp_names
```

```{r}
for (i in seq_along(cvvs_olsbp_list)) {
  title <- names(cvvs_olsbp_list)[i]
  if (!title == "FALSE") {
    print(plot(cvvs_olsbp_list[[i]], stats = "mlpd", deltas = TRUE, 
               ranking_abbreviate = TRUE, 
               ranking_abbreviate_args = list(minlength = 4)) +
             ggtitle(title))
    
    smmry <- summary(cvvs_olsbp_list[[i]], stats = "mlpd", 
                 type = c("mean", "lower", "upper"),
                 deltas = TRUE)
    print(smmry, digits = 1)
    
    rk <- ranking(cvvs_olsbp_list[[i]])
...
```{r}
size_decided_olsbp_list <- list(8, 5, 8, 2, 10, 9, 3)
size_decided_olsbp_list
```
