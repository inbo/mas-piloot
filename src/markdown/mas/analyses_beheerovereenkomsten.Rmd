...
```{r}
smmry_refmodel1 <- summary(cvvs_refmodel1, stats = "mlpd", type = c("mean", "lower", "upper"),
                 deltas = TRUE)
...
```{r}
size_decided2_refmodel1 <- 8
predictors_final2_refmodel1 <- head(rk_refmodel1[["fulldata"]], 
                                   size_decided2_refmodel1)
predictors_final2_refmodel1
```
...
```{r}
# voorlopig
predictors_final2_refmodel1 <- c("sbp", "haag", "houtkant", 
                                 "wintergranen_hfdtlt", 
                                 "openheid_waarde_1000")
prj_refmodel1 <- project(ref_gelekwik_model2,
                         solution_terms = predictors_final2_refmodel1)
...
```{r}
formula_final1 <- bf(paste("aantal ~", paste0(predictors_final2_refmodel1, 
                                              collapse = " + ")))

data_projpred_centered <- data_projpred %>%
  mutate(across(.cols = c(where(is.numeric), -aantal), 
...
p2 <- plot_brms_fit(final, x = "houtkant", factor = 100, show_data = FALSE,
  ...
  coord_cartesian(ylim = c(0, 1.6))
p3 <- plot_brms_fit(final, x = "wintergranen_hfdtlt", factor = 100, show_data = FALSE,
