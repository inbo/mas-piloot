...
conflicted::conflicts_prefer(projpred::project)
...
Er moet een geschikte afweging worden gemaakt tussen voorspellende prestaties (nauwkeurigheid) en modelgrootte.
...
```{r}
size_decided_olpropsb_new_list <- list(8, 5, 5, NA, 12, 14, 19, 8, 9, 5)
...
```{r}
size_decided_dmpropsb_new_list <- list(13, 9, 7, 10, 6, 6, 7, 9, 2, 4)
...
```{r}
size_decided_ol_total_list <- list(12, NA, NA, 3, NA, 10, 5, 3, 10, 8)
...
```{r}
size_decided_dm_total_list <- list(10)
...
for (i in seq_along(cvvs_olpropsb_new_list)) {
  ...
  if (is.na(mod_size)) {
    ...
    if ("jaar.f" %in% preds) {
      predictors_olpropsb_new_list[[i]]$formula <- bf(
        paste("aantal", "~", paste(preds, collapse = " + "), " + (1|plotnaam)")
        )
    } else {
      predictors_olpropsb_new_list[[i]]$formula <- bf(
        paste("aantal", "~", paste(preds, collapse = " + "))
        )
    }
  }
...
for (i in seq_along(cvvs_ol_total_list)) {
  ...
  if (is.na(mod_size)) {
    ...
    if ("jaar.f" %in% preds) {
      predictors_ol_total_list[[i]]$formula <- bf(
        paste("aantal", "~", paste(preds, collapse = " + "), " + (1|plotnaam)")
        )
    } else {
...
mod_final_oltotal_list <- list(
  mod_final_pois_oltotal_list$Geelgors,
  mod_final_zip_oltotal_list$Haas,
  mod_final_pois_oltotal_list$Kneu,
  mod_final_pois_oltotal_list$Kwartel,
  mod_final_pois_oltotal_list$Torenvalk,
...
for (i in seq_along(cvvs_dmpropsb_new_list)) {
  ...
  if (is.na(mod_size)) {
    ...
    if ("jaar.f" %in% preds) {
      predictors_dmpropsb_new_list[[i]]$formula <- bf(
        paste("aantal", "~", paste(preds, collapse = " + "), " + (1|plotnaam)")
        )
    } else {
      predictors_dmpropsb_new_list[[i]]$formula <- bf(
...
mod_final_dmpropsb_list <- list(
  ...
  mod_final_zip_dmpropsb_list$Haas,
  mod_final_pois_dmpropsb_list$Kievit,
  mod_final_pois_dmpropsb_list$Kneu,
  mod_final_pois_dmpropsb_list$Veldleeuwerik
  )
names(mod_final_dmpropsb_list) <- c("GeleKwikstaart", "Haas", "Kievit", "Kneu",
                                    "Veldleeuwerik")
...
for (i in seq_along(cvvs_dm_total_list)) {
  ...
  if (is.na(mod_size)) {
    ...
    if ("jaar.f" %in% preds) {
      predictors_dm_total_list[[i]]$formula <- bf(
        paste("aantal", "~", paste(preds, collapse = " + "), " + (1|plotnaam)")
        )
    } else {
      predictors_dm_total_list[[i]]$formula <- bf(
...
Resultaten Veldleeuwerik 2022-2023.
We willen figuren voor de continue variabelen.

```{r}
cont_vars_ol <- mod_final_olpropsb_list$Veldleeuwerik$data %>%
    select(-aantal) %>%
    select(where(is.numeric)) %>%
    names()
cont_vars_dm <- mod_final_dmpropsb_list$Veldleeuwerik$data %>%
    select(-aantal) %>%
    select(where(is.numeric)) %>%
    names()
```

Continue variabelen in de Oostelijke leemstreek: `r cont_vars_ol`
Continue variabelen in De Moeren: `r cont_vars_dm`

```{r}
label_list <- list(
  "openheid_waarde_1000" = list(
    "label" = "openheid landschap waarde (1000 m)",
    "factor" = 1),
  "haag" = list(
    "label" = "oppervlakte haag binnen telcirkel (%)",
    "factor" = 100),
  "houtkant" = list(
    "label" = "oppervlakte houtkant binnen telcirkel (%)",
    "factor" = 100),
  "bomenrij" = list(
    "label" = "oppervlakte bomenrij binnen telcirkel (%)",
    "factor" = 100),
  "meerjarige_fruitteelt_hfdtlt" = list(
    "label" = paste("oppervlakte hoofdteelt meerjarige fruitteelt",
                    "binnen telcirkel (%)", sep = "\n"),
    "factor" = 100),
  "grasland_hfdtlt_pre" = list(
    "label" = paste("oppervlakte hoofdteelt grasland (jaar ervoor)",
                    "binnen telcirkel (%)", sep = "\n"),
    "factor" = 100),
  "bosrand" = list(
    "label" = "oppervlakte bosrand binnen telcirkel (%)",
    "factor" = 100),
  "boom" = list(
    "label" = "oppervlakte boom binnen telcirkel (%)",
    "factor" = 100)
)

for (pred in cont_vars_ol) {
  p <- plot_brms_fit(
    mod_final_olpropsb_list$Veldleeuwerik,
    pred,
    species = "Veldleeuwerik",
    axis_label = label_list[[pred]]$label,
    factor = label_list[[pred]]$factor,
    interval = c(0, 0.95),
    show_data = TRUE,
    scale_data = df_count_list_olnew$Veldleeuwerik)
  file <- paste(pred, "veldleeuwerik_data.pdf", sep = "_")
...
for (pred in cont_vars_dm) {
  p <- plot_brms_fit(
    mod_final_dmpropsb_list$Veldleeuwerik,
    pred,
    species = "Veldleeuwerik",
    axis_label = label_list[[pred]]$label,
    factor = label_list[[pred]]$factor,
    interval = c(0, 0.95),
    show_data = TRUE,
    scale_data = df_count_list_dmnew$Veldleeuwerik)
  file <- paste(pred, "veldleeuwerik_data.pdf", sep = "_")
  ggsave(here("media", "beheerovereenkomsten", "de_moeren", file),
         p,
         dpi = 300,
         width = 8,
         height = 6)
...
mean en sd voor ol en dm cont vars

```{r}
cont_vars_total <- unique(c(cont_vars_ol, cont_vars_dm))

q25_f <- function(x) quantile(x, 0.25)
q75_f <- function(x) quantile(x, 0.75)

bind_rows(
  df_count_list_olnew$Veldleeuwerik %>%
    select(regio, all_of(cont_vars_ol)),
  df_count_list_dmnew$Veldleeuwerik %>%
    select(regio, all_of(cont_vars_dm))
  ) %>%
  pivot_longer(cols = -"regio", names_to = "naam", values_to = "waarde") %>%
  drop_na() %>%
  group_by(regio, naam) %>%
  summarise(across(where(is.numeric), list(gemiddelde = mean,
                   ...
                   .names = "{.fn}"),
            .groups = "drop") %>%
  select(naam, regio, everything()) %>%
  write_csv(here("media", "beheerovereenkomsten", "data_stats_veldleeuwerik.csv"))
```
