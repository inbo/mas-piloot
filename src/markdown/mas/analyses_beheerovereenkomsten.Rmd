...
# Priors
beta_prior_projpred <- set_prior(R2D2(mean_R2 = 0.3, prec_R2 = 5, cons_D2 = 10, 
                                      autoscale = TRUE))

# Formula
indices <- which(names(data_projpred) %in% c("id", "aantal", "plotnaam", 
                                             "regio", "jaar", "jaar.f", 
                                             "area_prop_sb", 
                                             "area_prop_bo_overig",
                                             "area_prop_sb_cat",
                                             "is_sbp", "openheid_klasse_300"))
predictor_names <- names(data_projpred)[-indices]
form_projpred <- bf(paste("aantal", "~", paste(predictor_names, 
                                               collapse = " + ")))
form_projpred
```

We centreren en schalen alle continue variabelen.

```{r}
ref_model1 <- 
  brm(formula = form_projpred,
      data = data_projpred %>%
        mutate(across(.cols = c(where(is.numeric), -aantal), 
                      .fns = ~ as.numeric(scale(.x)))), 
      family = poisson(),
      prior = beta_prior_projpred,
      chains = nchains, 
      warmup = burnin, 
      iter = niter,
      cores = nparallel,
      ...
      sample_prior = TRUE,
      backend = "cmdstanr",
      file = paste0(model_path, "ref_model1"),
      file_refit = "on_change")
```
...
indices <- which(names(data_projpred) %in% c("id", "aantal", "plotnaam", 
                                             ...
                                             "openheid_klasse_300"))
predictor_names <- names(data_projpred)[-indices]
form_projpred2 <- bf(paste("aantal", "~", paste(predictor_names, 
                                               collapse = "*sbp + ")))
test_model1 <- 
  brm(formula = form_projpred2,
      data = data_projpred %>%
        mutate(across(.cols = c(where(is.numeric), -aantal), 
                      .fns = ~ as.numeric(scale(.x)))), 
      family = poisson(),
...
cvvs_fast_test1 <- cv_varsel(test_model1,
                       # L1 not possible with interactions
                       method = "forward",
                       nclusters_pred = 20, # speed
                       cv_method = "LOO",
                       nterms_max = length(predictor_names)*2,
                       # only for the sake of speed
                       validate_search = FALSE)
plot(cvvs_fast_test1, stats = "mlpd", ranking_nterms_max = NA)
```

Ok. Wel forward nodig ipv L1 search.

Wat met smoothers?
geen R2D2 of horshoe prior mogelijk?
Support for additive models is still experimental.
...
indices <- which(names(data_projpred) %in% c("id", "aantal", "plotnaam", 
                                             ...
                                             "is_sbp", "openheid_klasse_300"))
predictor_names <- names(data_projpred)[-indices]
form_projpred3 <- bf(gsub("((x|y)_plot)", "s(\\1)", 
                         paste("aantal", "~", paste(predictor_names, 
                               collapse = " + "))))

test_model2 <- 
  brm(formula = form_projpred3,
      data = data_projpred %>%
        mutate(across(.cols = c(where(is.numeric), -aantal), 
                      .fns = ~ as.numeric(scale(.x)))), 
      family = poisson(),
...
cvvs_fast_test2 <- cv_varsel(test_model2,
                       # L1 not possible with smoothing terms
                       method = "forward",
                       nclusters_pred = 20, # speed
                       cv_method = "LOO",
                       nterms_max = length(predictor_names),
                       # only for the sake of speed
                       validate_search = FALSE)
plot(cvvs_fast_test2, stats = "mlpd", ranking_nterms_max = NA)
```


```{r}
D <- length(predictor_names)
...
test <- set_prior(horseshoe(df = 100, scale_global = tau0, scale_slab = 1))
```

```{r}
match_variables <- predictor_names
...
# Behoud enkel de combinaties die sbp bevatten
forms1 <- forms1[grep("sbp", forms1)]
```
...
Keuze van priors lijkt ok.

```{r}
prior_check_samples <- prior_draws(ref_model1)
posterior_sample_b0 <- as_draws_df(ref_model1, "b_Intercept")
df_b0 <- data.frame(posterior = posterior_sample_b0$b_Intercept,
                    prior = prior_check_samples$Intercept) %>%
  pivot_longer(c("posterior", "prior"), 
               names_to = "var", values_to = "intercept")

ggplot(df_b0, aes(x = intercept, colour = var)) +
  geom_density()
```

```{r}
posterior_sample_r2 <- bayes_R2(ref_model1, summary = FALSE)
df_r2 <- data.frame(posterior = posterior_sample_r2[,1],
                    prior = prior_check_samples$R2D2_R2) %>%
  pivot_longer(c("posterior", "prior"), 
               names_to = "var", values_to = "R2")

ggplot(df_r2, aes(x = R2, colour = var)) +
  geom_density()
```
...
```{r}
pp_check(ref_model1, type = "bars", ndraws = 200)
```
...
```{r}
# Fast varsel for finding a suitable value for argument nterms_max
cvvs_fast <- cv_varsel(ref_model1,
                       # for speed, better is "forward"
                       method = "L1",
                       nclusters_pred = 20, # speed
                       cv_method = "LOO",
                       nterms_max = length(predictor_names),
...
We zien dat de "mean log predictive density" (MLPD) van de submodellen niet hard meer verandert vanaf modellen groter met meer dan acht predictors.
Nu doen we een grondige forward search die meer tijd nodig heeft met `validate_search = TRUE` (beschermt tegen overfitting).
Om op veilig te spelen zetten we  `nterms_max = 16`.
We maken dezelfde plot als voordien maar nu als verschil met ons referentiemodel (`deltas = TRUE`).

```{r}
cvvs <- cv_varsel(ref_model1,
                  method = "forward",
                  cv_method = "LOO",
                  nterms_max = 16,
                  validate_search = TRUE)
plot(cvvs, stats = "mlpd", deltas = TRUE, ranking_abbreviate = TRUE, 
