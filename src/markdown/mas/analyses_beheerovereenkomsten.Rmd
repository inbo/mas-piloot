...
```{r}
prior_check_samples <- prior_draws(ref_gelekwik_model1)
posterior_sample_b0 <- as_draws_df(ref_gelekwik_model1, "b_Intercept")
...
```{r}
prior_check_samples <- prior_draws(ref_gelekwik_model2)
posterior_sample_r2 <- bayes_R2(ref_gelekwik_model2, summary = FALSE)
...
```{r}
prior_check_samples <- prior_draws(ref_gelekwik_model3)
posterior_sample_r2 <- bayes_R2(ref_gelekwik_model3, summary = FALSE)
df_r2 <- data.frame(posterior = posterior_sample_r2[,1],
                    prior = prior_check_samples$R2D2_R2) %>%
  pivot_longer(c("posterior", "prior"), 
               names_to = "var", values_to = "R2")
...
```{r}
prior_check_samples <- prior_draws(ref_gelekwik_model4)
posterior_sample_r2 <- bayes_R2(ref_gelekwik_model4, summary = FALSE)
df_r2 <- data.frame(posterior = posterior_sample_r2[,1],
                    prior = prior_check_samples$R2D2_R2) %>%
  pivot_longer(c("posterior", "prior"), 
               names_to = "var", values_to = "R2")
...
```{r}
prior_check_samples <- prior_draws(ref_gelekwik_model5)
posterior_sample_r2 <- bayes_R2(ref_gelekwik_model5, summary = FALSE)
df_r2 <- data.frame(posterior = posterior_sample_r2[,1],
                    prior = prior_check_samples$R2D2_R2) %>%
  pivot_longer(c("posterior", "prior"), 
               names_to = "var", values_to = "R2")
...
```{r}
pp_check(ref_gelekwik_model3, type = "bars", ndraws = 200)
```
...
```{r}
search_terms_file <- paste0(cache_path, "search_terms_refmodel1.rds")
if (!file.exists(search_terms_file)) {
  search_terms_refmodel1 <- get_search_terms(
    n_predictors_max = 10, 
    predictors = predictors_refmodel1[-match("sbp", predictors_refmodel1)],
    keep_var = "sbp")
  saveRDS(search_terms_refmodel1, search_terms_file)
} else {
  search_terms_refmodel1 <- readRDS(search_terms_file)
}
```
...
cvvs_fast_test <- cv_varsel(ref_gelekwik_model1,
                            method = "forward",
...
plot(cvvs_fast_test, stats = "mlpd", ranking_nterms_max = NA)
```

```{r}
# Fast varsel for first exploration
cvvs_fast <- cv_varsel(ref_gelekwik_model2,
                       method = "forward",
                       nclusters_pred = 20, # speed
                       cv_method = "LOO",
                       search_terms = search_terms_refmodel1,
