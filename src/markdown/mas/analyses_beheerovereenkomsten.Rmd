...
plot_brms_fit(final, x = "bosrand", factor = 100,
              show_data = FALSE,
              axis_label = "proportie bosranden (%)")
...
```{r}
size_decided_olsbp_new_list <- list(8, 5, 8, 2, 10, 9, 3)
...
```{r}
size_decided_olpropsb_new_list <- list(8, 5, 8, 2, 10, 9, 3)
...
```{r}
size_decided_dmsbp_new_list <- list(8, 5, 8, 2, 10, 9, 3)
...
```{r}
size_decided_dmpropsb_new_list <- list(8, 5, 8, 2, 10, 9, 3)
...
```{r}
size_decided_ol_total_list <- list(8, 5, 8, 2, 10, 9, 3)
...
```{r}
predictors_olsbp_list <- vector("list", length = length(cvvs_olsbp_list))
predictors_olsbp_names <- vector(length = length(cvvs_olsbp_list))

for (i in seq_along(cvvs_olsbp_list)) {
  cvvs <- cvvs_olsbp_list[[i]]
  spec <- names(cvvs_olsbp_list)[i]
  
  rk <- ranking(cvvs)
  preds <- head(rk[["fulldata"]], size_decided_olsbp_list[[i]])
  
  predictors_olsbp_list[[i]]$predictors <- preds
  predictors_olsbp_list[[i]]$formula <- bf(paste("aantal", "~", 
                                                 paste(preds, 
                                                       collapse = " + ")))
  predictors_olsbp_names[i] <- spec
}
names(predictors_olsbp_list) <- predictors_olsbp_names
```
