...
```{r, eval=FALSE}
size_decided_dm_total_list <- list(8, 5, 8, 2, 10, 9, 3)
  ...
  out <- df %>%
    mutate(
      trend_fine = case_when(
          effect_fine == "++" ~ "strong positief effect",
          effect_fine == "+" ~ "positief effect",
          effect_fine == "+~" ~ "gematigd positief effect",
          effect_fine == "~" ~ "stabiel",
          effect_fine == "-" ~ "negatief effect",
          effect_fine == "-~" ~ "gematigd negatief effect",
          effect_fine == "--" ~ "sterk negatief effect",
          TRUE ~ "onzeker",
      ),
      trend_fine = factor(trend_fine,
        levels = c(
          "strong positief effect",
          "positief effect",
          "gematigd positief effect",
          "stabiel",
          "gematigd negatief effect",
          "negatief effect",
    ...
    mutate(
      trend_coarse = case_when(
          effect_coarse == "+" ~ "positief effect",
          effect_coarse == "-" ~ "negatief effect",
          effect_coarse == "~" ~ "stabiel",
          TRUE ~ "onzeker",
      ),
      trend_coarse = factor(trend_coarse,
        levels = c("positief effect",
                   "stabiel",
                   "negatief effect",
                   "onzeker"),
        ordered = TRUE)
    ...
    mutate(
      trend_simple = case_when(
        lcl > ref ~ "positief effect",
        ucl < ref ~ "negatief effect",
        TRUE ~ "stabiel of onzeker effect"
        ),
      trend_simple = factor(trend_simple,
        levels = c("positief effect",
                   "negatief effect",
                   "stabiel of onzeker effect"),
        ordered = TRUE)
    ) %>%
  ...
  p <- draws_olpropsb_list[[spec]] %>%
    ...
    ggplot(aes(x = label)) +
      ...
      scale_fill_manual(values =  c("chartreuse3",
                                    ...
                                    "skyblue"),
                        drop = FALSE) +
      labs(y = "Procentuele wijziging t.o.v. nulbeleid",
           x = "",
           title = spec,
           fill = "Trend") +
      scale_y_continuous(
        ...
        sec.axis = sec_axis(~ exp(.),
          ...
          name = "Relatieve wijziging t.o.v. nulbeleid")
        ) +
      theme(
