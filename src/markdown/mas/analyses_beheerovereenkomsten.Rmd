...
plot_brms_fit <- function(model, x, species = "", axis_label = NULL, FUN = mean,
  ...
  if (x %in% cat_vars) {
    ...
    out <- plot_df %>%
      ggplot(aes(x = .data[[x]], y = aantal, fill = .data[[x]]))
    ...
    out <- out +
      stat_halfeye(aes(y = .epred), alpha = 0.5,
                   .width = interval) +
      labs(x = axis_label,
...
Model selection between poisson and zero-inflated poisson.
...
```{r}
fit_olsbp_geelgors <- brm(
  formula = predictors_olsbp_list$Geelgors$formula,
  data = df_count_list[["Geelgors"]], 
  family = zero_inflated_poisson(),
  chains = nchains, 
  warmup = burnin, 
  iter = niter,
  ...
  thin = thinning,
  backend = "cmdstanr",
  file = paste0(model_path, "fit_olsbp_geelgors"),
  file_refit = "on_change")
...
```{r}
plot(fit_olsbp_geelgors, ask = FALSE)
```

```{r}
pp_check(fit_olsbp_geelgors, type = "bars_grouped", ndraws = 100, 
         group = "sbp", facet_args = list(ncol = 1, scales = "free_y"))
```

```{r}
for (pred in predictors_olsbp_list$Geelgors$predictors) {
  p <- plot_brms_fit(fit_olsbp_geelgors, pred)
  print(p)
}
```
...
```{r}
fit_olsbp_gelekwikstaart <- brm(
  formula = predictors_olsbp_list$GeleKwikstaart$formula,
  data = df_count_list[["GeleKwikstaart"]], 
  family = poisson(),
  chains = nchains, 
  warmup = burnin, 
  iter = niter,
  ...
  thin = thinning,
  backend = "cmdstanr",
  file = paste0(model_path, "fit_olsbp_gelekwikstaart"),
  file_refit = "on_change")

fit_olsbp_gelekwikstaart
```

```{r}
plot(fit_olsbp_gelekwikstaart, ask = FALSE)
```

```{r}
pp_check(fit_olsbp_gelekwikstaart, type = "bars_grouped", ndraws = 100, 
         group = "sbp", facet_args = list(ncol = 1, scales = "free_y"))
```

```{r}
for (pred in predictors_olsbp_list$GeleKwikstaart$predictors) {
  p <- plot_brms_fit(fit_olsbp_gelekwikstaart, pred)
  print(p)
}
```
...
```{r}
fit_olsbp_grasmus <- brm(
  formula = predictors_olsbp_list$Grasmus$formula,
  data = df_count_list[["Grasmus"]], 
  family = poisson(),
  chains = nchains, 
  warmup = burnin, 
  iter = niter,
  ...
  thin = thinning,
  backend = "cmdstanr",
  file = paste0(model_path, "fit_olsbp_grasmus"),
  file_refit = "on_change")
...
```{r}
plot(fit_olsbp_grasmus, ask = FALSE)
```

```{r}
pp_check(fit_olsbp_grasmus, type = "bars_grouped", ndraws = 100, 
         group = "sbp", facet_args = list(ncol = 1, scales = "free_y"))
```

```{r}
for (pred in predictors_olsbp_list$Grasmus$predictors) {
  p <- plot_brms_fit(fit_olsbp_grasmus, pred)
  print(p)
}
```
...
```{r}
fit_olsbp_haas <- brm(
  formula = predictors_olsbp_list$Haas$formula,
  data = df_count_list[["Haas"]], 
  family = zero_inflated_poisson(),
  chains = nchains, 
  warmup = burnin, 
  iter = niter,
  ...
  thin = thinning,
  backend = "cmdstanr",
  file = paste0(model_path, "fit_olsbp_haas"),
  file_refit = "on_change")
...
```{r}
plot(fit_olsbp_haas, ask = FALSE)
```

```{r}
pp_check(fit_olsbp_haas, type = "bars_grouped", ndraws = 100, 
         group = "sbp", facet_args = list(ncol = 1, scales = "free_y"))
```

```{r}
for (pred in predictors_olsbp_list$Haas$predictors) {
  p <- plot_brms_fit(fit_olsbp_haas, pred)
  print(p)
}
```
...
```{r}
fit_olsbp_kievit <- brm(
  formula = predictors_olsbp_list$Kievit$formula,
  data = df_count_list[["Kievit"]], 
  family = zero_inflated_poisson(),
  chains = nchains, 
  warmup = burnin, 
  iter = niter,
  ...
  thin = thinning,
  backend = "cmdstanr",
  file = paste0(model_path, "fit_olsbp_kievit"),
  file_refit = "on_change")
...
```{r}
plot(fit_olsbp_kievit, ask = FALSE)
```

```{r}
pp_check(fit_olsbp_kievit, type = "bars_grouped", ndraws = 100, 
         group = "sbp", facet_args = list(ncol = 1, scales = "free_y"))
```

```{r}
for (pred in predictors_olsbp_list$Kievit$predictors) {
  p <- plot_brms_fit(fit_olsbp_kievit, pred)
  print(p)
}
```
...
```{r}
fit_olsbp_veldleeuwerik <- brm(
  formula = predictors_olsbp_list$Veldleeuwerik$formula,
  data = df_count_list[["Veldleeuwerik"]], 
  family = poisson(),
  chains = nchains, 
  warmup = burnin, 
  iter = niter,
  ...
  thin = thinning,
  backend = "cmdstanr",
  file = paste0(model_path, "fit_olsbp_veldleeuwerik"),
  file_refit = "on_change")

fit_olsbp_veldleeuwerik
```

```{r}
plot(fit_olsbp_veldleeuwerik, ask = FALSE)
```

```{r}
pp_check(fit_olsbp_veldleeuwerik, type = "bars_grouped", ndraws = 100, 
         group = "sbp", facet_args = list(ncol = 1, scales = "free_y"))
```

```{r}
for (pred in predictors_olsbp_list$Veldleeuwerik$predictors) {
  p <- plot_brms_fit(fit_olsbp_veldleeuwerik, pred)
  print(p)
}
```
...
```{r}
fit_olsbp_wittekwikstaart <- brm(
  formula = predictors_olsbp_list$WitteKwikstaart$formula,
  data = df_count_list[["WitteKwikstaart"]], 
  family = poisson(),
  chains = nchains, 
  warmup = burnin, 
  iter = niter,
  ...
  thin = thinning,
  backend = "cmdstanr",
  file = paste0(model_path, "fit_olsbp_wittekwikstaart"),
  file_refit = "on_change")

fit_olsbp_wittekwikstaart
```

```{r}
plot(fit_olsbp_wittekwikstaart, ask = FALSE)
```

```{r}
pp_check(fit_olsbp_wittekwikstaart, type = "bars_grouped", ndraws = 100, 
         group = "sbp", facet_args = list(ncol = 1, scales = "free_y"))
```

```{r}
for (pred in predictors_olsbp_list$WitteKwikstaart$predictors) {
  p <- plot_brms_fit(fit_olsbp_wittekwikstaart, pred)
  print(p)
}
```
