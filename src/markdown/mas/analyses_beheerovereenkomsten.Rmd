...
# Oostelijke leemstreek 2022-2023: sbp

We bekijken eerst het effect van `sbp`.

```{r}
doel_soorten_ol_pre <- c(
  "Veldleeuwerik", 
  "Gele Kwikstaart", 
  "Geelgors", 
  "Kievit", 
...
Onze doelsoorten zijn `r doel_soorten_ol_pre`.

## Lijst van datasets


```{r}
df_count_list <- create_data_list(doel_soorten_ol_pre,
                                  "Oostelijke leemstreek",
                                  2022:2023)
```

Voor Ringmus hebben we bijna geen data.
We zullen deze soort niet opnemen in de analyses van de Oostelijke leemstreek.

```{r}
for (i in seq_along(df_count_list)) {
  df <- df_count_list[[i]]
  spec <- names(df_count_list)[i]
...
```{r}
df_count_list[["Ringmus"]] <- NULL
```

We centreren en schalen de continue variabelen.

```{r}
df_count_list_scaled <- lapply(df_count_list, function(df) {
    df %>%
      mutate(across(.cols = c(where(is.numeric), -c(id, aantal, jaar)), 
                    .fns = ~ as.numeric(scale(.x))))
  })
...
```{r}
# Formula
indices_to_remove_olsbp <- which(names(df_count_list$GeleKwikstaart) %in% c(
  "id", 
  "aantal", 
  "plotnaam", 
  "regio",
...
predictors_olsbp <- names(df_count_list$GeleKwikstaart)[-indices_to_remove_olsbp]

form_refmod_olsbp <- bf(paste("aantal", "~", paste(predictors_olsbp, 
                                                   collapse = " + ")))
form_refmod_olsbp
```
...
```{r}
refmod_olsbp_list <- vector("list", length = length(df_count_list_scaled))
refmod_olsbp_names <- vector(length = length(df_count_list_scaled))

for (i in seq_along(df_count_list_scaled)) {
  spec <- names(df_count_list_scaled)[i]
  name <- paste0("refmod_olsbp_", spec)
  print(paste("Busy fitting", name, "..."))
  assign(name,
         brm(formula = form_refmod_olsbp,
             data = df_count_list_scaled[[i]], 
               family = poisson(),
               prior = R2D2_prior,
               chains = nchains, 
               warmup = burnin, 
  ...
  refmod_olsbp_list[[i]] <- get(name)
  refmod_olsbp_names[i] <- spec
}

names(refmod_olsbp_list) <- refmod_olsbp_names
```
...
for (i in seq_along(refmod_olsbp_list)) {
  title <- paste("Fit referentiemodel", names(refmod_olsbp_list)[i])
  ref_mod <- refmod_olsbp_list[[i]]
...
# Oostelijke leemstreek 2022-2023: area_prop_sb

We doen hetzelfde, maar nu beschouwen we `area_prop_sb` in plaats van de `sbp` variabele.

```{r}
# Formula
indices_to_remove2 <- which(names(df_count_list$GeleKwikstaart) %in% c(
  "id", 
  "aantal", 
  "plotnaam", 
  "regio",
...
predictors_olpropsb <- names(df_count_list$GeleKwikstaart)[-indices_to_remove2]

form_refmod_olpropsb <- bf(paste("aantal", "~", paste(predictors_olpropsb, 
                                                   collapse = " + ")))
form_refmod_olpropsb
```

```{r}
refmod_olpropsb_list <- vector("list", length = length(df_count_list_scaled))
refmod_olpropsb_names <- vector(length = length(df_count_list_scaled))

for (i in seq_along(df_count_list_scaled)) {
  spec <- names(df_count_list_scaled)[i]
  name <- paste0("refmod_olpropsb_", spec)
  print(paste("Busy fitting", name, "..."))
  assign(name,
         brm(formula = form_refmod_olpropsb,
             data = df_count_list_scaled[[i]], 
               family = poisson(),
               prior = R2D2_prior,
               chains = nchains, 
  ...
  refmod_olpropsb_list[[i]] <- get(name)
  refmod_olpropsb_names[i] <- spec
}

names(refmod_olpropsb_list) <- refmod_olpropsb_names
```

Niet altijd een goede fit:

```{r}
for (i in seq_along(refmod_olpropsb_list)) {
  title <- paste("Fit referentiemodel", names(refmod_olpropsb_list)[i])
  ref_mod <- refmod_olpropsb_list[[i]]
  print(pp_check(ref_mod, type = "bars", ndraws = 200) +
          ggtitle(title))
}
```
...
```{r}
search_terms_olpropsb <- get_search_terms(
  n_predictors_max = 2, 
  predictors = predictors_olpropsb[-match("area_prop_sb_cat", predictors_olpropsb)],
  keep_var = "area_prop_sb_cat")
```

```{r, eval=FALSE}
cvvs_olpropsb_list <- vector("list", length = length(refmod_olpropsb_list))
cvvs_olpropsb_names <- vector(length = length(refmod_olpropsb_list))

for (i in seq_along(refmod_olpropsb_list)) {
  spec <- names(refmod_olpropsb_list)[i]
  name <- paste0("cvvs_olpropsb_", spec)
  print(paste("Busy with", name, "..."))
  ...
  if (!file.exists(cvvs_file)) {
    assign(name, 
           cv_varsel(
             object          = refmod_olpropsb_list[[i]],
             method          = "forward",
             cv_method       = "LOO",
             search_terms    = search_terms_olpropsb,
             validate_search = TRUE))
    saveRDS(get(name), cvvs_file)
  } else {
    assign(name, readRDS(cvvs_file))
  ...
  cvvs_olpropsb_list[[i]] <- get(name)
  cvvs_olpropsb_names[i] <- spec
}

names(cvvs_olpropsb_list) <- cvvs_olpropsb_names
```

```{r, eval=FALSE}
for (i in seq_along(cvvs_olpropsb_list)) {
  title <- names(cvvs_olpropsb_list)[i]
  if (!title == "FALSE") {
    print(plot(cvvs_olpropsb_list[[i]], stats = "mlpd", deltas = TRUE, 
               ranking_abbreviate = TRUE, 
               ranking_abbreviate_args = list(minlength = 4)) +
             ggtitle(title))
    
    smmry <- summary(cvvs_olpropsb_list[[i]], stats = "mlpd", 
                 type = c("mean", "lower", "upper"),
                 deltas = TRUE)
    print(smmry, digits = 1)
    
    rk <- ranking(cvvs_olpropsb_list[[i]])
    pr_rk <- cv_proportions(rk)
...
# Oostelijke leemstreek totaalanalyse

Voor de totaalanalyse beschouwen we alle plots van 2018-2021 en voegen enkel plots van 2022 en 2023 toe als ze minstens eenmaal geteld zijn in een van de voorgaande jaren (cf. distance sampling analyses).
We voeren de PPVS uit zonder restricties.
...
Het gaat om zoveel plots (en dus datapunten) per jaar:

```{r}
df_count_list_oltotal$Geelgors %>%
  group_by(jaar) %>% summarise(n = n_distinct(plotnaam)) %>%
  kable()
```
...
```{r}
# Formula
indices_to_remove_oltotal <- which(names(df_count_list_oltotal$GeleKwikstaart) %in% c(
  "id", 
  "aantal", 
  "plotnaam", 
  "regio",
...
predictors_oltotal <- names(df_count_list_oltotal$GeleKwikstaart)[-indices_to_remove_oltotal]

form_refmod_oltotal <- bf(paste("aantal", "~", paste(predictors_oltotal, 
                                                   collapse = " + "),
                                "+", "(1 | plotnaam)"))
form_refmod_oltotal
```
...
```{r}
refmod_oltotal_veldleeuwerik <- brm(
  formula = form_refmod_oltotal,
  data = df_count_list_oltotal_scaled[["Veldleeuwerik"]], 
  family = poisson(),
  prior = R2D2_prior,
  chains = nchains, 
  warmup = burnin, 
...
```{r}
pp_check(refmod_oltotal_veldleeuwerik, type = "bars", ndraws = 200)
```

```{r}
cvvs_olsbp_veldleeuwerik_fast <- cv_varsel(
  refmod_oltotal_veldleeuwerik,
...
```{r}
cvvs_olsbp_veldleeuwerik <- cv_varsel(
  object          = refmod_oltotal_veldleeuwerik,
  method          = "forward",
  cv_method       = "LOO",
  nterms_max      = 19,
  validate_search = TRUE)
...
# De Moeren 2023: area_prop_sb

# De Moeren totaalanalyse
