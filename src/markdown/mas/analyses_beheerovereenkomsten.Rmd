...
```{r}
pp_check(test_pois1, type = "bars_grouped", ndraws = 100, 
         group = "regio", facet_args = list(ncol = 1, scales = "free_y"))
```
...
```{r}
hypothesis(test_pois1,
           "sbpbuiten + sbpbuiten:regioOostelijkeleemstreek = 0")
```

Visueel:

```{r}
plot(conditional_effects(test_pois1, 
                         effects = "regio:sbp"))
```
...
```{r}
test_pois2 <- 
  brm(formula = bf(aantal ~ regio*area_prop_sb),
      data = gele_kwik_df2_2022, 
      family = zero_inflated_poisson(), 
      chains = nchains, 
      warmup = burnin, 
...
```{r}
pp_check(test_pois2, type = "bars_grouped", ndraws = 100, 
         group = "regio", facet_args = list(ncol = 1, scales = "free_y"))
```
...
```{r}
plot(conditional_effects(test_pois2, 
                         effects = "area_prop_sb:regio"))
```
