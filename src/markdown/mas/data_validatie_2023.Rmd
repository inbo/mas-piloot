...
Welke soorten zijn gezien?
Zijn er typfoutenin de soortnamen?
...
Soorten die minder dan 10 keer gezien zijn:

```{r}
mas_telperiodes %>%
  st_drop_geometry() %>%
  group_by(naam) %>%
  mutate( `aantal waarnemingen` = n()) %>%
  filter( `aantal waarnemingen` < 10) %>%
  select( `aantal waarnemingen`, naam, plotnaam, waarnemer = waarneme, datum) %>%
  arrange( `aantal waarnemingen`) %>%
  kable()
```
...
```{r}
write_csv(mas_telperiodes,
          file.path(data_path, "20230802_qgis_export_sovon_wfs_2023.csv"))
```
