...
-  excl. natuurbeheeerplannen type I en beperkte bosbeheerplannen
...
# nog iets raars hier
nb_vlaanderen2 <- splits_polys(nb_vlaanderen, "naamdossie")

nb_nederland2 <- nb_nederland %>%
  st_cast("POLYGON") %>%
  rownames_to_column("id") %>%
  dplyr::select(id)

nb_frankrijk2 <- nb_frankrijk %>%
  group_by(NOM) %>%
  summarise() %>%
  st_cast("MULTIPOLYGON") %>%
  st_cast("POLYGON") %>%
...
# Selecteer polygonen binnen 20 km Vlaanderen
```
