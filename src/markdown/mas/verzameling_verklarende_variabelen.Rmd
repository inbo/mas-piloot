...
```{r}
jaar <- 2018:2023
telpunten_2018_2023_df <- merge(telpunten_2018_2022, as.data.frame(jaar))
```
...
telpunten_2018_2023_bo_soort <- add_bo_by_year(telpunten_2018_2023_df, bo_file_2022)
```
...
```{r}
aggregate(telpunten_2018_2023_bo_soort$area_prop_sb, 
          by = list(telpunten_2018_2023_bo_soort$jaar), FUN = summary)
```
...
```{r}
# Add proportion bo per year
telpunten_2018_2023_bo_rest <- add_bo_by_year(
  telpunten_2018_2023_df, 
  bo_file_2022,
  bh_doel = c("erosiebestrijding (ER)",
              "perceelsrandenbeheer (PR)"))

telpunten_2018_2023_bo_rest <- telpunten_2018_2023_bo_rest %>%
  st_drop_geometry() %>%
...
```{r}
aggregate(telpunten_2018_2023_bo_rest$area_prop_bo_overig, 
          by = list(telpunten_2018_2023_bo_rest$jaar), FUN = summary)
```
...
hftlt_groepen <- hftlt_groepen_raw %>%
  select(GWSNAM_H = LBLHFDTLT, GEWASGROEP = nieuwe_groep) %>%
  drop_na(GEWASGROEP)

hftlt_groepen %>%
  arrange(GEWASGROEP, GWSNAM_H) %>%
  kable()
```

```{r}
telpunten_2018_2023_hfdtlt_long_tot <- calc_vzml_by_year(
  telpunten_2018_2023_df,
  group_by_col = "GWSNAM_H",
  clip_bo = path_to_bo(2022))

telpunten_2018_2023_hfdtlt_long <- telpunten_2018_2023_hfdtlt_long_tot %>%
  left_join(hftlt_groepen, by = join_by(GWSNAM_H)) %>%
  # necessary for weird unicode character
  mutate(GEWASGROEP = ifelse(grepl("^Loods", GWSNAM_H),
                             "Landbouwinfrastructuur",
                             GEWASGROEP))

telpunten_2018_2023_hfdtlt <- telpunten_2018_2023_hfdtlt_long %>%
...
summary_df <- rbind(summary_df, tibble(
  categorie = "primair", 
  variabele = paste("hoofdtteelt", 
    tolower(unique(telpunten_2018_2023_hfdtlt_long$GEWASGROEP))),
  naam = telpunten_2018_2023_hfdtlt %>%
    select(ends_with("_hfdtlt")) %>%
    names(),
  eenheid = "proportie", 
  kaart = "landbouwgebruikspercelen",
...
```{r}
telpunten_2018_2023_hfdtlt_long %>%
...
## Oppervlakte-aandeel hoofdteelten vorig jaar

### Beschrijving

Het aandeel oppervlakte aan verschillende hoofdteelten binnen de telcirkel in het jaar ervoor.
Gegroepeerd volgens gewasgroep.
Kan van belang zijn voor bepaalde soorten, bv. Kievit die broed op maïsstoppels van teelt vorig jaar.
...
Zie oppervlakte-aandeel hoofdteelten (\@ref(opp-hfdtlt)).
Voor 2019-2023 kunnen we de waardes gebruiken van de vorige jaren.
Voor 2018 moeten we de berekingen van hierboven opniew doen o.b.v. de kaartlaag van 2017.

```{r}
# Add proportion main crop of the year before 2018
telpunten_2017 <- telpunten_2018_2023_df %>%
  filter(jaar == 2018) %>%
  mutate(jaar = jaar - 1)
telpunten_2017_hfdtlt_pre_long_tot <- calc_vzml_by_year(
  telpunten_2017,
  group_by_col = "GWSNAM_H",
  clip_bo = path_to_bo(2022))
telpunten_2017_hfdtlt_pre_long <- telpunten_2017_hfdtlt_pre_long_tot %>%
  left_join(hftlt_groepen, by = join_by(GWSNAM_H))

# Combine proportions of all years
telpunten_2018_2023_hfdtlt_pre_long <- bind_rows(
  telpunten_2017_hfdtlt_pre_long, telpunten_2018_2023_hfdtlt_long) %>%
  mutate(jaar = jaar + 1) %>%
  filter(jaar <= max(telpunten_2018_2023_hfdtlt$jaar)) # Filter year too much
  
telpunten_2018_2023_hfdtlt_pre <- telpunten_2018_2023_hfdtlt_pre_long %>%
  pivot_wider(id_cols = c(pointid, jaar), 
              names_from = GEWASGROEP, 
              values_from = area_prop, 
              values_fill = 0,
              values_fn = sum,
              names_glue  = "{GEWASGROEP}_hfdtlt_pre")
```

### Visualisatie

```{r}
telpunten_2018_2023_hfdtlt_pre_long %>%
  group_by(jaar, GEWASGROEP) %>%
  summarise(area_prop_hfdtlt = median(area_prop), .groups = "drop") %>%
  mutate(GEWASGROEP = reorder(factor(GEWASGROEP), area_prop_hfdtlt, sum,
                              decreasing = TRUE)) %>%
  ggplot() +
    geom_bar(aes(x = GEWASGROEP, y = area_prop_hfdtlt, 
                 fill = factor(jaar, ordered = TRUE)), 
             stat = "identity") +
    labs(title = "Oppervlakte-aandeel gewasgroepen vorig jaar", fill = "Jaar",
         x = "", y = "Mediaan proportie over telcirkels") +
    theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
```
...
We herschalen wel zodat de proporties opsommen tot 1 per telcirkel per jaar.
...
```{r}
telpunten_2018_2023_hfdtlt_div1 <- telpunten_2018_2023_hfdtlt_long_tot %>%
    group_by(pointid, jaar) %>%
    mutate(sum = sum(area_prop)) %>%
    ungroup() %>%
    mutate(area_prop_new = area_prop / sum,
           calc = area_prop_new * log(area_prop_new)) %>%
    group_by(pointid, jaar) %>%
...
```{r}
telpunten_2018_2023_hfdtlt_div1 %>%
  ggplot() +
...
```{r}
aggregate(telpunten_2018_2023_hfdtlt_div1$gewas_div_shannon, 
          by = list(telpunten_2018_2023_hfdtlt_div1$jaar), FUN = summary)
```
...
```{r}
telpunten_2018_2023_hfdtlt_div2 <- telpunten_2018_2023_hfdtlt_long_tot %>%
...
```{r}
telpunten_2018_2023_hfdtlt_div2 %>%
  ggplot() +
...
```{r}
aggregate(telpunten_2018_2023_hfdtlt_div2$gewas_div_n, 
          by = list(telpunten_2018_2023_hfdtlt_div2$jaar), FUN = summary)
```
...
```{r}
telpunten_2018_2023_hfdtlt_evenness <- telpunten_2018_2023_hfdtlt_div2 %>%
  full_join(telpunten_2018_2023_hfdtlt_div1, by = join_by(pointid, jaar)) %>%
...
```{r}
telpunten_2018_2023_hfdtlt_evenness %>%
  ggplot() +
...
```{r}
aggregate(telpunten_2018_2023_hfdtlt_evenness$gewas_evenness, 
          by = list(telpunten_2018_2023_hfdtlt_evenness$jaar), FUN = summary)
```
...
vrtlt_groepen %>%
  arrange(GEWASGROEP, GWSNAM_V) %>%
  kable()
```

```{r}
telpunten_2018_2023_vrtlt_long_tot <- calc_vzml_by_year(
  telpunten_2018_2023_df,
  group_by_col = "GWSNAM_V",
  clip_bo = path_to_bo(2022))

telpunten_2018_2023_vrtlt_long <- telpunten_2018_2023_vrtlt_long_tot %>%
  left_join(vrtlt_groepen, by = join_by(GWSNAM_V))

telpunten_2018_2023_vrtlt <- telpunten_2018_2023_vrtlt_long %>%
  pivot_wider(id_cols = c(pointid, jaar), 
              names_from = GEWASGROEP, 
              values_from = area_prop,
              values_fn = sum,
...
summary_df <- rbind(summary_df, tibble(
  categorie = "primair", 
  variabele = paste("voorteelt",
    tolower(unique(telpunten_2018_2023_vrtlt_long$GEWASGROEP))), 
  naam = telpunten_2018_2023_vrtlt %>%
    select(ends_with("_vrtlt")) %>%
    names(),
  eenheid = "proportie",
  kaart = "landbouwgebruikspercelen",
  bron = "Departement Landbouw en Visserij",
...
```{r}
telpunten_2018_2023_vrtlt_long %>%
  group_by(jaar, GEWASGROEP) %>%
...
telpunten_2018_2023_openheid_1000 <- add_openheid_landschap_to_frame(
...
telpunten_2018_2023_openheid_300 <- add_openheid_landschap_to_frame(
...
telpunten_2018_2023_openheid <- telpunten_2018_2023_openheid_1000 %>%
  full_join(st_drop_geometry(telpunten_2018_2023_openheid_300),
...
summary_df <- rbind(summary_df, tibble(
  ...
  naam = c("openheid_waarde_1000", "openheid_klasse_300"),
  eenheid = c("continu", "categorisch"), 
  kaart = "Digitaal Hoogtemodel Vlaanderen II (5 m)",
  bron = "Agentschap Digitaal Vlaanderen",
  afkorting = "",
  url = "https://www.vlaanderen.be/digitaal-vlaanderen/onze-oplossingen/earth-observation-data-science-eodas/het-digitaal-hoogtemodel/digitaal-hoogtemodel-vlaanderen-ii"))
```

### Visualisatie

```{r}
telpunten_2018_2023_openheid %>%
  ggplot() +
...
```{r}
aggregate(telpunten_2018_2023_openheid$openheid_waarde_1000, 
          by = list(telpunten_2018_2023_openheid$regio), FUN = summary)
```
...
```{r}
telpunten_2018_2023_sbp <- add_sbp_per_regio(telpunten_2018_2022, perimeters)
```
...
perimeters %>%
  ...
  ggplot() +
    ...
    geom_sf(data = telpunten_2018_2023_sbp %>% 
              filter(regio == "De Moeren"),
            aes(colour = sbp)) +
...
telpunten_2018_2023_kle_long <- telpunten_2018_2022 %>%
...
telpunten_2018_2023_kle <- telpunten_2018_2023_kle_long %>%
  st_drop_geometry() %>%
...
summary_df <- rbind(summary_df, tibble(
  categorie = "secundair", 
  variabele = tolower(unique(telpunten_2018_2023_kle_long$subklasse)), 
  naam = tolower(unique(telpunten_2018_2023_kle_long$subklasse)),
  eenheid = "proportie", 
  kaart = "kaart landschapselementen",
  bron = "Departement Landbouw en Visserij",
  afkorting = "Dept. LV",
...
> Doordat de lagen van 2022 en 2023 van een andere bron komen zit er een if statement in de functie `calc_perceelsgrootte_by_year()` die hiermee reking houdt, maar dit is geen mooie code

```{r}
telpunten_2018_2023_prcl <- calc_perceelsgrootte_by_year(telpunten_2018_2023_df)
```
...
# Filter and load
testcase <- telpunten_2018_2023_df %>%
...
summary_df <- rbind(summary_df, tibble(
  ...
  variabele = c("centrale waarde perceelsgroottes", 
                ...
                "variatiecoëfficiënt perceelsgroottes"),
  naam = c("perceel_median_area", "perceel_iqr_area", "perceel_cv_area"),
  eenheid = c("m²", "m²", "fractie"), 
  kaart = "landbouwgebruikspercelen",
  bron = "Departement Landbouw en Visserij",
  afkorting = "Dept. LV",
  url = "https://landbouwcijfers.vlaanderen.be/open-geodata-landbouwgebruikspercelen"))
...
```{r}
aggregate(telpunten_2018_2023_prcl$perceel_median_area, 
          by = list(telpunten_2018_2023_prcl$jaar), FUN = summary)
```
...
```{r}
aggregate(telpunten_2018_2023_prcl$perceel_iqr_area, 
          by = list(telpunten_2018_2023_prcl$jaar), FUN = summary)
```
...
```{r}
aggregate(telpunten_2018_2023_prcl$perceel_cv_area, 
          by = list(telpunten_2018_2023_prcl$jaar), FUN = summary)
```
...
# Add proportion bo per year
telpunten_2018_2023_inertia_sb <- add_bo_inertia(
    punten_df = telpunten_2018_2023_df, 
...
summary_df <- rbind(summary_df, c(
  categorie = "secundair",
  naam = "inertie soortbeschermingsmaatregelen",
  variabele = "inertie_sb", 
  eenheid = "aantal", 
  kaart = "Beheerovereenkomsten",
  bron = "Vlaamse Landmaatschappij",
  afkorting = "VLM",
...
```{r}
telpunten_2018_2023_inertia_sb %>%
  ggplot() +
    geom_bar(aes(x = inertie_sb)) +
    scale_x_continuous(breaks = 
                         unique(telpunten_2018_2023_inertia_sb$inertie_sb)) +
    labs(
...
```{r}
aggregate(telpunten_2018_2023_inertia_sb$inertie_sb, 
          by = list(telpunten_2018_2023_inertia_sb$jaar), FUN = summary)
```
...
```{r}
# Add proportion bo per year
telpunten_2018_2023_inertia_overig <- add_bo_inertia(
    telpunten_2018_2023_df, 
    path_bo = bo_file_2022, 
...
```{r}
telpunten_2018_2023_inertia_overig %>%
  ggplot() +
    geom_bar(aes(x = inertie_overig)) +
    scale_x_continuous(breaks = 
                         unique(telpunten_2018_2023_inertia_overig$inertie_overig)) +
    labs(
...
```{r}
aggregate(telpunten_2018_2023_inertia_overig$inertie_overig, 
          by = list(telpunten_2018_2023_inertia_overig$jaar), FUN = summary)
```
...
summary_df <- rbind(summary_df, tibble(
  categorie = "secundair", 
  variabele = c("aantal predatoren", "aantal soorten predatoren"),
  naam = c("n_predators", "n_predators_spec"),
  eenheid = "aantal", 
  kaart = "",
  bron = "",
  afkorting = "",
...
telpunten_2018_2023_natuurgebieden <- distance_weights %>%
  group_by(pointid) %>%
...
summary_df <- rbind(summary_df, tibble(
  categorie = "secundair", 
  variabele = "aanwezigheid natuurgebieden", 
  naam = "natuur_exp",
  eenheid = "km²", 
  kaart = "(zie tekst)",
  bron = "(zie tekst)",
...
```{r}
telpunten_2018_2023_natuurgebieden %>%
...
```{r}
aggregate(telpunten_2018_2023_natuurgebieden$natuur_exp, 
          by = list(telpunten_2018_2023_natuurgebieden$regio), FUN = summary)
```
...
variables_df_per_year <- telpunten_2018_2023_bo_soort %>%
  full_join(telpunten_2018_2023_bo_rest) %>%
  full_join(telpunten_2018_2023_hfdtlt, by = c("pointid", "jaar")) %>%
  full_join(telpunten_2018_2023_hfdtlt_pre, by = c("pointid", "jaar")) %>%
  full_join(telpunten_2018_2023_hfdtlt_div1, by = c("pointid", "jaar")) %>%
  full_join(telpunten_2018_2023_hfdtlt_div2, by = c("pointid", "jaar")) %>%
  full_join(telpunten_2018_2023_hfdtlt_evenness, by = c("pointid", "jaar")) %>%
  full_join(telpunten_2018_2023_vrtlt, by = c("pointid", "jaar")) %>%
  full_join(telpunten_2018_2023_prcl, by = c("pointid", "regio", "jaar", 
                                             "geometry")) %>%
  full_join(telpunten_2018_2023_inertia_sb, by = c("pointid", "regio", "jaar", 
                                                   "geometry")) %>%
  full_join(telpunten_2018_2023_inertia_overig, by = c("pointid", "regio", "jaar", 
                                                     "geometry"))

# Variables that do not vary by year
variables_df <- cbind(telpunten_2018_2023_openheid, 
                      st_coordinates(telpunten_2018_2023_openheid)) %>%
  full_join(st_drop_geometry(telpunten_2018_2023_sbp), 
            by = c("pointid", "regio")) %>%
  full_join(st_drop_geometry(telpunten_2018_2023_kle), 
            by = c("pointid", "regio")) %>%
  full_join(st_drop_geometry(telpunten_2018_2023_natuurgebieden), 
            by = c("pointid", "regio")) %>%
  expand_grid(jaar = unique(telpunten_2018_2023_df$jaar))
...
write_csv(final_variables_df, file.path("data", "processed", 
                                        "verklarende_variabelen_2018_2023.csv"))
```

```{r}
summary_df_out <- summary_df %>%
  rownames_to_column(var = "id") %>%
  mutate(naam = gsub(",?\\s", "_", naam),
         naam = iconv(tolower(naam), from = "UTF-8", to = "ASCII//TRANSLIT"))
write_csv(summary_df_out, file.path(
  "data", "processed", "verklarende_variabelen_2018_2023_metadata.csv"))
```
