...
> Qua manier van berekenen lijkt me meest relevant een oppervlakte in een buffer of nog beter een distance decay gewogen oppervlakte (cf https://www.jstor.org/stable/3071919)

> Of zie mail Hans: gewichten met formules van https://github.com/STBrinkmann/GVI#viewshed-greenness-visibility-index-vgvi

Logistic density decay function:
...
Exponential density decay function:
...
tibble(dist = seq(0, 10000, 1)) %>% 
  mutate(exponentieel = calc_weights(dist, 10000, b = 1, m = 1.3, 
                                     mode = "exponential")) %>%
  mutate(logistisch = calc_weights(dist, 10000, mode = "logistic")) %>%
  pivot_longer(cols = c("exponentieel", "logistisch"), 
