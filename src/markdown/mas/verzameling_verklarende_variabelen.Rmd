...
Belangrijkste teelten 2017-2021:\
<https://www.vlaanderen.be/datavindplaats/catalogus/landbouwgebruikspercelen-lv-2017>\
...
Bijvoorbeeld voor 2019 (2017-2021):
...
Bijvoorbeeld voor 2022 (2022-2023):
...
sfarrow::st_write_parquet(obj = temp, dsn = file.path(mbag_dir, "data", "landbouwgebruikspercelen", "parquet",
...
telpunten_2018_2023_hfdtlt_long_tot <- calc_lbg_by_year(telpunten_2018_2023_df)
telpunten_2018_2023_hfdtlt_long <- telpunten_2018_2023_hfdtlt_long_tot %>%
  filter(!is.na(LBLHFDTLT)) %>%
  select(-GEWASGROEP) %>%
  left_join(hftlt_groepen, by = join_by(LBLHFDTLT))
...
telpunten_2017 <- telpunten_2018_2023_df %>%
  filter(jaar == 2018) %>%
  mutate(jaar = jaar - 1)
telpunten_2017_hfdtlt_pre_long_tot <- calc_lbg_by_year(telpunten_2017)
...
```{r}
telpunten_2018_2023_vrtlt_long_tot <- calc_vzml_by_year(telpunten_2018_2023_df)
