...
telpunten_2018_2022_hfdtlt_div1 <- telpunten_2018_2022_hfdtlt_long_tot %>%
  ...
  summarise(gewas_div_shannon = -sum(calc), .groups = "drop")
```
...
```{r}
test <- st_intersection(kle_layer, perimeters)

# Bereken proportie kle's
prop_kle <- landusemetrics_grid_cell(
  grid_cell = telpunten_2018_2022 %>% st_buffer(dist = 300),
  layer = kle_layer %>% rename(geometry = geom) %>% mutate(kle = "kle"),
  grid_group_by_col = "pointid",
  layer_group_by_col = "kle") %>%
  ungroup()

# Voeg afwezige kle's toe aan dataframes
telpunten_2018_2022_kle <- telpunten_2018_2022 %>% 
  full_join(prop_kle, by = c("pointid")) %>%
  mutate(area_prop_kle = area_prop * 100) %>%
  select(-c(kle, cell_area, area_m2, area_prop)) %>%
  replace(is.na(.), 0)
```
...
summary_df <- rbind(summary_df, c(
  categorie = "secundair", 
  variabele = "oppervlakte-aandeel kleine landschapselementen", 
  eenheid = "percentage", 
...
```{r}
summary(telpunten_2018_2022_kle$area_prop_kle)
```
...
-  Collineariteit mediaan en IQR onderzoeken
-  Log-transformatie?
...
add_bo_inertia <- function(punten_df, path_bo) {
  ...
  for (i in seq_along(years)) {
    ...
    out_df_year <- st_intersection(st_buffer(punten_df_year, 300), 
      ...
      group_by(pointid) %>%
      summarise(inertia_sb = mean(inertia_row), .groups = "drop")
...
telpunten_2018_2022_inertia %>%
  ggplot() +
    geom_histogram(aes(x = inertia_sb), binwidth = 0.25) +
    labs(
      title = "Oppervlakte-aandeel soortbeschermingsmaatregelen",
      x = "Oppervlakte-aandeel (%)",
      y = "Aantal telpunten") +
...
Aantal predatoren (kraaiachtigen, roofvogels) per telcirkel.
...
Aantal predatoren tellen per telcirkel. Berekenen bij laden van data.
...
variables_df_per_year <- telpunten_2018_2022_bo %>%
  ...
  full_join(telpunten_2018_2022_inertia, by = c("pointid", "regio", "jaar", 
                                                "geometry")) %>%
  full_join(expand_grid(jaar = unique(telpunten_2018_2022_df$jaar), 
                        telperiode = paste0("R", 1:4)), by = "jaar")
...
final_variables_df <- variables_df_per_year %>%
  full_join(variables_df, by = c("pointid", "regio", "jaar", "telperiode", 
                                 "geometry")) %>%
