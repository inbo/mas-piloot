...
```{r}
aggregate(telpunten_2018_2022_hfdtlt_div1$shannon_index, 
...
```{r}
aggregate(telpunten_2018_2022_hfdtlt_div2$n, 
...
calc_perceelsgrootte_by_year <- function(punten_df) {
  ...
  for (i in seq_along(years)) {
    ...
    out_df_year <- lbg_binding %>% 
      ...
      st_drop_geometry() %>%
      group_by(pointid) %>%
      summarise(median_area = units::drop_units(median(area)),
                iqr_area = IQR(area),
                .groups = "drop")
...
```{r}
aggregate(telpunten_2018_2022_prcl$median_area, 
...
```{r}
aggregate(telpunten_2018_2022_prcl$iqr_area, 
          by = list(telpunten_2018_2022_prcl$jaar), FUN = summary)
```
...
add_bo_inertia <- function(punten_df, path_bo) {
  ...
  for (i in seq_along(years)) {
    ...
    out_df_year <- st_intersection(st_buffer(punten_df_year, 300), 
      ...
      group_by(pointid) %>%
      summarise(inertia = mean(inertia_row), .groups = "drop")
...
telpunten_2018_2022_inertia %>%
  ggplot() +
    geom_histogram(aes(x = inertia), binwidth = 0.25) +
    labs(
      title = "Oppervlakte-aandeel soortbeschermingsmaatregelen",
      x = "Oppervlakte-aandeel (%)",
      y = "Aantal telpunten") +
...
telpunten_2018_2022_inertia %>% 
  st_drop_geometry() %>% 
  mutate(cat = ifelse(inertia == 0, "0", ">0")) %>% 
  count(jaar, cat) %>%
  pivot_wider(names_from = cat, values_from = n)
```

```{r}
aggregate(telpunten_2018_2022_inertia$inertia, 
