...
Mediaan van groottes van percelen die overlappen met telcirkels. Een perceel is de combinatie van aaneenliggende landbouwgebruikspercelen met dezelfde hoofdteelt.
...
calc_perceelsgrootte_by_year <- function(punten_df) {
  ...
  for (i in seq_along(years)) {
    ...
    # Intersection
    intersect <- st_intersection(testpunt %>% st_buffer(300), lbg_binding)
    
    out_df_year <- lbg_binding %>% 
      filter(OIDN %in% intersect$OIDN) %>%       # Filter percelen
      full_join(st_drop_geometry(intersect)) %>% # Get poin ids back
...
testpunt <- telpunten_2018_2022_df %>% filter(jaar == 2021)

point <- st_intersection(testpunt %>% st_buffer(300), lbg_binding)

hallo <- lbg_binding %>% 
  filter(OIDN %in% point$OIDN) %>%
  full_join(st_drop_geometry(point)) %>%
  group_by(pointid, LBLHFDTLT) %>%
  summarise() %>%
  st_cast("MULTIPOLYGON") %>%
  st_cast("POLYGON") %>%
...
mapview::mapview(testpunt) + 
  mapview::mapview(testpunt %>% st_buffer(300), col.regions = "red") +
  mapview::mapview(hallo)
