...
Voor ons zijn de soortbeschermingsmaatregelen van belang.
...
```{r}
aggregate(telpunten_2018_2022_bo$area_prop_sb, 
          by = list(telpunten_2018_2022_bo$jaar), FUN = summary)
```
...
summary_df <- rbind(summary_df, tibble(
  ...
  eenheid = "proportie", 
  bron = "landbouwgebruikspercelen kaart (LV)"))
```
...
summary_df <- rbind(summary_df, c(
  categorie = "prioritair", 
  variabele = "gewasdiversiteit 1", 
  eenheid = "Shannon index", 
  bron = "landbouwgebruikspercelen kaart (LV)"))
```
...
We beshouwen vier categoriën van openheid landschap:
...
We zijn enkel geïnteresseerd in OL in De Moeren en OL en HOL in de Oostelijke leemstreek. Voor de modellen kunnen deze eruit gefilterd worden. Verder is het mogelijk dat deze variabele niet wordt meegenomen in het model, maar dat we voor open landschap soorten bijvoorbeeld enkel waarnemingen in OL selecteren HOL voor half-open landschap soorten. 
...
summary_df <- rbind(summary_df, tibble(
  ...
  eenheid = "proportie", 
  bron = "kaart landschapselementen (LV)"))
```
...
```{r}
telpunten_2018_2022_prcl <- calc_perceelsgrootte_by_year(
  telpunten_2018_2022_df %>% filter(jaar != 2022))
```
...
Gewogen gemiddelde van jaar sinds implementatie beheerovereenkomsten per telcirkel.  
...
-  Wel beheerovereekomst: Gewogen (oppervlakte beheerovereenkomst) gemiddelde van ...
   ...
   -  2: start beheerovereenkomst 1 jaar vòòr telling
   -  3: start beheerovereenkomst 2 jaar vòòr telling
   -  ...
...
```{r}
add_bo_inertia <- function(punten_df, path_bo) {
  # Read in bo layer and calculate start and stop year
  bo_layer <- read_bo(path = path_bo)
  bo_layer2 <- bo_layer %>% 
    mutate(startjaar = year(START), 
  ...
  for (i in seq_along(years)) {
    ...
    out_df_year <- st_intersection(st_buffer(punten_df_year, 300), 
      ...
      group_by(pointid) %>%
      summarise(inertia_sb = round(mean(inertia_row)), .groups = "drop")
...
# Add proportion bo per year
telpunten_2018_2022_inertia <- add_bo_inertia(telpunten_2018_2022_df, 
                                               bo_file_2022)
```
...
summary_df <- rbind(summary_df, c(
  categorie = "secundair", 
  variabele = "inertie beheerovereenkomsten", 
  eenheid = "aantal", 
  bron = "VLM"))
```
...
```{r}
telpunten_2018_2022_inertia %>%
  ggplot() +
    geom_bar(aes(x = inertia_sb)) +
    scale_x_continuous(breaks = 
                         unique(telpunten_2018_2022_inertia$inertia_sb)) +
    labs(
      title = "Implementatie beheerovereenkomsten",
      x = "Gemiddeld jaar sinds beheerovereenkomst (gecategoriseerd)",
      y = "Aantal telpunten") +
    facet_wrap(~jaar)
```
...
telpunten_2018_2022_inertia %>% 
  st_drop_geometry() %>% 
  mutate(cat = ifelse(inertia_sb == 0, "0", ">0")) %>% 
  count(jaar, cat) %>%
  pivot_wider(names_from = cat, values_from = n)
```

```{r}
aggregate(telpunten_2018_2022_inertia$inertia_sb, 
          by = list(telpunten_2018_2022_inertia$jaar), FUN = summary)
```

### Opmerkingen

Vreemde waarden: je hebt ofwel 0 ofwel continu tussen 1 en 5 ... Nuttig?
...
Een maat voor de oppervlatke aan natuurgebieden in de buurt. Hierbij willen we gebieden dichtbij meer gewicht geven dan gebieden veraf.
...
```{r}
# Visualise
mapview::mapview(st_buffer(perimeters, buffer_km * 1000), color = "red", 
...
Beschouw telpunt $i$ met $J$ natuurgebieden met oppervlakte $A$ ($j = 1, ..., J$) binnen een arbitraire straal. We kiezen hier voor `r max_distance / 1000` km. Dat wil zeggen we geen rekening houden met eventuele invloeden van natuurgebieden op grotere afstanden.
...
met $d_ij$ de afstand tussen telpunt $i$ en het centrum van natuurgebied $j$.  
...
```{r}
par(mfrow = c(2, 2))
GVI::visualizeWeights(max_distance, m = 0.4, b = 8, mode = "logit")
GVI::visualizeWeights(max_distance, m = 0.5, b = 8, mode = "logit")
GVI::visualizeWeights(max_distance, m = 0.4, b = 10, mode = "logit")
GVI::visualizeWeights(max_distance, m = 0.5, b = 10, mode = "logit")
par(mfrow = c(1, 1))
```
...
```{r}
par(mfrow = c(2, 2))
GVI::visualizeWeights(max_distance, m = 1, b = 6, mode = "exponential")
GVI::visualizeWeights(max_distance, m = 1.3, b = 6, mode = "exponential")
GVI::visualizeWeights(max_distance, m = 1, b = 3, mode = "exponential")
GVI::visualizeWeights(max_distance, m = 1.3, b = 3, mode = "exponential")
par(mfrow = c(1, 1))
```
...
distance_weights <- distance_long %>%
  mutate(w_log  = calc_weights(dist_m, max_distance, 
                               mode = "logistic"),
         wA_log = w_log * oppervlakte_km2,
         w_exp  = calc_weights(dist_m, max_distance, b = 1, m = 1.3, 
                               mode = "exponential"),
...
telpunten_2018_2022_natuurgebieden <- distance_weights %>%
  group_by(pointid) %>%
  summarise(natuur_log = sum(wA_log),
            natuur_exp = sum(wA_exp), .groups = "drop") %>%
  full_join(telpunten_2018_2022, by = "pointid") %>%
  select(pointid, regio, natuur_log, natuur_exp)
```

```{r}
summary_df <- rbind(summary_df, tibble(
  categorie = "secundair", 
  variabele = c("gewogen oppervlakte natuurgebieden (exponentieel verval)", 
                "gewogen oppervlakte natuurgebieden (logistisch verval)"), 
  eenheid = "km²", 
...
telpunten_2018_2022_natuurgebieden %>%
  pivot_longer(cols = c(natuur_log, natuur_exp), names_to = "vervalfunctie", 
               values_to = "natuur") %>%
  mutate(vervalfunctie = ifelse(vervalfunctie == "natuur_log", 
                                "logistische vervalfunctie",
                                "exponentiële vervalfunctie")) %>%
  ggplot(aes(x = natuur, fill = regio)) +
    geom_histogram(bins = 40) +
    facet_grid(regio~vervalfunctie, scales = "free_y") +
...
telpunten_2018_2022_natuurgebieden %>%
  ...
  mutate(vervalfunctie = ifelse(vervalfunctie == "natuur_log", 
                                "logistische vervalfunctie",
                                "exponentiële vervalfunctie")) %>%
  group_by(regio, vervalfunctie) %>%
  summarise(mediaan = median(natuur), 
            gemiddelde = mean(natuur),
            min = min(natuur),
            max =  max(natuur), 
            .groups = "drop") %>%
  kable()
...
variables_df_per_year <- telpunten_2018_2022_bo %>%
  ...
  full_join(telpunten_2018_2022_prcl, by = c("pointid", "regio", "jaar", 
                                             "geometry")) %>%
  full_join(telpunten_2018_2022_inertia, by = c("pointid", "regio", "jaar", 
                                                "geometry"))
