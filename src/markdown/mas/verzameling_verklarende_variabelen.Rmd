...
We selecteren alle natuurgebieden in Vlaanderen en Nederland, Wallonië en Frankrijk binnen een buffer van 20 km rond Vlaanderen.

> toon code voor selectie
> merge by naamdossie? nee zie id 46534 --> merge aanliggende polygonen?
...
nb_vlaanderen1 <- buffer_layers_to_perimeter(nb_vlaanderen, 20, "naamdossie")
nb_nederland1 <- buffer_layers_to_perimeter(nb_nederland, 20)
nb_frankrijk1 <- buffer_layers_to_perimeter(nb_frankrijk, 20, "NOM")
nb_wallonie1 <- buffer_layers_to_perimeter(nb_wallonie, 20)

# Visualise
mapview::mapview(st_buffer(perimeters, 20000), color = "red", 
                 alpha.regions = 0, legend = FALSE) + 
  mapview::mapview(nb_vlaanderen1, layer = "Vlaanderen",
                   col.regions = "yellow") + 
  mapview::mapview(select(nb_nederland1, geometry), layer = "Nederland", 
                   col.regions = "orange") +
  mapview::mapview(nb_frankrijk1, layer = "Frankrijk") + 
  mapview::mapview(nb_wallonie1, layer = "Wallonië",
                   col.regions = "red")
```

```{r}
# Opsplitsen in aparte polygonen (ev. mergen per gebied eerst)
splits_polys <- function(layer, group_var = NULL) {
  if (is.null(group_var)) {
    out <- layer %>%
      st_cast("POLYGON") %>%
      rownames_to_column("id") %>%
...
nb_vlaanderen2 <- splits_polys(nb_vlaanderen1, "naamdossie")
nb_nederland2 <- splits_polys(nb_nederland1)
nb_frankrijk2 <- splits_polys(nb_frankrijk1, "NOM")
...
Beschouw telpunt $i$ met $J$ natuurgebieden met oppervlakte $A$ ($j = 1, ..., J$) binnen een arbitraire straal (bv. 10 kilometer).
...
```{r}
par(mfrow = c(2, 2))
visualizeWeights(10000, m = 0.4, b = 8, mode = "logit")
visualizeWeights(10000, m = 0.5, b = 8, mode = "logit")
visualizeWeights(10000, m = 0.4, b = 10, mode = "logit")
visualizeWeights(10000, m = 0.5, b = 10, mode = "logit")
par(mfrow = c(1, 1))
```
...
```{r}
par(mfrow = c(2, 2))
visualizeWeights(10000, m = 1, b = 6, mode = "exponential")
visualizeWeights(10000, m = 1.3, b = 6, mode = "exponential")
visualizeWeights(10000, m = 1, b = 3, mode = "exponential")
visualizeWeights(10000, m = 1.3, b = 3, mode = "exponential")
par(mfrow = c(1, 1))
```
...
```{r}
tibble(dist = seq(0, 10000, 1)) %>% 
  mutate("exponentieel\n(m = 1.3, b = 1)" = calc_weights(
    dist, 10000, b = 1, m = 1.3, mode = "exponential")) %>%
  mutate("logistisch\n(m = 0.5, b = 8)" = calc_weights(
    dist, 10000, mode = "logistic")) %>%
