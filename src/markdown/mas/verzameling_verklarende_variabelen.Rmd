...
source(file.path("src", "R", "berekening_verklarende_variabelen.R"))
```
...
```{r}
telpunten_2018_2022_vrtlt_long <- calc_vzml_by_year(telpunten_2018_2022_df)
...
summary_df <- rbind(summary_df, tibble(
  categorie = "prioritair", 
  variabele = tolower(unique(telpunten_2018_2022_vrtlt_long$GWSNAM_V)), 
  eenheid = "proportie",
...
telpunten_2018_2022_vrtlt_long %>%
  group_by(jaar, GWSNAM_V) %>%
  summarise(area_prop_vrtlt = median(area_prop), .groups = "drop") %>%
  mutate(GWSNAM_V = reorder(factor(GWSNAM_V), area_prop_vrtlt, sum,
                              decreasing = TRUE)) %>%
  ggplot() +
    geom_bar(aes(x = GWSNAM_V, y = area_prop_vrtlt  , 
                 fill = factor(jaar, ordered = TRUE)), 
             stat = "identity") +
    labs(title = "Oppervlakte-aandeel gewasgroepen", fill = "Jaar",
         x = "", y = "Mediaan proportie over telcirkels") +
...
telpunten_2018_2022_vrtlt_long %>%
  ...
  summarise(mediaan = median(area_prop), 
            ...
            max =  max(area_prop), 
            .groups = "drop") %>%
  mutate(GWSNAM_V = reorder(factor(GWSNAM_V), mediaan, sum,
                              decreasing = TRUE)) %>%
  arrange(jaar, GWSNAM_V) %>%
  kable()
```
...
telpunten_2018_2022_inertia_sb %>%
  ggplot() +
    ...
    scale_x_continuous(breaks = 
                         unique(telpunten_2018_2022_inertia_sb$inertia_sb)) +
    labs(
...
```{r}
aggregate(telpunten_2018_2022_inertia_sb$inertia_sb, 
...
```{r}
# Add proportion bo per year
telpunten_2018_2022_inertia_rest <- add_bo_inertia(
    telpunten_2018_2022_df, 
    path_bo = bo_file_2022, 
    bh_doel = c("erosiebestrijding (ER)", "perceelsrandenbeheer (PR)")) %>%
  rename(inertie_rest = inertia)
```
...
```{r}
telpunten_2018_2022_inertia_rest %>%
  ggplot() +
    geom_bar(aes(x = inertia_rest)) +
    scale_x_continuous(breaks = 
                         unique(telpunten_2018_2022_inertia_rest$inertia_rest)) +
    labs(
...
telpunten_2018_2022_inertia_rest %>% 
  st_drop_geometry() %>% 
  mutate(cat = ifelse(inertia_rest == 0, "0", ">0")) %>% 
  count(jaar, cat) %>%
  pivot_wider(names_from = cat, values_from = n) %>%
  kable()
```
...
```{r}
aggregate(telpunten_2018_2022_inertia_rest$inertia_rest, 
          by = list(telpunten_2018_2022_inertia_rest$jaar), FUN = summary)
```
...
variables_df_per_year <- telpunten_2018_2022_bo_soort %>%
  ...
  full_join(telpunten_2018_2022_inertia_sb, by = c("pointid", "regio", "jaar", 
                                                   "geometry")) %>%
  full_join(telpunten_2018_2022_inertia_rest, by = c("pointid", "regio", "jaar", 
                                                     "geometry"))
