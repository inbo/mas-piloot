...
summary_df <- rbind(summary_df, c(
  categorie = "secundair", 
  variabele = "openheid landschap (1000 m)", 
  eenheid = "continu", 
...
telpunten_2018_2022_openheid %>%
  ggplot() +
    geom_histogram(aes(x = openheid_waarde, fill = openheid_klasse)) +
    facet_wrap(~regio) +
...
```{r}
aggregate(telpunten_2018_2022_openheid$openheid_waarde, 
...
telpunten_2018_2022_inertia_rest %>%
  ggplot() +
    geom_bar(aes(x = inertie_rest)) +
    scale_x_continuous(breaks = 
                         unique(telpunten_2018_2022_inertia_rest$inertie_rest)) +
    labs(
...
telpunten_2018_2022_inertia_rest %>% 
  st_drop_geometry() %>% 
  mutate(cat = ifelse(inertie_rest == 0, "0", ">0")) %>% 
  count(jaar, cat) %>%
  pivot_wider(names_from = cat, values_from = n) %>%
  kable()
```
...
```{r}
aggregate(telpunten_2018_2022_inertia_rest$inertie_rest, 
