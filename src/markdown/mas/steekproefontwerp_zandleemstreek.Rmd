...
We merkten echter dat een maximale trekking niet voor een goede spatiale spreiding zorgt omdat het algorime niet genoeg vrijheid krijgt. Daarom besluiten we uiteindelijk om 20 % van de steekproefkader te nemen per stratum.

Minima onder x4 scenario.
4 strata (OL, HOL en binnen, buiten sbp) met minstens 50 telpunten per stratum.
x4 betekend halvering betrouwbaarheidsinterval --> minimum sample size is 200 telpunten per stratum.
...
Om te veel overlap te voorkomen binnen enzelfde stratum, is voor elk punt binnen hetzelfde stratum nagegaan of een punt met grotere `sample_order` op minder dan 300 m afstand ligt. Als dit het geval is wordt het punt met de lagere `sample_order` weerhouden. Dan eindigen we met de volgende proporties:  
...
```{r}
first_n <- function(steekproef_df, n) {
  steekproef_df %>%
