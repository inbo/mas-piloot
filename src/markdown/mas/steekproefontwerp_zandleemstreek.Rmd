...
Berekening cumulatieve zichtbaarheidsindex binnen straal van 25 m rond telpunt (zone bewegingsvrijheid van de waarnemer) in een raster (resolutie 5 m) van locaties die 10 m van elkaar liggen. Door middel van Digital Surface Model (DSM) en Digital Terrain Model (DTM). Aanpassing aan **GVI** package. Zie: `visibility.Rmd`.

In de pipeline werd gebruik gemaakt van batching per 200 telpunten om rekentijd in te perken. Zie:  
...
We verwijderen telpunten waar de cumulatieve 'viewshed of visibility' kleiner is dan 0.10.
...
Het oppervlakte aandeel aan SBP's binnen het steekproefkader. Som per telcirkel:
...
Het oppervlakte aandeel aan SBP's binnen het steekproefkader. Union van telcirkels:
...
$\times 4$ betekend halvering betrouwbaarheidsinterval $\rightarrow$ minimum sample size is 200 telpunten per stratum. Niet echt belangrijk hier omdat we een maximum steekproefkader trekken en de tellers gewoon zoveel mogelijk punten laten doen (volgnummer).
...
```{r trekking}
steekproef <- tar_read(steekproef, store = targets_store) %>%
  mutate(openheid_sbp = paste(openheid_klasse,
                             ifelse(is_sbp,
                                    "binnen plan",
                                    "buiten plan"))
...
```{r check-balans}
first_samples <- function(steekproef_df, n) {
  steekproef_df %>%
    filter(sample_order <= n) %>%
    mutate(puntenset = paste("steekproef eerste", n, "per stratum", sep = " "))
}

balansvergelijking <- first_samples(steekproef, 200) %>%
  bind_rows(first_samples(steekproef, 100)) %>%
  bind_rows(first_samples(steekproef, 50)) %>%
  bind_rows(first_samples(steekproef, 20)) %>%
  ...
  mutate(fillvar = factor(puntenset, levels = c(paste("steekproef eerste", 
                                                    c(20, 50, 100, 200), 
                                                    "per stratum", sep = " "),
...
We zien dat de steekproef vanaf de eerste 50 en nog beter de eerste 100 punten per stratum een goede afspiegeling is van de volledige populatie (boxplots voor steekproef eerste punten en volledig steekproefkader zijn zeer gelijkend).

## Visualisatie

Waar liggen de punten?

```{r}
mapview(perimeters_data, color = "red", alpha.regions = 0, legend = FALSE) +
  mapview(first_samples(steekproef, 200), alpha.regions = 1) +
  mapview(first_samples(steekproef, 100), col.regions = "green", 
          alpha.regions = 1) +
  mapview(first_samples(steekproef, 50), col.regions = "red", 
          alpha.regions = 1) 
```
...
We willen een lijst met alle bestaande punten en hun historie (vanaf wanneer beginnen tellen). Zo kunnen we een regel verzinnen om bestaande telpunten over te nemen bij een nieuwe trekking. Bijvoorbeeld: Als een nieuw punt voor meer dan 50 % overlapt met een bestaand punt en deze behoort tot hetzelfde stratum, dan wordt het nieuwe punt vervangen door het bestaande punt.
