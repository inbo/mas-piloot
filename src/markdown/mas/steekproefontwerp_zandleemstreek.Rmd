...
```{r}
punten_zichtbaarheid <- tar_read(punten_zichtbaarheid, store = targets_store)
...
In de zandleemstreek hebben we:
...
Het oppervlakte aandeel aan SBP's binnen de zandleemstreek:
...
Het oppervlakte aandeel aan SBP's binnen OL en HOL in de zandleemstreek:
...
Er zijn 4 strata (OL, HOL en binnen, buiten sbp) binnen de zandleemstreek.
...
De eerste bv. 300 (laagste volgnummers) zullen dan de set zijn voor de zandleemstreek, die kan verdeeld worden over de medewerkers.
...
Om te veel overlap te voorkomen binnen enzelfde stratum, is voor elk punt binnen hetzelfde stratum nagegaan of een punt met grotere `sample_order` op minder dan 300 m afstand ligt. Als dit het geval is wordt het punt met de lagere `sample_order` weerhouden.

```{r trekking}
steekproef <- tar_read(steekproef_thinned, store = targets_store) %>%
  mutate(openheid_sbp = paste(openheid_klasse,
                             ifelse(is_sbp,
                                    "binnen plan",
                                    "buiten plan"))
...
balansvergelijking <- do.call(rbind, lapply(min_n_per_stratum, first_samples, 
  ...
  mutate(fillvar = factor(puntenset, levels = c(paste("steekproef", 
                                                    c(20, 50, 100, 200), 
                                                    "per stratum", sep = " "),
...
balansvergelijking %>%
  st_drop_geometry() %>%
  group_by(fillvar, openheid = openheid_klasse, is_sbp) %>%
...
We beschouwen verschillende steekproeven waarbij we de eerste `r min_n_per_stratum` samples per stratum nemen. Dit wil zeggen dat het stratum met de kleinste allocatiekans het minimum aantal samples heeft en de andere strata meer samples proportioneel met dit minimum.
...
Ook lijken de SBP's gemiddeld iets zuidelijker gelegen.
...
Minstens 50 punten per stratum:

```{r}
mapview_n(50)
```

Minstens 100 punten per stratum:

```{r}
mapview_n(100)
```

Minstens 200 punten per stratum:
...
In dit geval zal er overlap zijn met de punten uit de pilootstudie in De Moeren. 
We vervangen alle telpunten uit de steekproef van de zandleemstreek door reeds bestaande punten indien ze voor minstens 50 % overlappen en ze tot hetzelfde stratum behoren.
...
cirkels_zandleemstreek <- steekproef %>% 
  st_buffer(300)
```
...
intersect <- st_intersection(oude_cirkels_sbp, cirkels_zandleemstreek) %>% 
...
intersect %>%
  kable(col.names = c("bestaand", "nieuw", "intersect_area"))
```
...
mapview(oude_cirkels_sbp %>% filter(plotnaam %in% intersect$plotnaam), 
  ...
  mapview(cirkels_zandleemstreek %>% filter(pointid %in% intersect$pointid),
          layer = "nieuwe punten")
```

We vervangen de nieuwe punten in de steekproef door de bestaande.

```{r}
# Vervang id en geometry
bestaand_overlap <- oude_punten_sbp %>%
  inner_join(intersect, by = "plotnaam") %>%
  select(plotnaam, pointid, regio) %>%
  inner_join(steekproef %>% st_drop_geometry(), by = "pointid") %>%
  select(pointid = plotnaam, key, value, Naam, tar_group, cvvi, area_prop_sb, 
         openheid_klasse, openheid_waarde, is_sbp, in_sample, sample_order, 
         batch, geometry, openheid_sbp)
class(bestaand_overlap) <- class(steekproef)

# voeg toe aan steekproef
steekproef_overlap <- steekproef %>%
  filter(!pointid %in% intersect$pointid) %>%
  bind_rows(bestaand_overlap) %>%
  arrange(openheid_sbp, sample_order)
```


## Output als KML

We zullen een steekproef van 50 cirkels per stratum wegschrijven (eerste set) waarbij we ook de volgende 50 punten per stratum includeren voor als er een punt niet zou kunnen geteld worden in het veld (reserve set).

```{r}
steekproef_out <- first_samples(steekproef_overlap, 50, 2) %>%
   select(pointid, Naam, openheid_sbp, sample_order, zichtbaarheid = cvvi, 
          wegtype = value)

mapview(steekproef_overlap, alpha.regions = 0,
        layer = "steekproef") +
  mapview(steekproef_out, zcol = "batch",
          layer = "eerste 50 per stratum en 50")
```
...
steekproef_out %>%
  ...
  st_write(dsn = file.path(
    "data",
    "processed",
    "steekproef_zandleemstreek_telpunten.kml"),
    delete_dsn = TRUE)
...
steekproef_out %>%
  ...
  st_write(dsn = file.path(
    "data",
    "processed",
    "steekproef_zandleemstreek_telcirkels.geojson"),
    delete_dsn = TRUE)
...
steekproef_out %>%
  ...
  st_write(dsn = file.path(
    "data",
    "processed",
    "steekproef_zandleemstreek_telcirkels.kml"),
    delete_dsn = TRUE)
```
