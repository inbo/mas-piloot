...
```{r}
first_samples <- function(steekproef_df, min_n) {
  min_factor <- min_n / min(allocatie$allocatie)
  first_design <- data.frame(openheid_klasse = allocatie$openheid_klasse, 
                             is_sbp = allocatie$is_sbp,
                             n = ceiling(allocatie$allocatie * min_factor))
  
  steekproef_df %>%
    group_by(openheid_sbp) %>% 
    mutate(id = row_number()) %>%
    ungroup() %>%
    full_join(first_design, by = c("openheid_klasse", "is_sbp")) %>%
    filter(id <= n) %>%
    select(-n) %>%
    mutate(puntenset = paste("steekproef minstens", min_n, "per stratum", 
                             sep = " "))
}
```
...
balansvergelijking <- do.call(rbind, lapply(min_n_per_stratum, first_samples, 
  ...
  mutate(fillvar = factor(puntenset, levels = c(paste("steekproef minstens", 
                                                    c(20, 50, 100, 200), 
                                                    "per stratum", sep = " "),
...
We beschouwen verschillende steekproeven waarbij we minstens de eerste `r min_n_per_stratum` samples per stratum nemen. Dit wil zeggen dat het stratum met de kleinste allocatiekans het minimum aantal samples heeft en de andere strata meer samples proportioneel met dit minimum.
...
We zien dat de steekproef vanaf minimum 50 punten per stratum een goede afspiegeling is van de volledige populatie (boxplots voor steekproef eerste punten en volledig steekproefkader zijn zeer gelijkend).
...
mapview_n <- function(n) {
  mapview(perimeters_data, color = "red", alpha.regions = 0, legend = FALSE) +
  mapview(first_samples(steekproef, n) %>% st_buffer(dist = 300), 
...
Is er overlap bij dataset steekproef minstens 50 per stratum?
...
nieuwe_cirkels <- first_samples(steekproef, 50) %>% 
  st_buffer(300)

intersect <- st_intersection(oude_cirkels, nieuwe_cirkels) %>% 
  mutate(intersect_area = st_area(.) %>% units::drop_units()) %>%
  select(plotnaam, pointid, intersect_area) %>%
  filter(intersect_area >= 0.5 * 300 * 300 * pi) %>%
  st_drop_geometry()

mapview(oude_cirkels %>% filter(plotnaam %in% intersect$plotnaam), 
        col.regions = "red") + 
  mapview(nieuwe_cirkels %>% filter(pointid %in% intersect$pointid))
```

Er zijn twee cirkels die minstens 50% overlappen.


## Output als KML

Proporties minstens 50 per stratum.

```{r}
first_samples(steekproef, 50) %>%
  st_drop_geometry() %>%
...
We schrijven de steekproef van minstens 50 per stratum weg.

```{r}
steekproef_out <- first_samples(steekproef, 50) %>%
