...
m <- mapview(st_crop(perimeters_data, detail), color = "red", col.regions = "red", 
...
m <- mapview(st_crop(perimeters_data, detail), color = "red", col.regions = "red", 
  ...
  mapview(st_crop(exclusie_osm_landgebruiken, detail), col.regions = "green", 
          legend = FALSE) +
  mapview(st_crop(paden, detail), legend = FALSE)+
  mapview(st_crop(punten, detail), zcol = "value", layer = "OSM value")
...
telcirkels_selectie_landgebruik <- tar_read(telcirkels_selectie_landgebruik,
                                   store = targets_store) %>%
  filter(startsWith(pointid, abbreviate("Zandleemstreek", 2)))
```
...
Om te veel overlap te voorkomen binnen enzelfde stratum, is voor elk punt binnen hetzelfde stratum nagegaan of een punt met grotere `sample_order` op minder dan 600 m afstand ligt. Als dit het geval is wordt het punt met de lagere `sample_order` weerhouden. Voor een vergelijking tussen 300 en 600 m thinning, zie steekproefontwerp_leemstreek.Rmd.

```{r trekking}
steekproef <- tar_read(steekproef_final_600, store = targets_store) %>%
  filter(Naam == "Zandleemstreek") %>%
  mutate(openheid_sbp = paste(openheid_klasse,
                             ifelse(is_sbp,
                                    "binnen plan",
...
cirkels_zandleemstreek <- steekproef_raw %>% 
  st_buffer(300)
```
...
In dit geval zal er wat overlap zijn met de punten uit De Moeren. 
...
In de Zandleemstreek zijn 23 punten gevonden die niet mogelijk zijn om te tellen.  
...
Uiteindelijk moeten we tot een CSV komen waarbij enkel de punten *behouden* van de eerste batch en *vervanging* van de reserve batch geüpload worden in Avimap (met tellerscode erbij). Overlappen met reeds getelde MAS punten worden niet meegegeven (5 punten).

```{r}
zandleemstreek_avimap_df <- read_csv(file.path(
    "data",
    "processed",
    "MAS netwerk Vlaanderen- Zandleemstreek_telpunten.csv"))

zandleemstreek_avimap_sf <- st_as_sf(zandleemstreek_avimap_df,
                                 wkt  = "WKT",
                                 crs = st_crs(4326)) %>%
  st_zm() %>%
  st_transform(31370) %>%
...
zandleemstreek_avimap_sf %>%
  st_write(dsn = file.path(
    "data",
    "processed",
    "steekproef_zandleemstreek_avimap.gpkg"),
    delete_dsn = TRUE)
...
zandleemstreek_avimap_sf %>%
  st_write(dsn = file.path(
    "data",
    "processed",
    "steekproef_zandleemstreek_avimap.geojson"),
    delete_dsn = TRUE)
```
...
```{r}
git2rdata::write_vc(zandleemstreek_avimap_df,
                    file = file.path(
                      "data",
                      "processed",
                      "steekproef_zandleemstreek_avimap"),
                    sorting = "pointid")
```
