...
# Loop over species
for (soort in names(distance_dfs_2018_2022_final)) {
  distance_data <- distance_dfs_2018_2022_final[[soort]]
  sample_table_2018_2022 <- sample_tables_2018_2022[[soort]]
  obs_table_2018_2022 <- obs_tables_2018_2022[[soort]]
  
  out_models <- list()
  model_names <- c()

  # Loop over formules en over key functions
  for (i in seq_len(length(forms_2018_2022))) {
    f <- as.character(forms_2018_2022[i]) # f is de formule
    
    for (k in keys) {
      # Naam voor model
      mod_name <- paste0(soort, "_distmod_2022_", k, i)
      file <- paste0(model_path, mod_name, ".rds")
  ...
  all_model_names <- c(all_model_names, soort)
}
...
```{r}
summarize_ds_models2(all_models$Zwartkop)
```

```{r}
par(mfrow = c(1, 2))
von_mises <- gof_ds(Zwartkop_distmod_2022_hn2 )
von_mises
plot(Zwartkop_distmod_2022_hn2 , pdf = TRUE, showpoints = FALSE)
par(mfrow = c(1, 1))
```

```{r}
plot_detection_curve(Zwartkop_distmod_2022_hn2)
```
