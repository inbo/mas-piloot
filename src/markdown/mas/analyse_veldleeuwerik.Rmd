...
1.  Dieren zijn uniform verdeeld over de ruimte (random plaatsing van steekproefeenheden met betrekking tot de verdeling van de dieren in de ruimte)
2.  Detectiekans is een functie van afstand en is gelijk aan 1 op afstand 0
3.  Individuen worden gedetecteerd op hun oorspronkelijke locatie
...
par(mfrow = c(1, 2))
gof_ds(test1, main = "test1")

plot(test1, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     main = paste0("test1", " - ", "AIC: ",
                   as.character(round(summary(test1)$ds$aic, digits = 2))
                   )
     )
par(mfrow = c(1, 1))
```
...
plot_detection_curve(veldleeuwerik_dist_hr12, n_breaks = 60)
plot_detection_curve(test1)
```
