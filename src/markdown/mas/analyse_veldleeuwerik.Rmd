...
```{r, cache=TRUE}
model_selection <- summarize_ds_models(
  veldleeuwerik_dist_hn0, veldleeuwerik_dist_hr0, 
...
Het Hazard-rate model waarbij de detectiekans afhankelijk is van regio, jaar en openheid scoort het best.
...
```{r, cache=TRUE}
veldleeuwerik_dist_hr22 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*jaar + openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)

veldleeuwerik_dist_hr23 <- ds(data = veldleeuwerik_distance, key = "hr", 
...
summarize_ds_models(
  veldleeuwerik_dist_hr12, veldleeuwerik_dist_hr15,
  veldleeuwerik_dist_hr16, veldleeuwerik_dist_hr20,
  veldleeuwerik_dist_hr22, veldleeuwerik_dist_hr23
  )
```

We selecteren het Hazard-rate model waarbij de detectiekans afhankelijk is van regio, jaar en openheid met regio en jaar in interactie.
...
De Cramér-von Mises kwantificeert de informatie van de Q-Q-plot door te testen of punten van de EDF en CDF uit dezelfde verdeling komen [@buckland2015; @miller2019].
Voor studies met punttransecten geven kansdichtheidsfunctieplots ook een goed idee over model fit.

```{r}
par(mfrow = c(1, 2))
von_mises <- gof_ds(veldleeuwerik_dist_hr22)
von_mises
plot(veldleeuwerik_dist_hr22, pdf = TRUE, showpoints = FALSE)
par(mfrow = c(1, 1))
```

De visuele fit van de Q-Q-plot ziet er goed uit maar Cramér-von Mises test suggereert dat het model niet goed past bij de data ($W^2 =$ `r round(von_mises$dsgof$CvM$W, digits = 3)`, $p =$ `r round(von_mises$dsgof$CvM$p, digits = 3)`). Ook de kansdichtheidsfunctieplot toont geen ideale fit. Inderdaad, als we kijken naar de verdeling van de afstanden, zien we dat de detectiekans niet altijd afneemt over de afstand. Het is dus niet mogelijk om een detectiecurve perfect te fitten.

```{r}
plot_detection_curve(veldleeuwerik_dist_hr22, plot_average_fit = FALSE, 
                     n_breaks = 60)
```
...
```{r, cache=TRUE}
veldleeuwerik_dist_hr22_binned <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*jaar + openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
...
```{r}
plot(veldleeuwerik_dist_hr22_binned, pdf = TRUE, showpoints = FALSE)
```

```{r}
veldleeuwerik_dist_model <- veldleeuwerik_dist_hr22_binned
```
...
```{r}
cbind(veldleeuwerik_dist_model$ddf$data, 
      det_prob = predict(veldleeuwerik_dist_model, se.fit = TRUE)$fitted, 
      det_prob_se = predict(veldleeuwerik_dist_model, se.fit = TRUE)$se) %>%
  select(regio, jaar, openheid, det_prob, det_prob_se) %>%
  distinct() %>%
  arrange(regio, jaar, openheid) %>%
  kable()
```
...
```{r}
plot_detection_curve(veldleeuwerik_dist_model, plot_average_fit = TRUE, 
                     n_breaks = 6)
```

```{r}
summary_veldleeuwerik_dist <- summary(veldleeuwerik_dist_model)
...
Het probleem ligt dus waarschijnlijk in het gedrag van de soort ***... (opvliegen etc. ?? uitleg??...)***.
@roodbergen2011 fitten daarom een detectiecurve m.b.v. afstandsintervallen.
