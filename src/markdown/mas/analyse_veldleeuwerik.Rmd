...
model_selection <- summarize_ds_models(
  ...
  veldleeuwerik_dist_hr17, veldleeuwerik_dist_hn18, veldleeuwerik_dist_hr18, 
  veldleeuwerik_dist_hn19, veldleeuwerik_dist_hr19, veldleeuwerik_dist_hn20, 
  veldleeuwerik_dist_hr20
  )
model_selection %>%
  kable()
```
...
```{r, cache=TRUE}
veldleeuwerik_dist_hr21 <- ds(data = veldleeuwerik_distance, key = "hr", 
...
summarize_ds_models(
  ...
  veldleeuwerik_dist_hr16, veldleeuwerik_dist_hr19,
  veldleeuwerik_dist_hr21, veldleeuwerik_dist_hr22
  ) %>%
  kable()
```
...
```{r}
par(mfrow = c(1, 2))
von_mises <- gof_ds(veldleeuwerik_dist_hr21)
von_mises
plot(veldleeuwerik_dist_hr21, pdf = TRUE, showpoints = FALSE)
par(mfrow = c(1, 1))
```
...
```{r}
plot_detection_curve(veldleeuwerik_dist_hr21, plot_average_fit = FALSE, 
                     n_breaks = 60)
```
...
```{r, cache=TRUE}
veldleeuwerik_dist_hr21_binned <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*jaar + openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
...
```{r}
plot(veldleeuwerik_dist_hr21_binned, pdf = TRUE, showpoints = FALSE)
```

```{r}
veldleeuwerik_dist_model <- veldleeuwerik_dist_hr21_binned
```
...
... vergelijking met max en gemiddelde over periodes ...
