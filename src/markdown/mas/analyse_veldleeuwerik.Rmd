...
De data voor GroeneSpecht, GroteBonteSpecht passen niet voor distance sampling.
...
De detectiekans voor Blauwborst, Bosrietzanger, Graspieper, Groenling, Haas, Holenduif, Roodborsttapuit, Torenvalk, Tuinfluiter en Veldleeuwerik neemt soms weer toe na een bepaalde afstand.
...
De volgende tabel geeft een overzicht van de soorten en modellen die gefit zijn.
...
```{r}
plot_densiteit <- function(dsmodel, soort, analyse) {
  ...
  if (soort == "Haas") {
    p + labs(x = "", y = "Aantal individuen per 100 ha",
           title = soort)
  } else {
    p + labs(x = "", y = "Aantal broedparen per 100 ha",
           title = soort)
  }
}
```
...
for (i in seq_along(final_models_update)) {
  ...
  p <- plot_densiteit(dsmodel = final_models_update[[i]], soort = titel, 
                      analyse = "2018_2022")
  print(p)
}
```
...
De data voor BruineKiekendief, Roek passen niet voor distance sampling.
Een belangrijke aanname voor distance sampling is dat de detectiekans afneemt over de afstand.
...
for (i in seq_along(final_models_2022_update)) {
  titel <- paste(unlist(strsplit(names(final_models_2022_update)[i], 
                 "(?<=[a-z])(?=[A-Z])", perl = TRUE)), collapse = " ")
  p <- plot_densiteit(dsmodel = final_models_2022_update[[i]], soort = titel, 
                      analyse = "2022")
  print(p)
}
```
