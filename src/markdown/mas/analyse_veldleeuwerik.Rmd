...
Deze data hebben wij voorlopig niet.
...
Tot we de data van de datumgrenzen hebben, houden we voorlopig alle telperiodes voor de analyses.
...
We beschouwen data van De Moeren en de Oostelijke leemstreek van 2018-2022 waarbij er in elk jaar 4 telperiodes waren.
Het aantal bezochte plots neemt over het algemeen toe over de jaren, behalve in De Moeren tussen 2019 en 2020.
In De Moeren neemt het aantal waarnemingen ook toe met het aantal bezochte plots maar in De Oostelijke leemstreek is dit niet altijd het geval, namelijk van 2018 naar 2019 en 2021 naar 2022.
...
Vooral in De Moeren zien we variatie in het gemiddelde aantal broedparen over de telperiodes zoals we ook al in \@ref(selectie-data) opmerkten.
...
1.  Dieren zijn uniform verdeeld over de ruimte (= random plaatsing van steekproefeenheden)
2.  Detectiekans is een functie van afstand en is gelijk aan 1 op afstand 0
3.  Individuen worden gedetecteerd op hun oorspronkelijke locatie (voorafgaand eventuele reactie op de waarnemer)
4.  Afstanden worden foutloos gemeten
...
Een punttransect is een cirkelvormige plot met radius $w$ waar een waarnemer vanuit een centraal punt de radiale afstand $r_i$ meet tot elk van de gedetecteerde individuen $i = 1, ..., n$ met $n$ het totaal aantal waarnemingen (Fig. \@ref(fig:chunk-name)).
$w$ wordt ook wel de afknotafstand genoemd.

```{r punttransect-plot, fig.cap = "Punttransect met radius $w$ en observatie op afstand $r_i$."}
...
We hebben een kolom `object` nodig: volgnummer voor elke waarneming; een kolom `size`: aantal broedparen per waarneming; een kolom `distance`: afstand tot waarnemer voor elke waarneming; kolommen met covariaten: `regio`, `jaar`, `sbp` en `openheid`.

```{r}
veldleeuwerik_distance <- veldleeuwerik_presences %>%
  st_drop_geometry() %>%
...
We trekken een buffer van 300 m rond het steekproefkader: half-open landschap in De Moeren, en open en half-open landschap in de Oostelijke leemstreek.
...
We definieren dus de strata regio-jaar.
We maken we dataframes om de abundanties en densiteiten te berekenen.
...
effort <- veldleeuwerik_presences %>%
  pull(periode_in_jaar) %>%
  unique() %>%
  length()

sample_table <- design_2018_2022 %>%
...
obs_table <- veldleeuwerik_presences %>%
  st_drop_geometry() %>%
  select(object = oid, Region.Label, Sample.Label = plotnaam)
```

Ten slotte zet de conversiefactor de afstanden om van meter naar vierkante kilometer.
...
```{r detectie-plot, fig.cap = "Histogram en detectiecurve gefit op 1000 gesimuleerde datapunten uit de half-normale verdeling met parameter $\sigma = 120$ en afknotafstand 300 m. De oppervlakte onder de curve binnen de rode box is de geschatte detectiekans."}
...
Voor de formulatie van de detectiefunctie beschouwen we half-normal en hazard-rate sleutelfuncties die we laten afhangen van covariaten regio, jaar, sbp en openheid.
...
Met $\mathbf{z_i}$ een vector van $J$ (hier $J=4$) covariaten geassocieerd met observatie $i$.
We beschouwen ook alle tweewegsinteracties.
...
We fitten alle sleutelfuncties met de `ds()` functie in een for-loop.
Niet alle interacties kunnen gefit worden omdat niet elke categorie van openheid en sbp in elke regio en/of jaar aanwezig is (opgevangen door `tryCatch()`).
...
# Null models
veldleeuwerik_dist_hn0 <- ds(data = veldleeuwerik_distance, key = "hn", 
...
veldleeuwerik_dist_hr0 <- ds(data = veldleeuwerik_distance, key = "hr", 
  truncation = 300, transect = "point", dht_group = FALSE,  
  convert_units = conversion_factor, region_table = region_table, 
  sample_table = sample_table, obs_table = obs_table)

# Sla namen van de gefitte models op
fitted_models <- c("veldleeuwerik_dist_hn0", "veldleeuwerik_dist_hr0")
...
for (i in seq_len(length(forms))) {
  ...
  for (k in keys) {
    ...
    tt <- tryCatch(
      ds(data = veldleeuwerik_distance, key = k, formula = as.formula(f), 
    ...
    # Naam voor model
    mod_name <- paste0("veldleeuwerik_dist_", k, i)
    ...
    if (is(tt, "error")) {
      ...
      if (file.exists(paste0(model_path, mod_name, ".rda"))) {
        load(paste0(model_path, mod_name, ".rda"))
      } else {
        ...
        save(mod_name, file = paste0(model_path, mod_name, ".rda"))
      }
...
```{r, cache=TRUE}
model_selection <- summarize_ds_models(veldleeuwerik_dist_hn0, veldleeuwerik_dist_hr0, 
  veldleeuwerik_dist_hn1, veldleeuwerik_dist_hr1, veldleeuwerik_dist_hn2, 
  veldleeuwerik_dist_hr2, veldleeuwerik_dist_hn3, veldleeuwerik_dist_hr3, 
  veldleeuwerik_dist_hn4, veldleeuwerik_dist_hr4, veldleeuwerik_dist_hn5, 
  veldleeuwerik_dist_hr5, veldleeuwerik_dist_hn6, veldleeuwerik_dist_hr6, 
  veldleeuwerik_dist_hn7, veldleeuwerik_dist_hr7, veldleeuwerik_dist_hn8, 
  veldleeuwerik_dist_hr8, veldleeuwerik_dist_hn9, veldleeuwerik_dist_hr9, 
  veldleeuwerik_dist_hn10, veldleeuwerik_dist_hr10, veldleeuwerik_dist_hn11, 
  veldleeuwerik_dist_hr11, veldleeuwerik_dist_hn12, veldleeuwerik_dist_hr12,
  veldleeuwerik_dist_hn13, veldleeuwerik_dist_hr13, veldleeuwerik_dist_hn14, 
  veldleeuwerik_dist_hr14, veldleeuwerik_dist_hn15, veldleeuwerik_dist_hr15, 
  veldleeuwerik_dist_hn16, veldleeuwerik_dist_hr16, veldleeuwerik_dist_hn17, 
  veldleeuwerik_dist_hr17, veldleeuwerik_dist_hn19, veldleeuwerik_dist_hr19, 
  veldleeuwerik_dist_hn20, veldleeuwerik_dist_hr20, veldleeuwerik_dist_hn21, 
  veldleeuwerik_dist_hr21) 
model_selection %>%
  kable()
```

```{r}
test1 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*jaar + openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)

test3 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~openheid*jaar + regio, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)
test4 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*jaar + openheid + sbp, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)
test5 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*sbp + openheid + jaar, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)
test6 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~sbp*jaar + openheid + regio, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)

test9 <- ds(data = veldleeuwerik_distance, key = "hr", 
...
summarize_ds_models(veldleeuwerik_dist_hn0, veldleeuwerik_dist_hr12, test1, test3, test4, test5, test6,
                    test9)
```

## Model fit {#fit-distance}

Om te controleren of ons model goed past bij de data ("goodness of fit") maken we een Q-Q plot waarbij de cumulatieve distributiefunctie van de gefitte detectiefunctie wordt vergeleken met de distributie van de data

```{r}
breaks <- seq(0, 300, 300 / 15)

par(mfrow = c(1, 2))
gof_ds(veldleeuwerik_dist_hn0, main = "veldleeuwerik_dist_hn0")

plot(veldleeuwerik_dist_hn0, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     main = paste0("veldleeuwerik_dist_hn0", " - ", "AIC: ",
                   as.character(round(summary(veldleeuwerik_dist_hn0)$ds$aic, digits = 2))
                   )
     )
...
par(mfrow = c(1, 2))
gof_ds(test6, main = "test6")

plot(test6, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     main = paste0("test6", " - ", "AIC: ",
                   as.character(round(summary(test6)$ds$aic, digits = 2))
                   )
     )
par(mfrow = c(1, 1))
```
...
We hebben de studieregio in 2 strata opgesplits: De Moeren en de Oostelijke leemstreek.
...
In ons geval hebben we dus $a(s) = \pi 300^2 n_p 4$.
...
De abundanties kunnen ook opgeteld worden voor een totale schatting en een totale densiteit door de totale schatting te delen door de totale oppervlakte (som van oppervlaktes per stratum).
Voor berekening van de confidence limits zie `?dht`.
De confidence limits in de volgende resultaten zijn 95 % confidence intervallen.

```{r}
cbind(veldleeuwerik_dist_hn0$ddf$data, 
      det_prob = predict(veldleeuwerik_dist_hn0, se.fit = TRUE)$fitted, 
      det_prob_se = predict(veldleeuwerik_dist_hn0, se.fit = TRUE)$se) %>%
  select(regio, jaar, det_prob, det_prob_se) %>%
  distinct() %>%
  arrange(regio, jaar) %>%
  kable()
```
...
```{r}
plot_detection_curve(veldleeuwerik_dist_hn0)
plot_detection_curve(veldleeuwerik_dist_hn0, plot_average_fit = FALSE, n_breaks = 60)


plot_detection_curve(veldleeuwerik_dist_hn0, n_breaks = 60)
plot_detection_curve(test6, n_breaks = 60)
plot_detection_curve(test6)
```

```{r}
summary_veldleeuwerik_dist_hn0 <- summary(veldleeuwerik_dist_hn0)

summary_results_veldleeuwerik_dist_hn0 <- bind_rows(
  summary_veldleeuwerik_dist_hn0$dht$individuals$D %>%
    mutate(variable = "density",
           type = "Aantal broedparen per 100 ha"),
  summary_veldleeuwerik_dist_hn0$dht$individuals$N %>%
...
```{r}
summary_results_veldleeuwerik_dist_hn0 %>%
...
We visualiseren de absolute verschillen in densiteit over de jaren.
