...
stratum_cijfers_lijst_2022 <- lapply(seq_along(distance_dfs_2022_final), 
  function(i) {
    out <- distance_dfs_2022_final[[i]] %>% 
      ...
      group_by(sbp) %>%
      mutate(n_interactie = n_distinct(openheid)) %>%
      ungroup() %>%
      mutate(n_int_openheid_sbp = !(min(n_interactie) == 1 | n_sbp == FALSE)) %>%
      mutate(n_int_regio_sbp = ifelse(n_regio == TRUE & n_sbp == TRUE, TRUE, FALSE)) %>%
...
# Verwijder BruineKiekendief
index_to_remove <- which(names(distance_dfs_2022_final) %in% c("Buizerd", "Gaai", "GroeneSpecht", "Kauw", "Koolmees", "Spreeuw", "Tjiftjaf", "Winterkoning", "Zanglijster", "ZwarteRoodstaart"))

# Loop over species
for (spec in names(distance_dfs_2022_final)[-index_to_remove]) {
