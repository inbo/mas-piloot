...
Om te hoge flexibiliteit van detectiefunctiemodellen te beperken, bijvoorbeeld in aanwezigheid van overdispersie [@buckland2015], beperken we het aantal aanpassingstermen in afwezigheid van covariaten tot maximaal 2.
...
# Null models
veldleeuwerik_dist_hr12 <- ds(data = veldleeuwerik_distance, key = "hn", 
...
fitted_models <- c("veldleeuwerik_dist_hr12", "veldleeuwerik_dist_hr0")
...
```{r, cache=TRUE}
model_selection <- summarize_ds_models(veldleeuwerik_dist_hr12, veldleeuwerik_dist_hr0, 
...
Het model waarbij de detectiekans afhankelijk is van ... scoort het best. Op basis van dit model testen we nog enkele combinaties ...
...
par(mfrow = c(1, 2))
gof_ds(veldleeuwerik_dist_hr12, main = "veldleeuwerik_dist_hr12")

plot(veldleeuwerik_dist_hr12, pdf = TRUE, showpoints = FALSE, breaks = breaks,
     main = paste0("veldleeuwerik_dist_hr12", " - ", "AIC: ",
                   as.character(round(summary(veldleeuwerik_dist_hr12)$ds$aic, digits = 2))
                   )
     )
par(mfrow = c(1, 1))
```

De fit ziet er goed uit maar Cramér-von Mises test A significant result from either test gives evidence against the null hypothesis (that the data arose from the fitted model), suggesting that the model does not fit the data well.
[@buckland2015]

```{r}
plot_detection_curve(veldleeuwerik_dist_hr12)
```

```{r}
plot_detection_curve(veldleeuwerik_dist_hr12, plot_average_fit = FALSE, n_breaks = 60)
```
...
```{r}
cbind(veldleeuwerik_dist_hr12$ddf$data, 
      det_prob = predict(veldleeuwerik_dist_hr12, se.fit = TRUE)$fitted, 
      det_prob_se = predict(veldleeuwerik_dist_hr12, se.fit = TRUE)$se) %>%
...
```{r}
summary_veldleeuwerik_dist <- summary(veldleeuwerik_dist_hr12)
...
ook problemen SOVON, binning hielp niet en zorgde voor gelijkaardige resultaten...toch niet super slecht waarschijnlijk
