...
design_2018_2022_reduced <- design_2018_2022 %>%
  filter(plotnaam %in% plots_old)
```
...
region_table <- design_2018_2022_reduced %>%
  distinct(regio, jaar) %>%
...
sample_table <- design_2018_2022_reduced %>%
...
out_models <- list(veldleeuwerik_dist_2018_2022_hn0, 
                   veldleeuwerik_dist_2018_2022_hr0)
model_names <- c("veldleeuwerik_dist_2018_2022_hn0", 
                 "veldleeuwerik_dist_2018_2022_hr0")
...
for (i in seq_len(length(forms))) {
  ...
  for (k in keys) {
    # Naam voor model
    mod_name <- paste0("veldleeuwerik_dist_2018_2022_", k, i)
...
Op basis van dit model testen we nog enkele combinaties waarbij we interacties tussen elke combinatie van de variabelen toevoegen (niet regio\*openheid).

```{r, cache=TRUE}
veldleeuwerik_dist_2018_2022_hr21 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*jaar + openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)

veldleeuwerik_dist_2018_2022_hr25 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio + jaar*openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)

veldleeuwerik_dist_2018_2022_hr23 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*jaar + openheid*sbp, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)

veldleeuwerik_dist_2018_2022_hr24 <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio*sbp + jaar*openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
        convert_units = conversion_factor, region_table = region_table, 
        sample_table = sample_table, obs_table = obs_table)
...
summarize_ds_models2(
  veldleeuwerik_dist_2018_2022_hr12, veldleeuwerik_dist_2018_2022_hr15,
  veldleeuwerik_dist_2018_2022_hr21, veldleeuwerik_dist_2018_2022_hr25, 
  veldleeuwerik_dist_2018_2022_hr23, veldleeuwerik_dist_2018_2022_hr24,
  veldleeuwerik_dist_2018_2022_hr25
  ) %>%
  kable()
```

We selecteren het Hazard-rate model waarbij de detectiekans afhankelijk is van regio, sbp, jaar en openheid met sbp en jaar in interactie.
...
```{r}
par(mfrow = c(1, 2))
von_mises <- gof_ds(veldleeuwerik_dist_2018_2022_hr25)
von_mises
plot(veldleeuwerik_dist_2018_2022_hr25, pdf = TRUE, showpoints = FALSE)
par(mfrow = c(1, 1))
```
...
```{r}
plot_detection_curve(veldleeuwerik_dist_2018_2022_hr25, plot_average_fit = FALSE, 
                     n_breaks = 60)
```
...
```{r, cache=TRUE}
veldleeuwerik_dist_2018_2022_hr25_binned <- ds(data = veldleeuwerik_distance, key = "hr", 
        formula = ~regio + sbp*jaar + openheid, adjustment = NULL, truncation = 300, 
        transect = "point", dht_group = FALSE, 
...
```{r}
plot(veldleeuwerik_dist_2018_2022_hr25_binned, pdf = TRUE, showpoints = FALSE)
```
...
```{r}
veldleeuwerik_dist_2018_2022_model <- veldleeuwerik_dist_2018_2022_hr25_binned
veldleeuwerik_covars <- c("regio", "sbp", "jaar", "openheid")
```
...
```{r}
plot_detection_curve(veldleeuwerik_dist_2018_2022_model, 
...
plots_per_regiojaar <- design_2018_2022_reduced %>%
  group_by(regio, jaar) %>%
  summarise(aantal_plots = n_distinct(plotnaam)) %>%
  mutate(Region.Label = paste(regio, jaar, sep = " - "))
