...
We voegen ook Haas toe (geen broedcodes).
...
mas_observations <- mas_clean %>%
  ...
  mutate(Region.Label = paste(regio, jaar, sep = " - "),
         ...
         naam = ifelse(naam == "Witte Kwikstaart (spec)", "Witte Kwikstaart",
                       naam))
haas_observations <- mas_clean %>%
  filter(naam == "Haas", distance2plot <= 300, !is.na(periode_in_jaar)) %>%  
  inner_join(design_2018_2022_mas, by = c("plotnaam", "jaar")) %>%
  mutate(Region.Label = paste(regio, jaar, sep = " - "))

mas_observations <- bind_rows(mas_observations, haas_observations)
```

## Genereer dataframes voor elke soort

We maken in een for-loop een dataframe voor elke soort waar we ook filteren op de datumgrenzen. Voor Haas behouden we alle telperiodes.
