...
```{r}
spec_to_remove <- c("GroeneSpecht", "GroteBonteSpecht")
```
...
We hebben de densiteiten kunnen schatten van `r length(final_models_update)` soorten.
...
```{r}
spec_to_remove <- c("BruineKiekendief", "Roek")
```
...
We hebben de densiteiten kunnen schatten van `r length(final_models_2022_update)` soorten.
