...
***geef gemiddelde detectiekans*** $\hat{P}_a$
Visualisatie: te veel om alle te tonen ...
...
```{r}
dens_plot <- summary_results_veldleeuwerik_dist %>%
  filter(variable == "density") %>%
...
comparison_plots <- veldleeuwerik_df %>%
  ...
  mutate(tot_gemiddelde_plots = tot_gemiddelde / aantal_plots,
         tot_max_plots = tot_max / aantal_plots) %>%
  ...
  ggplot(aes(x = jaar, y = Estimate)) + 
    ...
    scale_y_continuous(sec.axis = sec_axis( ~. / cirkelopp * 1e6, 
         name = "Aantal broedparen Veldleeuwerik per 100 ha")) +
    labs(x = "", y = "Aantal broedparen Veldleeuwerik per plot") +
    theme(legend.position = "") +
...
grid.arrange(dens_plot, comparison_plots, nrow = 2)
```
