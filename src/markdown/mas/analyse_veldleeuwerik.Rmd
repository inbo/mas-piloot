...
Voor Bruine kiekendief kunnen we geen model fitten.
...
Voor Buizerd en Winterkoning lukt het niet om ~regio+openheid en ~regio+sbp+openheid te fitten.
...
# Verwijder
#index_to_remove <- which(names(distance_dfs_2022_final) %in% c("Winterkoning"))
#[-index_to_remove]
...
```{r}
final_models_complex <- list(Blauwborst = Blauwborst_distmod_2018_2022_hn1,
...
plot_densiteit <- function(dsmodel, soort, show_data = TRUE) {
  ...
  if (soort == "Haas") {
    p <- p + labs(x = "", y = "Aantal individuen per 100 ha", title = soort)
  } else {
