...
distance_dfs <- lapply(df_obs_list, function(df) {
    df %>%
      filter(plotnaam %in% plots_old) %>% 
      select(object = oid, size = aantal, distance = distance2plot, 
             regio, jaar) %>%
...
```{r}
efforts <- lapply(df_obs_list, function(df) {
    df %>%
      pull(periode_in_jaar) %>%
      unique() %>%
      length()
...
sample_tables <- lapply(efforts, function(effort) {
...
```{r}
obs_tables <- lapply(df_obs_list, function(df) {
    df %>%
      filter(plotnaam %in% plots_old) %>% 
      select(object = oid, Region.Label, Sample.Label = plotnaam)
  })
