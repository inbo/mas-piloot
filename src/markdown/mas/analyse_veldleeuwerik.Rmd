...
We maken een dataframe `bezoekenlijst_2018_2022_mas` waarbij we alle telperiodes toevoegen aan de bezochte plots.

```{r}
bezoekenlijst_2018_2022_mas <- design_2018_2022_mas %>% 
  expand_grid(distinct(mas_clean, periode_in_jaar)) %>%
  filter(!is.na(periode_in_jaar)) %>%
  arrange(plotnaam, jaar, periode_in_jaar)
```
