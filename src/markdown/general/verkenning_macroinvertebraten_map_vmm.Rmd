...
- minstens 4 metingen beschikbaar (per meetpunt, variabele, winterjaar)
...
nitraat_data %>%
  unnest(data) %>%
  ggplot() +
  geom_line(aes(x = jaar, y = `gemiddelde_NO3-`,
...
We maken een samengestelde dataset door de biotische gegevens en de fysico-chemische gegevens aan elkaar te koppelen op basis van meetplaats en jaar (winterjaar).
...
somdata_combined <- selectie_gewasgroep %>%
  select(meetplaats, Grasland, `Granen, zaden en peulvruchten`, Suikerbieten,
...
som_results %>%
  ...
  ggplot() +
  geom_line(aes(x = name, y = value, group = meetplaats,
                colour = factor(group))) +
  coord_flip()
...
Groepen 1, 4 en 5 selecteren (n = 50)?

```{r}
watersheds_selectie %>%
  filter(group %in% c(1, 4, 5)) %>%
  mapview(zcol = "fgroup")
