...
```{r hydrodtm}
dtm_hydro <- rast(here("data", "dem", "DHMVIIDTMRAS025mto100m_breachedDTM.tif"))
```

```{r wbt-d8, eval=file.exists(here("data", "dem", "DHMVIIDTMRAS025mto100m_breachedDTM.tif"))}
# flow accumulation
#qgis_arguments("wbt:D8FlowAccumulation")
d8_flow <- qgis_run_algorithm(
  "wbt:D8FlowAccumulation",
  input = here("data", "dem", "DHMVIIDTMRAS025mto100m_breachedDTM.tif"),
  output = here("data", "dem", "dhmvii_dtm_100m_d8_flow.tif"))

# d8 pointer
#qgis_arguments("wbt:D8Pointer")
d8_pointer <- qgis_run_algorithm(
  "wbt:D8Pointer",
  dem = here("data", "dem", "DHMVIIDTMRAS025mto100m_breachedDTM.tif"),
  output = here("data", "dem", "dhmvii_dtm_100m_d8_pointer.tif"))
```
...
streams <- qgis_run_algorithm(
  "wbt:ExtractStreams",
  flow_accum = here("data", "dem", "dhmvii_dtm_100m_d8_flow.tif"),
  threshold = threshold,
  output = here("data", "dem", paste0("dhmvii_dtm_100m_streams_t",
                                              threshold,".tif")))

#plot(streams, maxcell = 2500000, col = "black")
# setting pour points
...
snapped <- qgis_run_algorithm(
  ...
  streams = here("data", "dem", paste0("dhmvii_dtm_100m_streams_t",
                                              threshold,".tif")),
  snap_dist = 300,
...
watersheds <- qgis_run_algorithm(
  "wbt:UnnestBasins",
  d8_pntr = here("data", "dem", "dhmvii_dtm_100m_d8_pointer.tif"),
  pour_pts =
...
watersheds_4 <- rast(
  here("data", "vmm", "vmm_macroinvertebraten_watersheds_4.tif")) %>%
  as.polygons() %>%
  st_as_sf() %>%
  rename(rowname = vmm_macroinvertebraten_watersheds_4)
...
ws <- watersheds %>%
  slice(rownr)
tt <- rast(here("data", "dem", "dhmvii_dtm_100m_d8_pointer.tif")) %>%
...
round(exp(c(mm - 0.5 * sd, mm + 0.5 * sd)), -1)
exp(mm)
```

Focus op kleinere afstroomgebieden? Tussen 150 en 600 ha (300 is geometrisch gemiddelde)?
