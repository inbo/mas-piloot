...
```{r}
# Select Oostelijke leemstreek
steekproef_piloot_ol_raw <- steekproef_piloot %>%
  filter(Naam == "Oostelijke leemstreek")
```
...
```{r}
steekproef_piloot_ol <- steekproef_piloot_ol_raw %>%
  select(pointid, sample_order, batch) %>%
  left_join(steekproefkader_ol, by = join_by(pointid))

head(steekproef_piloot_ol) %>%
  kable(digits = 2)
```

### Finale steekproef na manuele controle

Dit zijn de punten die effectief zijn geteld en in avimap ingevoerd.
