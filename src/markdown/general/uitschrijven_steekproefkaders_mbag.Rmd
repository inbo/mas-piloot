...
> vervangen door VL0056

```{r}
final_points_ol <- c(final_points_ol, "VL0056")
```
...
> vervangen door VL0555

Ol_6358.22

> vervangen door VL0199

Ol_9621.5

> vervangen door VL0294
...
```{r}
final_points_ol <- unique(
  c(final_points_ol, "VL0555", "VL0199", "VL0294", "VL0294", "VL0199")
)
```
...
> vervangen door VL0141

Ol_3041.8

> vervangen door VL0154

Ol_3242.2

> niet vervangen, behouden

```{r}
final_points_ol <- c(final_points_ol, "VL0141", "VL0154")
```
...
Verdere controle en overleg toonde nog twee punten die niet geteld worden en moeten vervangen worden:

Ol_815.5

> vervangen door VL0322
...
```{r}
final_points_ol <- unique(c(final_points_ol, "VL0322", "VL0555"))
final_points_ol <- final_points_ol[-which(final_points_ol %in%
                                            c("Ol_815.5", "Ol_5760.2.14"))]
```
...
We hebben enkele probleem gevallen:

Zn_16017.18

> vervangen door VL0614

Zn_91647.2

> vervangen door VL0612
...
```{r}
final_points_zn <- sort(plotnaam_zn)

final_points_zn <- unique(
  c(final_points_zn, "VL0614", "VL0612", "VL0617", "VL0608", "VL0611", "VL0616")
)
```
...
```{r}
zandleemstreek <- st_read(
  path_to_perimeters("zandleemstreek_perimeters.gpkg")
) %>%
  filter(Naam == "Zandleemstreek")

# We need to calculate SBP variable for the old points
old_points_zn_avimap_sf2 <- add_stratum_sbp(
    punten_sf = old_points_ol_avimap_sf,
    sbp = sbp_akkervogels
) %>%
  st_join(zandleemstreek) %>%
  filter(!is.na(Naam)) %>%
  select(-"regio") %>%
  rename("regio" = "Naam")

# We need to calculate openheid variable for the old points
old_points_zn_sbp <- add_openheid_landschap_to_frame(
      path = path_to_openheid_landschap(),
      punten_sf = old_points_zn_avimap_sf2,
      gebied = zandleemstreek,
      cutlevels = c(1.25, 1.35, 1.51),
      class_labels = c("GL", "HGL", "HOL", "OL")
      )

old_points_zn_avimap <- old_points_zn_sbp %>%
  mutate(x_coord = st_coordinates(geometry)[,1],
         y_coord = st_coordinates(geometry)[,2]) %>%
  st_drop_geometry() %>%
  mutate(sbp = ifelse(is_sbp, "binnen", "buiten"),
...
old_points_zn_avimap %>%
  write_csv(file.path(data_path, "old_points_zn_strata.csv"))
```

We voegen deze toe aan de andere punten om tot een finale dataset te komen.

```{r}
old_points_to_add_zn <- old_points_zn_avimap %>%
  filter(pointid %in% final_points_zn) %>%
  left_join(beslissingen_ol %>% select(-"pointid"),
            by = join_by("pointid" == "vervanging_id")) %>%
  mutate(area_prop_sb = NA,
         sample_order = as.numeric(sample_order)) %>%
...
steekproef_ol_avimap <- steekproef_zn %>%
  filter(pointid %in% final_points_zn) %>%
  bind_rows(old_points_to_add_zn)
