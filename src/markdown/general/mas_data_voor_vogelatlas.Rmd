...
We voegen de coördinaten toe van de telpunten.
...
mbag_controle_afstand <- mbag_telperiodes %>%
  ...
  inner_join(mbag_design %>% as_tibble() %>% distinct(plotnaam, regio, geometry),
...
```{r}
mbag_piloot_atlas <- mbag_distance %>%
  st_drop_geometry() %>%
  group_by(plotnaam, jaar, periode_in_jaar, naam) %>%
  mutate(totaal = sum(aantal)) %>%
  group_by(plotnaam, jaar, naam, x_plot, y_plot) %>%
...
```{r}
mbag_piloot_atlas_sf <- st_as_sf(mbag_piloot_atlas,
                                coords = c("x_plot", "y_plot"),
                                crs = 31370) %>%
  st_buffer(300)
```
