...
Onderscheid maken tussen OL, HOL en KL (open landschap, half-open landschap en kleinschalig landschap)?

Mogelijke bronnen:

- https://www.geopunt.be/catalogus/datasetfolder/2c64ca0c-5053-4a66-afac-24d69b1a09e7
    - laag groen / hoog groen / landbouw / overige
    - laatste jaar: 2018
- DTM en DSM (digitaal terrein en oppervlakte model) waaruit 'canopy' height model (CHM) kan afgeleid worden
- https://land.copernicus.eu/pan-european/high-resolution-layers/small-woody-features
    - voorlopig enkel voor 2015

DTM / DSM op 20 m resolutie.
...
Bij het CHM is 'canopy' ruim te interpreteren want inclusief gebouwen etc.
Dit is OK voor de typering van landschap.
Een landschap vol lintbebouwing kunnen we moeilijk als OL classificeren.
...
leemstreek_dsm20 <- get_coverage_wcs(
  wcs = "dsm",
  bbox = bbox,
  layername = "EL.GridCoverage.DSM",
  resolution = 20)

leemstreek_chm20 <- leemstreek_dsm20 - leemstreek_dtm20
```

```{r}
terra::plot(leemstreek_dtm20)
terra::plot(leemstreek_dsm20)
terra::plot(leemstreek_chm20)
```

```{r}
circle1000 <- focalMat(leemstreek_chm20, d = 1000, type = "circle")
leemstreek_chm20_c1000_sd <- terra::focal(
  leemstreek_chm20,
  w = circle1000,
  fun = "sd")
...
Onderstaande figuren geven eerst de standaarddeviatie van het CHM in een straal van 1000m rond elke rastercel.
De tweede figuur classificeert deze standaarddeviaties in drie klassen die we kunnen interpreteren als 1 = OL, 2 = HOL, 3 = KL.

```{r}
plot(leemstreek_chm20_c1000_sd)
rclasmat <- c(0, 0.0002, 1,
              0.0002, 0.0006, 2,
              0.0006, +Inf, 3) %>%
  matrix(ncol = 3, byrow = TRUE)
leemstreek_landschapstype <- terra::classify(
  leemstreek_chm20_c1000_sd,
  rcl = rclasmat,
  include.lowest = TRUE)
...
```{r landschapstype, warning=FALSE}
my_palette <- RColorBrewer::brewer.pal(n = 3, name = "Dark2")
leemstreek_landschapstype_raster <- raster::raster(leemstreek_landschapstype)
leemstreek_landschapstype_raster <- raster::ratify(leemstreek_landschapstype_raster)
levels(leemstreek_landschapstype_raster)[[1]]$LT <- c("OL", "HOL", "KL")

leemstreek_landschapstype_raster %>%
  mapview(col.regions = my_palette, att = "LT")
```
...
```{r eval=!file.exists(here("data", "groenkaart", "groenkaart2018_leemstreek.tif"))}
groenkaart2018 <- rast(
  file.path("S:", "Vlaanderen", "Natuur_Bos", "Boswijzer_Groenkaart",
            "Groenkaart_2018.tif"))
groenkaart2018 <- crop(groenkaart2018, leemstreek_pk)
terra::writeRaster(groenkaart2018, here("data", "groenkaart",
                                        "groenkaart2018_leemstreek.tif"))
```

```{r eval=file.exists(here("data", "groenkaart", "groenkaart2018_leemstreek.tif"))}
groenkaart2018 <- rast(here("data", "groenkaart",
                                        "groenkaart2018_leemstreek.tif"))

hlg2018 <- segregate(groenkaart2018, classes = c(1, 2))
names(hlg2018) <- c("hoog groen", "laag groen")
```

- laag groen (minder dan 3 m)
- hoog groen (meer dan 3 m)
...
Hoe OL, HOL, KL bepalen? Voorstel:

- in ruimere buffer van 1000 m rond elk potentieel telpunt
    - het oppervlakte-aandeel hoog groen bepalen?
    - de perimeter van hoog groen bepalen?
    - gemiddelde en standaardeviatie hoogte in het CHM model bepalen?

Een OL karakteriseert zich dan door:

- laag oppervlakte-aandeel hoog groen
- lage perimeter hoog groen
- lage perimeter / oppervlakte verhouding
- lage gemiddelde hoogte en lage standaardeviatie in hoogte van het CHM model

(Perceelsgrootte speelt hierin geen rol. Een OL kan zowel relatief kleinschalige perceelsgrootte hebben als grootschalige perceelsgrootte).
...
- leemstreek: 240 punten per jaar?
