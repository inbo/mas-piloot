...
```{r trekking}
samplesize <- 300
popsize <- nrow(leemstreek_point)
inclusiekansen <- rep(samplesize / popsize, popsize)
...
leemstreek_point %>%
  ...
  group_by(pk) %>%
  nest() %>%
  mutate(samplesize = 300,
         popsize = map_dbl(data, nrow),
         inclusiekansen = map2(samplesize, popsize, ~rep(.x / .y, .y)),
         sample = pmap_df(.l = list(data = data,
                                    samplesize = samplesize,
                                    inclusiekansen = inclusiekansen),
                          .f = ~draw_sample(
                            sampling_frame = data,
...
sample %>%
  st_buffer(dist = 300) %>%
  mutate(batch = ifelse(sample_order > 300, "eerste 300", "volgende 300")) %>%
  mapview(zcol = "batch", burst = TRUE) +
  mapview(plan_kiekendief_perimeter)
