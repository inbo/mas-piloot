...
We maken gebruik van het DSM (digitaal oppervlakte model - DHMV II Vlaanderen) op 5 m resolutie.
Werkwijze:

- berekening van "positive topographic openness" (dit is een waarde tussen 0 en 2 in radians) door voor elke rastercel in 8 kijkrichtingen na te gaan wat de gemiddelde hoek is t.o.v. zenith binnen een straal van 300m. Deze hoek geeft aan hoe plat het landschap is. Indien volledig plat is de hoek 90° (of 1.57 radians).
...
leemstreek_openheid <- rast(
  here("data", "dem", "vlaanderen_openheid_landschap.tif")) %>%
  crop(leemstreek_pk)
```
...
leemstreek_openheid_raster <- raster::raster(leemstreek_openheid)
leemstreek_openheid_raster <- raster::ratify(leemstreek_openheid_raster)
levels(leemstreek_openheid_raster)[[1]]$LO <- c("GL", "HOL", "OL")

leemstreek_openheid_raster %>%
  mapview(col.regions = my_palette, att = "LO", alpha = 0.5)
```

```{r openheid-points}
openheid_points <- terra::extract(
  x = leemstreek_openheid,
  y = vect(leemstreek_point))

leemstreek_point <- leemstreek_point %>%
  bind_cols(openheid_points %>% select(openheid = focal_sum)) %>%
  mutate(openheid = factor(openheid,
                           levels = 1:3, labels = c("GL", "HOL", "OL")))
```
...
```{r stratum-pk}
leemstreek_point <- leemstreek_point %>%
  mutate(pk = st_intersects(.,
                            plan_kiekendief_perimeter %>% st_union(),
                            sparse = FALSE) %>%
           as.logical()
         )
```
...
allocatie <- leemstreek_point %>%
  st_drop_geometry() %>%
  count(openheid, pk, name = "popsize") %>%
  mutate(allocatie_factor_pk = ifelse(pk, 0.5, 0.5)) %>%
  group_by(openheid) %>%
  mutate(allocatie_factor_openheid = sum(popsize) / nrow(leemstreek_point)) %>%
  ungroup() %>%
  mutate(allocatie = allocatie_factor_pk * allocatie_factor_openheid) %>%
  filter(allocatie > 0.1) %>%
  mutate(allocatie = allocatie / sum(allocatie),
         samplesize = round(allocatie * samplesize))

leemstreek_point %>%
  select(-key, -value) %>%
  inner_join(allocatie %>% select(openheid, pk, samplesize),
             by = c("openheid", "pk")) %>%
  group_by(pk, openheid, samplesize) %>%
  nest() %>%
  ...
  select(openheid, pk, sample, samplesize) %>%
  ungroup() %>%
  unnest(sample) %>%
  st_as_sf() -> samples
```
...
samples %>%
  st_buffer(dist = 300) %>%
  mutate(openheid_pk = paste(openheid,
                             ifelse(pk, "binnen plan", "buiten plan"))
         ) %>%
  mapview(zcol = "openheid_pk", burst = TRUE) +
