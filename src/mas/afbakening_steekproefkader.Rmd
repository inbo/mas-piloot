...
We maken een overlay van de kaart met openheid van het landschap en de kaart "hoog groen".
We zien dat (zoals te verwachten) er meer hoog groen te zien is in OL < HOL < GL.
Bij de definitie van openheid, spelen echter ook andere structuren zoals gebouwen een rol.
Misschien kunnen we dus
...
allocatie <- leemstreek_point %>%
  st_drop_geometry() %>%
  count(openheid, sbp_akkervogels, name = "popsize") %>%
...
leemstreek_point %>%
  select(-key, -value) %>%
  inner_join(allocatie %>% select(openheid, sbp_akkervogels, samplesize),
             by = c("openheid", "sbp_akkervogels")) %>%
  group_by(sbp_akkervogels, openheid, samplesize) %>%
  nest() %>%
  ...
  select(openheid, sbp_akkervogels, sample, samplesize) %>%
  ungroup() %>%
  unnest(sample) %>%
  st_as_sf() -> samples
```
...
samples %>%
  st_buffer(dist = 300) %>%
  mutate(openheid_sbp = paste(openheid,
