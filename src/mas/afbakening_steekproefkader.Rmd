...
e40_snap <- e40_union %>%
  st_snap(y = e40_union, tolerance = 20) %>%
...
buffer1200 %>%
  ...
  slice(2) -> buffer1200_onder_e40

perimeters %>%
  filter(Naam == "Oostelijke leemstreek") %>%
  bind_rows(buffer1200_onder_e40) -> nieuwe_perimeters

rownames(nieuwe_perimeters) <- 1:length(nieuwe_perimeters)
...
steekproef <- tar_read(steekproef, store = targets_store) %>%
  ...
  mutate(sample_batch = ifelse(sample_order < max(sample_order) / 2,
                               "eerste batch",
                               "reserve batch"))
```
...
steekproef %>%
  filter(sample_batch == "eerste batch") %>%
  st_buffer(dist = 300) %>%
...
balansvergelijking <- steekproef %>%
  ...
  mutate(fillvar = ifelse(!is.na(sample_batch),
                          paste("steekproef", sample_batch),
                          puntenset))
...
maspunt_binnen_300m %>%
  ...
  gekoppelde_nieuwe %>%
  st_buffer(dist = 300) %>%
  mapview(zcol = "sample_batch", col.region = c("green", "orange"))
```
