...
tellingen_haas %>%
  st_drop_geometry() %>%
  distinct(plotid, rondenr, doy, jaar, plotid) %>%
  ggplot(aes(x = rondenr, y = doy, colour  = factor(jaar))) + 
  geom_point() +
...
De schatting is 1.35.
Dit is een verschil in de log-schaal, dus exp(1.35) = 3.86 keer meer hazen per 100 ha in de Moeren (in 2021 vroeg op het seizoen).
