...
```{r calc-trend-aup, eval = !file.exists("data/processed/trend_aup.csv")}
shapefiles <- fs::dir_ls("data/landbouwgebruikspercelen/Shapefile", 
                         glob = "*.shp")
# sort shapefiles
years <- stringr::str_extract(shapefiles, "\\d+")
...
```{r read-trend-aup, eval = file.exists("data/processed/trend_aup.csv")}
...
lm_result %>%
  ...
  head(n = 20) %>%
  kable(caption = "Top 20 van grootste dalers and stijgers volgens een eenvoudig lineair model (minstens 8 jaar data).")
```
...
lm_result %>%
  ...
  head(n = 20) %>%
  select(-gewasgroep) %>%
  kable(caption = "Top 20 van grootste jaarlijkse procentuele verandering volgens een eenvoudig lineair model (minstens 8 jaar data).")
```
