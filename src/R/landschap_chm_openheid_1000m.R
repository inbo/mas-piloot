...
result <- qgis_run_algorithm(
  ...
  DEM = vlaanderen_chm5,
  POS = here::here("data", "dem", "openness1000m_chm_res5_vlaanderen.tif"),
  RADIUS = 1000,
  METHOD = 1,
  DLEVEL = 3,
  NDIRS = 8,
...
top_open_1000m <- terra::rast(
  here::here("data", "dem", "openness1000m_chm_res5_vlaanderen.tif"))

# aggregate to 25 m resolution
top_open_1000m_res25 <- aggregate(
  top_open_1000m, fact = 5,
  filename = here::here("data", "dem", "openness1000m_chm_res25_vlaanderen.tif"))

top_open_1000m_res25 <-  terra::rast(
  here::here("data", "dem", "openness1000m_chm_res25_vlaanderen.tif"))

# next calculated average openness in 1000m radius landscape
circle1000 <- focalMat(top_open_1000m_res25,
                       d = 1000,
                       type = "circle",
                       fillNA = TRUE)


openness1000m_chm_res25_c1000_mean_vlaanderen <- terra::focal(
  top_open_1000m_res25,
  w = circle1000,
  fun = "sum",
  na.rm = TRUE,
...
openness1000m_chm_res25_c1000_mean_vlaanderen <-
  rast(
    here("data", "dem",
         "openness1000m_chm_res25_c1000_mean_vlaanderen.tif")
  )
