...
akkervogelgebieden <- read_sf("data/bo_vlm/VLM_AF_PDPOIII.shp")
...
result <- qgis_run_algorithm(
  ...
  area_units = "m2",
  DEM = vlaanderen_dsm5,
  POS = here::here("data", "dem", "vlaanderen_postopopen300m.tif"),
  RADIUS = 300,
  METHOD = 1,
  DLEVEL = 3,
  NDIRS = 8,
...
top_open_300m <- terra::rast(
  here::here("data", "dem", "vlaanderen_postopopen300m.tif"))

# aggregate to 25 m resolution
top_open_300m_res25 <- aggregate(
  top_open_300m, fact = 5,
  filename = here::here("data", "dem", "vlaanderen_postopopen300m_res25.tif"))

top_open_300m_res25 <-  terra::rast(
  here::here("data", "dem", "vlaanderen_postopopen300m_res25.tif"))
...
vlaanderen_top_open_300m_c1000_mean_res25 <- terra::focal(
  ...
  fun = "sum",
  na.rm = TRUE,
  filename = "data/dem/vlaanderen_postopopen300m_c1000_mean.tif",
  overwrite = TRUE)
...
vlaanderen_openheid_landschap <- terra::classify(
  ...
  rcl = rclasmat,
  include.lowest = TRUE,
  filename = "data/dem/vlaanderen_openheid_landschap.tif",
  overwrite = TRUE)
